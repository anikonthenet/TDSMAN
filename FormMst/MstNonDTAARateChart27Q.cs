...
{
    public partial class MstNonDTAARateChart27Q : TDSMAN.FormGen.GenForm
    {
        ResizeForm _form_resize;

        #region System Generated Code
        public MstNonDTAARateChart27Q()
        {
            InitializeComponent();
            //--
        ...
        private void MstTdsRateChartFAyear_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ClearControls();
                //-----------------------------------------------------------
                //lblTitle.Text = "Non DTAA Rate Chart";
                lblTitle.Text = "Section 195 IT Rates under Remittances";
                //-----------------------------------------------------------
                ShowDataInGridBlank();
        ...
        private void ShowDataInGridBlank()
        {
            try
            {
                ...
                string[,] strMatrix = {{"PAR_NON_DTAA_RATE_ID", "0", "", "", "", "F", ""},
                                    {"REMITTANCE_ID", "0", "", "", "", "F", ""},
                                    {"Remittance Code", "300", "", "", "", "", "T"},
                                    {"Remittance", "300", "", "", "", "", "T"},
                                    {"From Date", "300", "", "", "", "", "T"},
                                    {"To Date", "300", "", "", "", "", "T"},
                                    {"Rate", "300", "", "", "", "", "T"}};
                ...
                strSQL = @" SELECT PAR_NON_DTAA_RATE_ID,
                                       MST_REMITTANCE.REMITTANCE_ID, 
	                                   MST_REMITTANCE.REMITTANCE_CODE, 
	                                   MST_REMITTANCE.REMITTANCE_DESC, 
	                                   FROM_DATE, 
	                                   TO_DATE, 
	                                   NON_DTAA_RATE
                              FROM   PAR_NON_DTAA_RATE, MST_REMITTANCE
                              WHERE  PAR_NON_DTAA_RATE.REMITTANCE_ID =  MST_REMITTANCE.REMITTANCE_ID
                              AND    1=2";
                //
        ...
        private void ShowDataInGrid()
        {
            try
            {
                ...
                strSQL = @" SELECT PAR_NON_DTAA_RATE_ID,
	                                   ...
	                                   TO_DATE, 
	                                   NON_DTAA_RATE
                              FROM   PAR_NON_DTAA_RATE, MST_REMITTANCE, MST_ASSESSMENT
                              WHERE  PAR_NON_DTAA_RATE.REMITTANCE_ID =  MST_REMITTANCE.REMITTANCE_ID ";
                if (cmbRemittance.SelectedIndex > 0)
                {
                    strSQL += " AND PAR_NON_DTAA_RATE.REMITTANCE_ID = " + Convert.ToInt32(Support.GetItemData(cmbRemittance, cmbRemittance.SelectedIndex)) + " ";
                }
                //--
                if (cmbFAYear.SelectedIndex > 0)
                {
                ...
                strSQL += @" AND PAR_NON_DTAA_RATE.FROM_DATE <= MST_ASSESSMENT.END_DATE
                               AND PAR_NON_DTAA_RATE.TO_DATE   >= MST_ASSESSMENT.START_DATE "; 
                //-----------------------------------------------------------
                strSQL = strSQL + "ORDER BY MST_REMITTANCE.REMITTANCE_DESC";
                //-----------------------------------------------------------
                if (dsetGridClone != null) dsetGridClone.Clear();
                dsetGridClone = dmlService.J_ShowDataInGrid(ref dgvGrid, strSQL, strMatrix);  //Show Data into the Grid
        ...
        private void BtnRefresh_Click(object sender, EventArgs e)
        {
            ...
            //
            string strCSVPath = "", strfolderName = "DTAA_RATES_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"), strCSVFileName = cmbRemittance.Text + "_" + cmbFAYear.Text + ".csv";            
