...
namespace TDSMAN.Classes
{
    ...
    #region T_ExportReportFormat
    public struct T_ExportReportFormat
    {
    ...
    public class ReportService : IDisposable
    {
        ...
        //-- Added By Abhishek Dey On 20/10/2017 --

        #region J_PopulateGridView [4]
        public bool J_PopulateGridView(DataGridView grdView, string SqlText, string[,] arrColumns, ref ComboBox SerchOnComboBox, bool MultipleColumns, J_Reports enumReport)
        {
            #region Commented
            //try
            //{
            ...
            //    if (enumReport == J_Reports.CertForm1624Q || enumReport == J_Reports.CertForm1624QPartB || enumReport == J_Reports.CertForm16A26Q || enumReport == J_Reports.CertForm16A27Q || enumReport == J_Reports.CertForm27D27EQ)
            //        intColumnIndex = 0;
            ...
            if (dataset.Tables[0].Rows.Count > 0)
            {
                ...
                switch (ExportReportFormat)
                {
                    case T_ExportReportFormat.PortableDocFormat:
                        strFileWithPath = Path.Combine(strFilePath, ExportFileName) + "." + T_ExportReportFormat.PortableDocFormat;
                    ...
                    case T_ExportReportFormat.Excel:
                        strFileWithPath = Path.Combine(Path.Combine(strFilePath, ExportFileName), "." + T_ExportReportFormat.Excel);
                        reportClass.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.Excel, strFileWithPath);
                        break;
                    case T_ExportReportFormat.Word:
                        strFileWithPath = Path.Combine(Path.Combine(strFilePath, ExportFileName), "." + T_ExportReportFormat.Word);
        ...
        public bool J_ExportReport(ref ReportClass reportClass, Form ReportDialog, DataSet dataset, string CompanyName, string CompanyAddress, string ReportTitle, J_PrintType enmPrintType, string ExportReportFormat, string ExportFileName, string ExportFilePath)
        {
            ...
            if (dataset.Tables[0].Rows.Count > 0)
            {
                ...
                strFileWithPath = strFilePath + "1" + "." + T_ExportReportFormat.PortableDocFormat;
                reportClass.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, strFileWithPath);
