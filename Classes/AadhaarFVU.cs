    ...
    public class FVUSubmissionByAdhar
    {
        ...
        public TracesResponse makeLogin(string UserID, string Pwd, string strCaptcha)
        {
            ...
            try
            {
                ...
                //
                if (IsConditionMatch(strServerResponse, "Invalid Password. Please retry."))
                {
                    objResponse.Respons = enmResponse.Failed;
                    objResponse.Message = "Invalid Password. Please retry.";
                    return objResponse;
                }
               ...
               if (IsStringExists(strServerResponse, "//form[@id=\"AuthUpdateAddressDetailsAction\"]"))
                {
                    IsSessionExists = false;
                    objResponse.Respons = enmResponse.Failed;
                    objResponse.Message = "You have not updated the Contact/Address details. Please update it first by logging in Income Tax Department website through your browser then file return using TDSMAN.";
                    return objResponse;
                }
        ...
        //    objNameval = TraceViewStateData(strServerResponse, "//form[@id=\"UploadTdsParamValidate\"]");
        //    //CHECKING ANY ERROR
        //    if (objNameval.Count <= 0)
        //    {
        //        // this.Logoff();
        ...
        //    Match match = Regex.Match(strServerResponse, "Aadhaar OTP request sent to UIDAI", RegexOptions.IgnoreCase);


        //    if (match.Success)
        //    {
        //        //respo.Message = "Aadhaar OTP has been generated successfully";
        //        respo.Message = "Aadhaar OTP request sent to UIDAI. Your principal contact will be receiving OTP from UIDAI shortly to the mobile number registered with Aadhaar.";
        //        respo.Respons = enmResponse.Success;
        //        respo.CustomeTypes = strSessionID;
        //        //--------------------------------------------------------------------------------------
        //        return respo;
        ...
        //    else //-- 2018/05/04
        //    {
        //        match = Regex.Match(strServerResponse, "Aadhaar OTP has been generated successfully", RegexOptions.IgnoreCase);
        //        if (match.Success)
        //        {
        //            respo.Message = "Aadhaar OTP has been generated successfully";
        //            respo.Respons = enmResponse.Success;
        ...
        public TracesResponse getOTPfromIncomeTaxSite(ParamAdhar paramData)
        {
            ...
            {
                //-- 2018/07/31 - ANIK - ####
                ////Verification PAN in the uploaded XML does not match with the existing principal contact PAN.
                match = Regex.Match(strServerResponse, "Verification PAN in the uploaded XML does not match with the existing principal contact PAN", RegexOptions.IgnoreCase);
                if (match.Success)
                {
                    respo.Message = "Your Principal Contact's PAN and Aadhaar are not linked";
                    respo.Respons = enmResponse.Failed;
                    //respo.CustomeTypes = strSessionID;
                    //--------------------------------------------------------------------------------------
                    return respo;
