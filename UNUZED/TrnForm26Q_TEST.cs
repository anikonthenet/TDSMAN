...
{
    public partial class TrnForm26Q_TEST : TDSMAN.FormGen.GenForm
    {
        #region System Generated Code
        public TrnForm26Q_TEST()
        {
            InitializeComponent();
        }
        #endregion
        ...
        int intSectionId = 0;
        int AsstId = 0;
        string strDateSeperator = "";
        bool blnIsDedShowRecDtRdrOpen = false;
        ...
        #region TrnForm26Q_TEST_Load

        private void TrnForm26Q_TEST_Load(object sender, EventArgs e)
        {
            try
            {
                GC.Collect();
            ...
            private void BtnSort_Click(object sender, EventArgs e)
            {
                try
                {
                    ...
                    else if (lblSearchMode.Text == Tabbed_Mode.Deductee)
                    {
                        ...
                        //DISBALE DEDUCTED DATE CONTROLS
        ...
        private void cmbMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //ADDED BY DHRUB FOR LATE DEDUCTION & SHORT DEDUCTION BUTTON VISIBILITY
            if ((cmbFinancialYear.SelectedIndex > 0) && (cmnService.J_GetComboBoxItemId(ref cmbFinancialYear, cmbFinancialYear.SelectedIndex) > 4)) // ASST_ID > F.Y 2009-2010
            {
                btnShortDeduction.Visible = true;
                btnLateDeduction.Visible = true;
            }
            else
            {
                btnShortDeduction.Visible = false;
                btnLateDeduction.Visible = false;
            }
        }
        #endregion
        ...
        private void lnkViewCompanyDetails_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            ViewForm.MdiParent = TrnForm26Q_TEST.ActiveForm;
            ViewForm.Show();
        ...
        private void dgcViewDeductee_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (dgcViewDeductee.CurrentRowIndex >= 0)
                {
                    ...
                    //--------------------------------------------------
                    txtDeducteeAmountOfPayment_TextChanged(sender, e);
                    lblMode.Text = J_Mode.Edit;
                    cmnService.J_StatusButton(this, lblMode.Text);
        ...
        private void txtDeducteePAN_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                //------
                //ADDED BY DHRUB ON 04/12/2013 FOR TDS CALCULATION COMPANY & NONCOMPANY WISE 
                lblTDSCalculatedAmt.Visible = false;
                ...
                if (txtDeducteePAN.Text == "PANNOTAVBL" && Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2010_11ID)
                {
                    ...
                    //------
                    //ADDED BY DHRUB ON 04/12/2013 FOR TDS CALCULATION COMPANY & NONCOMPANY WISE 
                    if (blnIsDedShowRecDtRdrOpen == false)
                    {
                        CalculateTDS();
                    }
                }
        ...
        private void lstDeducteeHelp_Click(object sender, EventArgs e)
        {
            ...
            //ADDED BY DHRUB  ON 05/12/2013 FOR 
        ...
        private void TrnForm26Q_TEST_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        //--Dhrub @2013/12/09 for Short Deduction and Late Payments Report
        #region btnShortDeduction_Click
        private void btnShortDeduction_Click(object sender, EventArgs e)
        {
            
            //--------------------------------------------------------------------------------------
            //--SHORT DEDUCTION FOR DEDUCTEE EXCEPT FORM_NO - 24Q [ADDED BY DHRUB ON 11/12/2013]
            //--------------------------------------------------------------------------------------
            TdsMan.T_ShortDeduction(Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbCompany, cmbCompany.SelectedIndex)), Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbFinancialYear, cmbFinancialYear.SelectedIndex)), Convert.ToString(cmbQuarter.Text), Convert.ToString(T_FormNo.F26Q));
            //
            TDSMAN.Classes.TDSMAN.T_pDeductionType = T_DeductionTypeStructure.ShortDeduction;

            TrnShortAndLateDeductionSummary objTrnDeduction = new TrnShortAndLateDeductionSummary(strFormNo, Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbCompany, cmbCompany.SelectedIndex)), Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbFinancialYear, cmbFinancialYear.SelectedIndex)),cmbQuarter.Text);
            objTrnDeduction.ShowDialog();
            this.Refresh();


            #region COMMENTED CODE FOR SHORT DEDUCTION 
...
//            strSQL = @"SELECT  BASEQUERY.COMPANY_NAME,
//                               BASEQUERY.FA_YEAR,
//                               BASEQUERY.QTR,
//                               BASEQUERY.FORM_NO,
//                               BASEQUERY.CHALLAN_ID, 
//                               BASEQUERY.CHALLAN_NO,
//                               BASEQUERY.PAYMENT_DATE, 
//                               BASEQUERY.DEDUCTEE_DETAIL_ID, 
//                               BASEQUERY.DEDUCTEE_SERIAL_NO,                               
//                               BASEQUERY.PARTY_ID, 
//                               BASEQUERY.DEDUCTEE_NAME,
//                               BASEQUERY.DEDUCTED_DATE, 
//                               BASEQUERY.PAYMENT_AMOUNT, 
//                               BASEQUERY.REASON, 
//                               BASEQUERY.DESCRIPTION, 
//                               BASEQUERY.SECTION_ID, 
//                               BASEQUERY.COMPANY_RATE, 
//                               BASEQUERY.NON_COMPANY_RATE, 
//                               BASEQUERY.INVALID_PAN_RATE, 
//                               BASEQUERY.TAX_DEPOSITED_AMOUNT,  
//                               BASEQUERY.DEDUCTEE_PAN,
//                               BASEQUERY.RATE,
//                               BASEQUERY.ACTUAL_TAX_AMOUNT
//                       FROM 
//                       (
//                           SELECT MST_COMPANY.COMPANY_ID,
...
//                                  TRN_DEDUCTEE_DETAILS.DEDUCTEE_DETAIL_ID,
//                                  TRN_DEDUCTEE_DETAILS.SL_NO                                      AS DEDUCTEE_SERIAL_NO,      
//                                  TRN_DEDUCTEE_DETAILS.PARTY_ID,
//                                  TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE, 
//                                  TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT, 
//                                  ROUND(TRN_DEDUCTEE_DETAILS.TAX_DEPOSITED_AMOUNT /
//                                        TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * 100, 2)             AS CALC_RATE, 
...
//                                             OR MST_DEDUCTEE.DEDUCTEE_PAN = 'PANAPPLIED' 
//                                             OR MST_DEDUCTEE.DEDUCTEE_PAN = 'PANINVALID', 
//                                      MST_TDS_NONSALARY_SLAB.INVALID_PAN_RATE,  
//                                           IIF(MID(MST_DEDUCTEE.DEDUCTEE_PAN, 3, 1) = 'C', 
//                                               MST_TDS_NONSALARY_SLAB.COMPANY_RATE, 
//                                               MST_TDS_NONSALARY_SLAB.NON_COMPANY_RATE)
//                                      )          AS RATE, 
//                                  IIF(MST_DEDUCTEE.DEDUCTEE_PAN = 'PANNOTAVBL' 
//                                             OR MST_DEDUCTEE.DEDUCTEE_PAN = 'PANAPPLIED' 
//                                             OR MST_DEDUCTEE.DEDUCTEE_PAN = 'PANINVALID', 
//                                      ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_TDS_NONSALARY_SLAB.INVALID_PAN_RATE ) / 100,0), 
//                                           IIF(MID(MST_DEDUCTEE.DEDUCTEE_PAN, 3, 1) = 'C', 
//                                               ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_TDS_NONSALARY_SLAB.COMPANY_RATE ) / 100,0), 
//                                               ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_TDS_NONSALARY_SLAB.NON_COMPANY_RATE ) / 100,0))
//                                      ) AS ACTUAL_TAX_AMOUNT,
//                                  MST_REASON.REASON, 
//                                  MST_REASON.DESCRIPTION, 
//                                  MST_TDS_NONSALARY_SLAB.SECTION_ID, 
//                                  MST_TDS_NONSALARY_SLAB.COMPANY_RATE, 
//                                  MST_TDS_NONSALARY_SLAB.NON_COMPANY_RATE, 
//                                  MST_TDS_NONSALARY_SLAB.INVALID_PAN_RATE
...
//                                  MST_DEDUCTEE, 
//                                  MST_REASON, 
//                                  MST_TDS_NONSALARY_SLAB,
//                                  TRN_BASIC_INFO,
//                                  TRN_CHALLAN,
//                                  MST_COMPANY,
//                                  MST_ASSESSMENT
//                           WHERE  TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE IS NOT NULL 
//                           AND    TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE >= " + cmnService.J_DateOperator() + dtService.J_ConvertMMddyyyy(strShortDeductionDefaultDate) + cmnService.J_DateOperator() + " " +
//                         @"AND    TRN_DEDUCTEE_DETAILS.PARTY_ID = MST_DEDUCTEE.DEDUCTEE_ID 
//                           AND    TRN_DEDUCTEE_DETAILS.REASON_ID = MST_REASON.REASON_ID 
//                           AND    TRN_DEDUCTEE_DETAILS.SECTION_ID = MST_TDS_NONSALARY_SLAB.SECTION_ID 
//                           AND    TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE BETWEEN  MST_TDS_NONSALARY_SLAB.FROM_DATE AND MST_TDS_NONSALARY_SLAB.TO_DATE 
...
//                           AND    TRN_BASIC_INFO.COMPANY_ID = MST_COMPANY.COMPANY_ID
//                           AND    TRN_BASIC_INFO.ASST_ID = MST_ASSESSMENT.ASST_ID ";
//            if(cmbCompany.SelectedIndex >0 )
//                strSQL = strSQL + " AND TRN_BASIC_INFO.COMPANY_ID = " + cmnService.J_GetComboBoxItemId(ref cmbCompany, cmbCompany.SelectedIndex);
//            if (cmbFinancialYear.SelectedIndex > 0)
//                strSQL = strSQL + " AND TRN_BASIC_INFO.ASST_ID = " + cmnService.J_GetComboBoxItemId(ref cmbFinancialYear, cmbFinancialYear.SelectedIndex);
//            if (cmbQuarter.SelectedIndex > 0)
//                strSQL = strSQL + " AND TRN_BASIC_INFO.QTR = '" + cmbQuarter.Text + "' ";

//            strSQL = strSQL + @")BASEQUERY
//            WHERE     BASEQUERY.TAX_DEPOSITED_AMOUNT < BASEQUERY.ACTUAL_TAX_AMOUNT 
//            ORDER  BY TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE, 
//                      TRN_DEDUCTEE_DETAILS.DEDUCTEE_DETAIL_ID ";
        ...
        #region btnLateDeduction_Click
        private void btnLateDeduction_Click(object sender, EventArgs e)
        {
            //------------------------------------------------------------------------------
            //--LATE DEDUCTION FOR DEDUCTEE [ADDED BY DHRUB ON 09/12/2013]
            //------------------------------------------------------------------------------
            TdsMan.T_LateDeduction(Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbCompany, cmbCompany.SelectedIndex)), Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbFinancialYear, cmbFinancialYear.SelectedIndex)), Convert.ToString(cmbQuarter.Text), Convert.ToString(T_FormNo.F26Q));

            TDSMAN.Classes.TDSMAN.T_pDeductionType = T_DeductionTypeStructure.LateDeduction;

            TrnShortAndLateDeductionSummary objTrnDeduction = new TrnShortAndLateDeductionSummary(strFormNo, Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbCompany, cmbCompany.SelectedIndex)), Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbFinancialYear, cmbFinancialYear.SelectedIndex)), cmbQuarter.Text);
            objTrnDeduction.ShowDialog();
            this.Refresh();

            #region COMMENTED CODE FOR LATE DEDUCTION

            //            string strShortDeductionDefaultDate = "01" + dtService.J_GetSystemDateSeperator() + "04" + dtService.J_GetSystemDateSeperator() + "2010";

            //            //---------------------------------------------------------
            //            //----- LATE DEDUCTION SQL 
            //            //---------------------------------------------------------

            //            string strLastDateCondition = "";

            //            strSQL = @"SELECT BASEQUERY.COMPANY_NAME, 
            //                              BASEQUERY.FA_YEAR, 
            //                              BASEQUERY.CHALLAN_ID, 
            //                              BASEQUERY.DEDUCTEE_ID, 
            //                              BASEQUERY.DEDUCTEE_NAME,
            //                              BASEQUERY.CHALLAN_DEPOSIT_DATE, 
            //                              BASEQUERY.DEDUCTED_DATE, 
            //                              BASEQUERY.LAST_DATE, 
            //                              IIF(DATEDIFF(" + "\"D\"" + ",FORMAT(BASEQUERY.CHALLAN_DEPOSIT_DATE," + "\"DD/MM/YY\"" + ")," +
            //                                                         " FORMAT(BASEQUERY.LAST_DATE," + "\"DD/MM/YY\"" + ") " +
            //                                          ") < 0, 'LATE', '') AS STATUS, " +
            //                             "ABS(DATEDIFF(" + "\"M\"" + ", FORMAT(BASEQUERY.CHALLAN_DEPOSIT_DATE," + "\"DD/MM/YY\"" + ")," +
            //                                                         "  FORMAT(BASEQUERY.LAST_DATE," + "\"DD/MM/YY\"" + ") " +
            //                                          @"))                 AS   DATE_DIFFERENCE                              
            //                       FROM  (SELECT TRN_DEDUCTEE_DETAILS.DEDUCTEE_DETAIL_ID    AS   DEDUCTEE_ID,
            //                                     TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE,
            //                                     MONTH(TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE)  AS   MONTH,
            //                                     IIF(MONTH(TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE) = 3, " +
            //                                               "\"30\"" + "+" + "RIGHT(CDATE(DATEADD(" + "\"M\"" + ", 1, FORMAT(TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE," + "\"DD/MM/YY\"" + "))), 8)," +
            //                                               "\"07\"" + "+" + "RIGHT(CDATE(DATEADD(" + "\"M\"" + ", 1, FORMAT(TRN_DEDUCTEE_DETAILS.DEDUCTED_DATE," + "\"DD/MM/YY\"" + "))), 8) " +
            //                                         ")    AS LAST_DATE, " +
            ...
            //            strSQL = strSQL + ") BASEQUERY " +
            //                      "WHERE  DATEDIFF(" + "\"D\"" + ", FORMAT (BASEQUERY.CHALLAN_DEPOSIT_DATE,"+ "\"DD/MM/YYYY\"" +" ),"+ 
            //                                                       "FORMAT(BASEQUERY.LAST_DATE,"+ "\"DD/MM/YYYY\"" + ")) < 0 "+
            //                      "ORDER  BY BASEQUERY.DEDUCTEE_ID ";
        ...
        #region btnDeducteeInvalidPAN_Click
        private void btnDeducteeInvalidPAN_Click(object sender, EventArgs e)
        {
            TrnDeducteeInvalidPANSummary objTrnDeducteeInvalidPAN = new TrnDeducteeInvalidPANSummary(strFormNo, Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbCompany, cmbCompany.SelectedIndex)), Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbFinancialYear, cmbFinancialYear.SelectedIndex)), cmbQuarter.Text);
            objTrnDeducteeInvalidPAN.ShowDialog();
            this.Refresh();
        }
        #endregion 
        ...
        private bool ShowDeducteeRecord(long Id)
        {
            ...
            try
            {
                ...
                {
                    //Added By dhrub on 04/12/2013 For Reader IS open Checking
                    blnIsDedShowRecDtRdrOpen = true;
                    ...
                    txtDeducteeName.Select();
                    blnIsDedShowRecDtRdrOpen = false; 
                    return true;
                }
                //-----------------------------------------------------------
                drdShowDeducteeRecord.Close();
        ...
        private void CalculateTDS()
        {
            ...
            //ADDED BY DHRUB ON 04/12/2013 FOR TDS CALCULATION COMPANY & NONCOMPANY WISE 
            //Section Combobox  
            intAsstId = Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex);

            if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) >= T_FinancialYearID.F2013_14ID)
            {
                if (Convert.ToInt32(cmbDDSection.SelectedIndex) <= 0)
                {
                    lblTDSCalculatedAmt.Visible = false;
                    return;
                }
                ...
                if (txtViewChallanDetailsSection.Text == "") return;

            //Deductee Payment Amount Checking
            if (cmnService.J_IsNumeric(txtDeducteeAmountOfPayment.Text) == false)
            {
                lblTDSCalculatedAmt.Visible = false;
                return;
            }
            if (Convert.ToDouble(txtDeducteeAmountOfPayment.Text) <= 0)
            {
                lblTDSCalculatedAmt.Visible = false;
                return;
            }
            ...
            if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) >= T_FinancialYearID.F2013_14ID)
                intSectionId = Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbDDSection, cmbDDSection.SelectedIndex));
            else
            {
                //strSQL = "SELECT  SECTION_ID  FROM MST_SECTION WHERE SECTION_NO ='" + txtViewChallanDetailsSection.Text + "'";
                intSectionId = Convert.ToInt32(dmlService.J_ExecSqlReturnScalar("SELECT  SECTION_ID  FROM MST_SECTION WHERE SECTION_NO ='" + txtViewChallanDetailsSection.Text + "'"));
            }

            //DEDUCTEE DATE BLANK CHECKING 
            strDateSeperator = dtService.J_GetSystemDateSeperator();
            if (mskDeducteeDate.Text == "  " + strDateSeperator + "  " + strDateSeperator)
            {
                lblTDSCalculatedAmt.Visible = false;
                return;
            }
            //
            lblTDSCalculatedAmt.Visible = true;
            //lblTDSCalculatedAmt.Text = string.Format("{0:0.00}",TdsMan.CalcTDSForCompanyNoncompany(intSectionId, mskDeducteeDate.Text, Convert.ToDouble(txtDeducteeAmountOfPayment.Text), txtDeducteePAN.Text));
            if (lblTDSCalculatedAmt.Text == "0.00")
                lblTDSCalculatedAmt.Visible = false;
            //--------
        }
        #endregion
