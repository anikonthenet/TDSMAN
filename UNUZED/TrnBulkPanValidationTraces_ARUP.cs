...
{
    public partial class TrnBulkPanValidationTraces_ARUP : TDSMAN.FormGen.GenForm
    {
        #region System Generated Code
        public TrnBulkPanValidationTraces_ARUP()
        {
            InitializeComponent();
            ...
            switch (enReqType)
            {
                ...
                case enmRequestType.PanValidation:
                    ...
                    foreach (DataGridViewRow row in rowcoll)
                    {
                        ...
                        TracesResponse response = objAccount.RequestForPANValidation(strPAN);
        ...
        void PopulateDatagridView(DataTable dsRecords)
        {
            ...
            grdPANList.Columns.Clear();
            grdPANList.DataSource = null;
            grdPANList.DataSource = dsRecords;
            //---------------------------------------
        ...
        {
            string   strFVUPath = cmnService.J_OpenFileDialog("Import File | *.xls; *.xlsx", "Import File | *.xls; *.xlsx", "Choose the File to import");
        ...
        private void btnValidate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Reset
                _results.Clear();
                this.objBindingSource.DataSource = _results;

                ArrayList objList = new ArrayList();
                objList.Add(enmRequestType.PanValidation);
                objList.Add(grdPANList.Rows);
                ////-------------------------------------------
                pgTimer.Start();
                ////-------------------------------------------
               ...
               // DataGridViewRow PANrow;

                //foreach (DataGridViewRow row in grdPANList.Rows)
                //{
                //    string strPAN = row.Cells[0].Value.ToString();
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(txtFVUPath.Text)) return;
                DataSet dsRecord = new DataSet();
                OleDbConnection con = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Extended Properties=Excel 8.0;Data Source=" + txtFVUPath.Text);
                OleDbCommand com = new OleDbCommand("SELECT * FROM [Sheet1$]", con);
                OleDbDataAdapter adap = new OleDbDataAdapter();
                adap.SelectCommand = com;

                adap.Fill(dsRecord);
                PopulatePANList(dsRecord.Tables[0]);
            }
            catch
            {
            }
        ...
        #region PopulatePANList
        void PopulatePANList(DataTable dsRecords)
        {
            // DATA BIND TO GRIDVIEW CONTROL
            grdPANList.Columns.Clear();
            grdPANList.DataSource = null;
