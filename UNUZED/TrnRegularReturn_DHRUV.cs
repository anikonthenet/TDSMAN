...
namespace TDSMAN.FormTrn
{
    ...
    public partial class TrnRegularReturn_DHRUV : TDSMAN.FormGen.GenForm
    {
        #region System Generated Code
        public TrnRegularReturn_DHRUV()
        {
            InitializeComponent();
        }
        #endregion
        ...
        //--For Company Details------------------Added by dhrub on 19/11/2013 For CompanyDetails  
        ...
        string strAltEmail = "";
        string strAltRPSTD = "";
        string strAltRPPhone = "";
        string strAltRPEmail = "";
        string strCompanyPrefix = "";
        string strDStateName = "";
        //---ID
        int intDeductorTypeId = 0;
        int intStateId = 0;
        int intMinistryId = 0;
        int intRPStateId = 0;
        int intDeductorStateId = 0;
        //-------------------------------------------------
        ...
        private struct Tabbed_Mode
        {
            ...
            public const string Deductee = "Employee Details";
            public const string GenerateTDS = "Generate TDS Return";
        }
        #endregion
        //----
        string strFormNo = "";
        //added by dhrub on 19/11/2013
        int intBookmark = 0;
        ...
        #region set SALARY DETAIL Index through ENUM
        ...
        public enum enmSALARY_DETAILIndex
        {
            ...
            INCOME_CHARGEABLE = 9,
            AIS_TOTAL = 10,
            GROSS_TOTAL_INCOME = 11,
            CVIA_DED_TOTAL = 12,
            TOTAL_INCOME = 13,
            TAX_TOTAL_INCOME = 14,
            SCHG_TOTAL_INCOME = 15,
            ECESS_TOTAL_INCOME = 16,
            TAX_PAYABLE_AGGREGATE = 17,
            US_89_LESS = 18,
            TAX_PAYABLE = 19,
            TOTAL_TDS_DEDUCTED = 20,
            SHORTFALL_TAX = 21
        }
        #endregion
        ...
        #region TrnRegularReturn_Load

        private void TrnRegularReturn_Load(object sender, EventArgs e)
        {
            try
            {
                GC.Collect();
                ...
                //----------------------
                //Initialize the Form No
                strFormNo = TDSMAN.Classes.TDSMAN.T_pTabFormCaption;
                //-----------------------------------------------------------
                lblMode.Text = J_Mode.View;
                cmnService.J_StatusButton(this, lblMode.Text);
                BtnExit.Text = "E&xit from " + strFormNo;
                //-----------------------------------------------------------
                //Form wise Title Caption 
                //-----------------------------------------------------------
                lblTitle.Text = "Form " + strFormNo; 
                if (strFormNo == "24Q")
                    lblTitle.Text = lblTitle.Text + " (Salary)";
                else if (strFormNo == "26Q")
                    lblTitle.Text = lblTitle.Text + " (Domestic)";
                else if (strFormNo == "27Q")
                    lblTitle.Text = lblTitle.Text + " (Foreign)";
                else if (strFormNo == "27EQ")
                    lblTitle.Text = lblTitle.Text + " (TCS)";
                //-----------------------------------------------------------
                lblRegularStatementFiled.Text = lblRegularStatementFiled.Text + "(" + strFormNo + ") earlier";
                lblPrevTokenNo.Text = lblPrevTokenNo.Text +" ("+ strFormNo+ ")";
                //-----------------------------------------------------------
                ...
                //-----------------------------------------------------------
                //------------------------------------
                //ADDED BY DHRUB FOR BOOKMARK HISTORY on 08/11/2013
        ...
        #region TrnRegularReturn_Activated
        private void TrnRegularReturn_Activated(object sender, EventArgs e)
        {
            ...
            //
            if (cmnService.J_ReturnDoubleValue(lblNetTaxableIncome.Text) > 0)
            {
                //--
        ...
        private void BtnEdit_Click(object sender, System.EventArgs e)
        {
            ...
            {
                //Added by dhrub on 21/11/2013 no records found and clicked the edit mode 
                if (lngChallanID>0)
                    //Added by Indrajit on 13-02-2013
                    //--------------------------------------------------
                    if (Check_Record(lngChallanID, "TRN_CHALLAN", "CHALLAN_ID", ref dgcViewChallan) == 0) return;
                //---
        ...
        {
            //Added by Indrajit on 13-02-2013 to refresh challan info on deductee screen
            ...
            //--
            //Modified by Indrajit on 13-02-2013
            //Insert_Update_Delete_Data();
            Insert_Update_Delete_Data(sender, e);
        ...
        #region BACK
            ...
            private void BtnCancel_Click(object sender, System.EventArgs e)
            {
                try
                {
                    ...
                    //Added by Indrajit on 23-02-2013 to incorporate Basic Info locking
                    #region INCORPORATE_BASIC_NFO_LOCKING

                    TDSMAN.Classes.TDSMAN.T_pBasicInfoId = lngBasicInfoID;
                    //================================================
                    if (TdsMan.LockBasicInfoEntry(T_FormNo.F24Q) == false)
                    {
                        BtnExit.Select();
                        return;
                    }
                    ...
                    if (lblSearchMode.Text == Tabbed_Mode.Basic)
                    {
                        ...
                        if (TDSMAN.Classes.TDSMAN.T_pVersionType == T_VERSION_TYPE.TRIAL_VERSION)
                        {
                            ...
                            {
                                cmnService.J_UserMessage(TDSMAN.Classes.TDSMAN.T_pVersionTypeMessage);
                                return;
                            }
                        }
                        ...
                        {
                            cmnService.J_UserMessage("Enter Token No. of previous regular statement (24Q)");
                    ...
                    else
                    {
                        ...
                        BtnSave.BackColor = Color.LightGray;
                        //--
                        btnChallanDeducteeEntry.Text = "Challan && Employee Entry";
                        //--
            ...
            private void BtnSort_Click(object sender, EventArgs e)
            {
                try
                {
                    if (lblSearchMode.Text == Tabbed_Mode.Challan)
                    {
                        ...
                        ClearControls();
                        //--
                        lblChallanInterestAllocated.Visible = false;
                        txtInterestAllocated.Visible = false;
                        lblChallanOthersAllocated.Visible = false;
                        txtOthersAllocated.Visible = false;
                        //lblChallanRemarks.Location = new Point(14, 334);
                    ...
                    else if (lblSearchMode.Text == Tabbed_Mode.Basic)
                    {
                        ...
                        {
                            btnSalaryDetail.Visible = false;
                        }
                    }
                    //Blocked by Indrajit on 23-02-2013
                    BtnRefresh_Click(sender, e);
                }
                catch (Exception err_handler)
                {
        ...
        private void cmbMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //--
            ////Added by Indrajit on 21-02-2013 to incorporate Basic Info locking
            //#region INCORPORATE_BASIC_NFO_LOCKING

            //TDSMAN.Classes.TDSMAN.T_pBasicInfoId = lngBasicInfoID;
            ////================================================
            //if (TdsMan.LockBasicInfoEntry(T_MODULENAME.FORM_24Q) == false)
            //{
        ...
        private void tbcCompany_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (tbcCompany.SelectedTab == tbpChallanDetails)
            {
                ...
                //--
                if (strDeductorType.Substring(0, 1) == "A" || strDeductorType.Substring(0, 1) == "S")
                {
        ...
        private void btnChallanEntry_Click(object sender, EventArgs e)
        {
            ...
            strSQL = " SELECT SECTION_ID," +
                ...
                "      AND    SECTION_TYPE = 'NOP' " +
                "      ORDER BY SECTION_ID";
            if (dmlService.J_PopulateComboBox(strSQL, ref cmbSection) == false) return;
            //-----------
            if (strDeductorType.Substring(0, 1) == "A" ||
                strDeductorType.Substring(0, 1) == "S" ||
                strDeductorType.Substring(0, 1) == "D" ||
                strDeductorType.Substring(0, 1) == "E" ||
                strDeductorType.Substring(0, 1) == "G" ||
                strDeductorType.Substring(0, 1) == "H" ||
                strDeductorType.Substring(0, 1) == "L" ||
                strDeductorType.Substring(0, 1) == "N")
                cmbSection.Text = T_24Q_SectionNo.Section192A;
            else
                cmbSection.Text = T_24Q_SectionNo.Section192B;
            //----------
            ...
            if (strDeductorType.Substring(0, 1) == "A" || strDeductorType.Substring(0, 1) == "S")
            {
                ...
                //txtTransferVoucherNo.BackColor = Color.White;
                //
                chkChallanBookEntry.Visible = true;
                lblChallanSrlNo.Location = new Point(17, 14);
            ...
            else
            {
                ...
                //txtTransferVoucherNo.BackColor = Color.LightGray;
                //
                chkChallanBookEntry.Visible = false;
                lblChallanSrlNo.Location = new Point(85, 14);
        ...
        private void lnkViewCompanyDetails_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            ViewForm.MdiParent = TrnRegularReturn_DHRUV.ActiveForm;
            ViewForm.Show();
        }
        #endregion
        ...
        private void BtnSearch_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    //-----------------------------
                    //grpSearch Controls Alignment[Added by dhrub on 21/11/2013]
                    lblSearchSrlNo.Location = new Point(lblSearchSrlNo.Location.X, lblSearchSrlNo.Location.Y);
                    txtSlNoSearch.Location = new Point(txtSlNoSearch.Location.X, txtSlNoSearch.Location.Y);
                    ...
                    if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) <= T_FinancialYearID.F2012_13ID)
                    {
                        ...
                        label109.Location = new Point(label109.Location.X, 49);
                        cmbSectionNoSearch.Location = new Point(cmbSectionNoSearch.Location.X, 46);
                        label108.Location = new Point(label108.Location.X, 77);
                        txtChallanNoSearch.Location = new Point(txtChallanNoSearch.Location.X, 73);
                        label113.Location = new Point(label113.Location.X, 102);
                        txtTransferVoucherNoSearch.Location = new Point(txtTransferVoucherNoSearch.Location.X, 98);
                        label111.Location = new Point(label111.Location.X, 129);
                        mskDepositDateSearch.Location = new Point(mskDepositDateSearch.Location.X, 123);
                        label112.Location = new Point(label112.Location.X, 129);

                        BtnSearchOK.Location = new Point(BtnSearchOK.Location.X, 163);
                        BtnSearchCancel.Location = new Point(BtnSearchCancel.Location.X, 163);

                        grpSearch.Height = 198;
                        grpSearch.Location = new Point(grpSearch.Location.X, grpSearch.Location.Y);
                    ...
                    else
                    {
                        ...
                        label108.Location = new Point(label108.Location.X, 49 );
                        txtChallanNoSearch.Location = new Point(txtChallanNoSearch.Location.X, 46 );
                        label113.Location = new Point(label113.Location.X, 77 );
                        txtTransferVoucherNoSearch.Location = new Point(txtTransferVoucherNoSearch.Location.X, 73 );
                        label111.Location = new Point(label111.Location.X, 102);
                        mskDepositDateSearch.Location = new Point(mskDepositDateSearch.Location.X, 98 );
                        label112.Location = new Point(label112.Location.X, 102 );
                        BtnSearchOK.Location = new Point(BtnSearchOK.Location.X, 127);
                        BtnSearchCancel.Location = new Point(BtnSearchCancel.Location.X, 127);

                        grpSearch.Height = 165;
                        grpSearch.Location = new Point(grpSearch.Location.X, grpSearch.Location.Y);
            ...
            try
            {
                ...
                else if (lblSearchMode.Text == Tabbed_Mode.Deductee)
                {
                    ...
                    if (txtPANSearch.Text.Trim() != "")
                        strCheckFields = strCheckFields + "AND MST_EMPLOYEE.EMPLOYEE_PAN ='" + cmnService.J_ReplaceQuote(txtPANSearch.Text.Trim().ToUpper()) + "' ";
                    //----------------------------------------------------------------------
                    //-- TRANSFER VOUCHER NO
                    //----------------------------------------------------------------------
                    if (txtDeducteeNameSearch.Text.Trim() != "")
                        strCheckFields = strCheckFields + "AND MST_EMPLOYEE.EMPLOYEE_NAME like '" + cmnService.J_ReplaceQuote(txtDeducteeNameSearch.Text.Trim().ToUpper()) + "%' ";
                    //----------------------------------------------------------------------
                    //-- DEPOSIT DATE
                    //----------------------------------------------------------------------
        ...
        private void dgcViewChallan_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (dgcViewChallan.CurrentRowIndex >= 0)
                {
                    ...
                    btnChallanDeducteeEntry.BackColor = Color.LightGray;
                    //--
                    lblChallanInterestAllocated.Visible = true;
                    txtInterestAllocated.Visible = true;
                    lblChallanOthersAllocated.Visible = true;
                    txtOthersAllocated.Visible = true;
                    //lblChallanRemarks.Location = new Point(14, 383);
        ...
        {
            if(chkChallanBookEntry.Checked == true)
            {
                //BSR Code/24G Rcpt # [410]
                if (strFormNo == T_FormNo.F24Q)
                {
                    lblChallanBSRCode.Text = "24G Receipt No. [309]";
                    lblChallanTrVchNo.Text = "Tr Vch(DDO Sl.) [310]";
                }
                if (strFormNo == T_FormNo.F26Q)
                {
                    lblChallanBSRCode.Text = "24G Receipt No. [408]";
                    lblChallanTrVchNo.Text = "Tr Vch(DDO Sl.) [409]";
                }
                if (strFormNo == T_FormNo.F27Q)
                {
                    lblChallanBSRCode.Text = "24G Receipt No. [710]";
                    lblChallanTrVchNo.Text = "Tr Vch(DDO Sl.) [711]";
                }
                if (strFormNo == T_FormNo.F27EQ)
                {
                    lblChallanBSRCode.Text = "24G Receipt No. [660]";
                    lblChallanTrVchNo.Text = "Tr Vch(DDO Sl.) [661]";
                }
                if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) <= T_FinancialYearID.F2012_13ID)
                {
                    if ((strFormNo == T_FormNo.F24Q) || (strFormNo == T_FormNo.F26Q))
                    {
                        lblChallanBSRCode.Location = new Point(lblChallanBSRCode.Location.X, 35);
                        lblChallanTrVchNo.Location = new Point(lblChallanTrVchNo.Location.X, 57);
                    }
                    if ((strFormNo == T_FormNo.F27Q) || (strFormNo == T_FormNo.F27EQ))
                    {
                        lblChallanBSRCode.Location = new Point(lblChallanBSRCode.Location.X, 93);
                        lblChallanTrVchNo.Location = new Point(lblChallanTrVchNo.Location.X, 115);
                    }
                }
                else
                {
                    if ((strFormNo == T_FormNo.F24Q) || (strFormNo == T_FormNo.F26Q))
                    {
                        lblChallanBSRCode.Location = new Point(lblChallanBSRCode.Location.X, 37);
                        lblChallanTrVchNo.Location = new Point(lblChallanTrVchNo.Location.X, 61);
                    }
                    if ((strFormNo == T_FormNo.F27Q) || (strFormNo == T_FormNo.F27EQ))
                    {
                        lblChallanBSRCode.Location = new Point(lblChallanBSRCode.Location.X, 50);
                        lblChallanTrVchNo.Location = new Point(lblChallanTrVchNo.Location.X, 72);
                    }               
                }
            ...
            else if (chkChallanBookEntry.Checked == false)
            {

                if (strFormNo == T_FormNo.F24Q)
                {
                    lblChallanBSRCode.Text = "BSR Code [309]";
                    lblChallanTrVchNo.Text = "Challan No. [310]";
                }
                if (strFormNo == T_FormNo.F26Q)
                {
                    lblChallanBSRCode.Text = "BSR Code [408]";
                    lblChallanTrVchNo.Text = "Challan No. [409]";
                }
                if (strFormNo == T_FormNo.F27Q)
                {
                    lblChallanBSRCode.Text = "BSR Code [710]";
                    lblChallanTrVchNo.Text = "Challan No. [711]";
                }
                if (strFormNo == T_FormNo.F27EQ)
                {
                    lblChallanBSRCode.Text = "BSR Code [660]";
                    lblChallanTrVchNo.Text = "Challan No. [661]";
                }
        ...
        private void cmbSection_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                lblChallanSectionDisplay.Visible = false;
                return;
            }

            lblChallanSectionDisplay.Visible = true;
            lblChallanSectionDisplay.Text = TdsMan.T_ReplaceAmpersand(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT SECTION_DESCRIPTION FROM MST_SECTION WHERE SECTION_ID = " + Convert.ToInt32(Support.GetItemData(cmbSection, cmbSection.SelectedIndex)))));
        }
        #endregion
        ...
        {
            if (Convert.ToInt64(e.KeyChar) == 13) SendKeys.Send("{tab}");
            if (mainVB.gTextBoxValidation(Convert.ToInt16(e.KeyChar), "N,12,2", txtOthersAllocated, "") == false)
                e.Handled = true;
        }
        #endregion
        ...
        {
             //-----------------------------------------------------
             // BOOKMARK COMMON FOR ALL REGULAR RETURN FORMS  Added by Dhrub On 08/11/2013 
             //------------------------------------------------------
             if (TDSMAN.Classes.TDSMAN.T_pBookMarkOption == true)
             {
                 strQuery = @"FORM_NAME= '" + strFormNo + "' ";
            ...
            //-----------------------------------------
            //Added by Indrajit on 13-02-2013 to add the basic information
            //-----------------------------------------------------------
            if (lngBasicInfoID == 0)
            {
                dmlService.J_BeginTransaction();
            ...
            //Incorporate Basic Info locking for four forms
            #region INCORPORATE_BASIC_NFO_LOCKING

            TDSMAN.Classes.TDSMAN.T_pBasicInfoId = lngBasicInfoID;
            //================================================
            if ((TdsMan.LockBasicInfoEntry(T_FormNo.F24Q) == false)||(TdsMan.LockBasicInfoEntry(T_FormNo.F26Q) == false)||
                (TdsMan.LockBasicInfoEntry(T_FormNo.F27Q) == false))
            {
                BtnExit.Select();
                return;
            }
            ...
            if (lblSearchMode.Text == Tabbed_Mode.Basic)
            {
                ...
                if (strFormNo == T_FormNo.F24Q)
                    btnChallanDeducteeEntry.Text = "Employee Details";
                else if ((strFormNo == T_FormNo.F26Q) || (strFormNo == T_FormNo.F27Q) || (strFormNo == T_FormNo.F27EQ))
                ...
                //Form wise visibility of Challan controls
                lblChallanInterestAllocated.Visible = false;
                txtInterestAllocated.Visible = false;
                lblChallanOthersAllocated.Visible = false;
                txtOthersAllocated.Visible = false;
            ...
            else if (lblSearchMode.Text == Tabbed_Mode.Challan)
            {
                ...
                strSQL = " SELECT SECTION_ID," +
                    "             SECTION_NO " +
                    "      FROM   MST_SECTION " +
                    "      WHERE  FORM_NAME ='" + T_FormNo.F24Q + "' " +
                    "      ORDER BY SECTION_ID";
                if (dmlService.J_PopulateComboBox(strSQL, ref cmbDDSection) == false) return;
                //-----------
                grpSearch.Visible = false;
                //Initialize the DeducteeRate for Rest of 24Q form
                if (strFormNo != T_FormNo.F24Q)
                ...
                //---------------------------------------------------
                //--Seperate the Visibility of controls forms wise 
                //---------------------------------------------------
                #region Form-24Q
                if (strFormNo == T_FormNo.F24Q)
                {
                    ...
                    lblDeductedDateFormat.Visible = false;
                    //----
                    lblDeducteeCode.Visible = false;
                    cmbDeducteeCode.Visible = false;
                    //----
                    lblDeducteeRate.Visible = false;
                    txtDeducteeRate.Visible = false;
                    lblDisplayRate.Visible = false;
                    //----
                    lblGrossingUpIndicator.Visible = false;
                    cmbGrossingUpIndicator.Visible = false;
                    //----
                    lblTDSRate.Visible = false;
                    cmbTDSRate.Visible = false;
                    //----
                    lblRemittance.Visible = false;
                    cmbRemittance.Visible = false;
                    //----
                    lblUniqueAckn.Visible = false;
                    txtUniqueAckn.Visible = false;
                    //----
                    ...
                    //----
                    lblTotValueOfPurchase.Visible = false;
                    txtTotalValueOfPurchase.Visible = false;
                    //-----
                    ...
                    if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >=  T_FinancialYearID.F2013_14ID)
                    {
                        ...
                        lblDeducteeCertificateNo.Visible = true;
                    ...
                    else
                    {
                        ...
                        lblDeducteeCertificateNo.Visible = false;
                ...
                else if (strFormNo == T_FormNo.F26Q)
                {
                    ...
                    lblDeductedDateFormat.Visible = false;
                    //----
                    lblDeducteeCode.Visible = true;
                    cmbDeducteeCode.Visible = true;
                    //----
                    lblDeducteeRate.Visible = true;
                    txtDeducteeRate.Visible = true;
                    lblDisplayRate.Visible = true;
                    //----
                    lblGrossingUpIndicator.Visible = false;
                    cmbGrossingUpIndicator.Visible = false;
                    //----
                ...
                else if (strFormNo == T_FormNo.F27Q)
                {
                    ...
                    if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2013_14ID)
                    {
                        ...
                        lblRemittance.Visible = true;
                        cmbRemittance.Visible = true;
                        lblTDSRate.Visible = true;
                        cmbTDSRate.Visible = true;
                        lblCountry.Visible = true;
                        cmbCountry.Visible = true;
                        lblUniqueAckn.Visible = true;
                        txtUniqueAckn.Visible = true;
                        //
                        chkCashBookEntry.Visible = false;
                        lblGrossingUpIndicator.Visible = false;
                        cmbGrossingUpIndicator.Visible = false;
                    ...
                    else
                    {
                        ...
                        //
                        chkCashBookEntry.Visible = true;
                        lblGrossingUpIndicator.Visible = true;
                        cmbGrossingUpIndicator.Visible = true;
                    }
                }
                #endregion 
                ...
                else if (strFormNo == T_FormNo.F27EQ)
                {
                    ...
                    cmbGrossingUpIndicator.Visible = false;
                    //----
                    lblTotValueOfPurchase.Visible = true;
                    txtTotalValueOfPurchase.Visible = true;
                    //-----
                    if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2013_14ID)
                    {
                        lblSection.Visible = true;
            ...
            //ChallanControlsVisibility(lblSearchMode.Text);            
        }
        #endregion
        ...
        private void dgcViewDeductee_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                //Added by Indrajit on 13-02-2013
                //--------------------------------------------------
                if (dmlService.J_IsRecordExist("TRN_CHALLAN", "CHALLAN_ID", lngChallanID) == false)
                {
                    cmnService.J_UserMessage("Challan has been deleted.");
                    dmlService.J_Rollback();

                    btnChallanDeducteeEntry_Click(sender, e);

                    return;
                }
                if (Check_Record(lngDeducteeID, "TRN_DEDUCTEE_DETAILS", "DEDUCTEE_DETAIL_ID", ref dgcViewDeductee) == 0) return;
                //--------------------------------------------------
        ...
        private void txtDeducteeName_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (blnShowHelp == false)
                    return;
                //-----------------------
                string[,] strShowHelpEmployeeMatrix = {{"MST_EMPLOYEE.CATEGORY = 'G'", "F", "GENERAL", "T"},
                                                       {"MST_EMPLOYEE.CATEGORY = 'W'", "F", "WOMAN", "T"},
                                                       {"MST_EMPLOYEE.CATEGORY = 'S'", "F", "SENIOR CITIZEN", "T"},
                                                       {"MST_EMPLOYEE.CATEGORY = 'O'", "F", "VERY SENIOR CITIZEN", "T"},
                                                       {"MST_EMPLOYEE.CATEGORY = ''", "F", "", "T"}};
                //
                strSQLShowHelpDeductee = "SELECT EMPLOYEE_ID," +
                    "                            EMPLOYEE_NAME," +
                    "                            EMPLOYEE_PAN," +
                    ...
                    "                     AND    EMPLOYEE_NAME       LIKE '" + cmnService.J_ReplaceQuote(txtDeducteeName.Text) + "%' " +
                ...
                else
                {
                    ...
                    {
                        lstDeducteeHelp.Items.Add(new ListBoxItem(drdShowDeducteeHelp["EMPLOYEE_NAME"].ToString().PadRight(45) + drdShowDeducteeHelp["EMPLOYEE_PAN"].ToString().PadRight(13) + drdShowDeducteeHelp["CATEGORY"]));
        ...
        private void txtDeducteePAN_Enter(object sender, EventArgs e)
        {
            try
            {
                ...
                strSQL = "SELECT EMPLOYEE_ID," +
                        "        EMPLOYEE_NAME " +
                        "FROM    MST_EMPLOYEE " +
                        "WHERE   EMPLOYEE_PAN   = '" + cmnService.J_ReplaceQuote(txtDeducteePAN.Text) + "' " +
                        "AND     EMPLOYEE_NAME <> '" + cmnService.J_ReplaceQuote(txtDeducteeName.Text) + "' " +
                        "ORDER BY EMPLOYEE_NAME";

                drdShowDeducteePAN = dmlService.J_ExecSqlReturnReader(strSQL);
                //--
                ...
                else
                {
                    ...
                    lstPANHelp.Items.Add(new ListBoxItem("Employees exist with same PAN - "));
                    while (drdShowDeducteePAN.Read())
                    {
                        lstPANHelp.Items.Add(new ListBoxItem(drdShowDeducteePAN["EMPLOYEE_NAME"].ToString()));
                        //--
        ...
        private void txtDeducteePAN_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    drdShowDeducteePAN = dmlService.J_ExecSqlReturnReader(dmlService.J_pCommand, strSQL);
                    //--
                    if (drdShowDeducteePAN == null)
                    {
                        lstPANHelp.Visible = false;
        ...
        {
            //cmbEmployeeCategory.Text = cmnService.J_Right(lstDeducteeHelp.Text, 2);
            ...
            strEmployeeHelp = lstDeducteeHelp.Text;
            //
            txtDeducteePAN.Text = cmnService.J_Mid(strEmployeeHelp, 45, 10);
        ...
        #region cmbDDSection_Leave
        private void cmbDDSection_Leave(object sender, EventArgs e)
        {
            lblDDSectionDisplay.Visible = false;
        }
        #endregion 
        ...
        private void btnErrorViewClose_Click(object sender, EventArgs e)
        {
            ...
            //Commented by Shrey Kejriwal on 13/02/2012
            //btnSalaryDetail.Visible = true;
        ...
        private void btnGenerateTextFile_Click(object sender, EventArgs e)
        {
            ...
            if (TDSMAN.Classes.TDSMAN.T_pVersionType == T_VERSION_TYPE.TRIAL_VERSION)
            {
                ...
                if (cmnService.J_NullToZero(dmlService.J_ExecSqlReturnScalar(strSQL)) > TDSMAN.Classes.TDSMAN.T_pMaxDeducteeDetailsCount)
                {
                    ...
                    TrialMessageBox.lblMessage2.Text = "Return cannot be generated as the trial version supports only " + TDSMAN.Classes.TDSMAN.T_pMaxDeducteeDetailsCount + " nos. of employee records in total.";
            ...
            if (txtTextFilePath.Text == "")
            {
                ...
                cmnService.J_UserMessage("Creation of Text File failed");
                btnGenerateTextFile.Select();
                return;
            }
            ...
            if (TdsMan.T_InsertFileGenerationData(lngBasicInfoID, txtTextFilePath.Text, Convert.ToString(DateTime.Now),
                ...
                return;
            // 
            cmnService.J_UserMessage("Text File is successfully generated");
        ...
        private void btnOutputFolderChange_Click(object sender, EventArgs e)
        {
            ...
            {
                cmnService.J_UserMessage("Select Folder to Save the Output File");
        ...
        private void btnValidateTextFile_Click(object sender, EventArgs e)
        {
            ...
            {
                cmnService.J_UserMessage("No Input File found. Generate Text File first");
                btnGenerateTextFile.Select();
                return;
            } 
            
            //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            //Checking if Java Runtime Environment is installed
            ...
            //
            //## ANIK 2013/04/26
            if (File.Exists(txtInputFileName.Text) == true)
                File.Copy(txtInputFileName.Text, Path.Combine(txtOutputFolder.Text, Path.GetFileName(txtInputFileName.Text)), true);
            //##
        ...
        private void btnSalaryDetails_Click(object sender, EventArgs e)
        {
            ...
            TDSMAN.Classes.TDSMAN.T_pFinancialYear = cmbFinancialYear.Text;
            TDSMAN.Classes.TDSMAN.T_pQuarter = cmbQuarter.Text;
            TDSMAN.Classes.TDSMAN.T_pCompanyName = cmbCompany.Text;
            TDSMAN.Classes.TDSMAN.T_pCompanyId = Convert.ToInt32(Support.GetItemData(cmbCompany, cmbCompany.SelectedIndex));
            ...
            SalaryDetailsForm.MdiParent = TrnForm24Q.ActiveForm;
        ...
        #region TrnRegularReturn_FormClosing
        private void TrnRegularReturn_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        #region cmbGrossingUpIndicator_KeyPress
        private void cmbGrossingUpIndicator_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (Convert.ToInt64(e.KeyChar) == 13)
                SendKeys.Send("{tab}");
        }
        ...
        private void ClearControls()
        {
            ...
            else if (lblSearchMode.Text == Tabbed_Mode.Challan)
            {
                ...
                txtChallanNo.Text = "";
                txtTransferVoucherNo.Text = "";
                chkChallanBookEntry.Checked = false;
                //txtBSRCode.Text = "";
                //txtChequeNo.Text = "";
                txtTDS.Text = "0.00";
            ...
            else if (lblSearchMode.Text == Tabbed_Mode.Deductee)
            {
                ...
                txtDeducteePAN.Text = "";
                //--
                //cmbEmployeeCategory.Items.Clear();
                //cmbEmployeeCategory.Items.Add("");
                //cmbEmployeeCategory.Items.Add("01");
                //cmbEmployeeCategory.Items.Add("02");
                //cmbEmployeeCategory.SelectedIndex = 0;
                //string[] strEmployeeCategory ={ T_EmployeeCategory.General, T_EmployeeCategory.Woman, T_EmployeeCategory.SeniorCitizen };
                ...
                if (strFormNo == T_FormNo.F27Q)
                {
                    ...
                    if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) <= T_FinancialYearID.F2012_13ID)
                    {
                        cmbGrossingUpIndicator.Items.Clear();
                        cmbGrossingUpIndicator.Items.Add("");
                        cmbGrossingUpIndicator.Items.Add("Yes");
                        cmbGrossingUpIndicator.Items.Add("No");
                        cmbGrossingUpIndicator.SelectedIndex = 0;
                    }
                    else
                    {
                        ...
                        strSQL = " SELECT REMITTANCE_ID," +
                            "             REMITTANCE_CODE + ' ' + REMITTANCE_DESC " +
                            "      FROM   MST_REMITTANCE " +
                        ...
                        strSQL = " SELECT TDS_APPLICABILITY_ID," +
                            "             TDS_APPLICABILITY_CODE + ' ' + TDS_APPLICABILITY_DESC " +
                            "      FROM   MST_TDS_APPLICABILITY " +
                            "      ORDER BY TDS_APPLICABILITY_CODE";
                        if (dmlService.J_PopulateComboBox(strSQL, ref cmbTDSRate) == false) return;
                        //--
                        strSQL = " SELECT COUNTRY_ID," +
                            "             COUNTRY_CODE + ' ' + COUNTRY_DESC " +
                            "      FROM   MST_COUNTRY " +
                            "      ORDER BY COUNTRY_CODE";
                        if (dmlService.J_PopulateComboBox(strSQL, ref cmbCountry) == false) return;
                        //--
                        txtUniqueAckn.Text = "";
                    }
                }
        ...
        private void InsertCompanyBasicInfo(long CompanyId, long BasicInfoId)
        {
            ...
            strSQL = "INSERT INTO TRN_COMPANY_INFO (" +
                     ...
                     "            '" + cmnService.J_ReplaceQuote(strTanNo) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strPanNo) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(txtDedEmpColName.Text.Trim()) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strBranch.Trim()) + "'," +
                     "             " + intDeductorTypeId + "," +
                     "             " + intMinistryId  + "," +
                     "            '" + cmnService.J_ReplaceQuote(strOtherMinistry) + "'," +
                     ...
                     "             " + intStateId  + "," +
                     "            '" + cmnService.J_ReplaceQuote(strPinCode) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strStd) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strPhone) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strEmail) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPName) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPDesignation) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPFatherName) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPAddress1) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPAddress2) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPAddress3) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPAddress4) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPAddress5) + "'," +
                     "             " + intRPStateId  + "," +
                     "            '" + cmnService.J_ReplaceQuote(strRPPin) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPStd) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPPhone) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPEmail) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strRPMobileNo) + "'," +
                     ...
                     "            '" + cmnService.J_ReplaceQuote(strAltRPSTD) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strAltRPPhone) + "'," +
                     "            '" + cmnService.J_ReplaceQuote(strAltRPEmail) + "')";
            //-----------------------------------------------------------
            if (dmlService.J_ExecSql(strSQL) == false)
            {
        ...
        private bool LoadBasicDetails(long Company,long BasicInfoId)
        {
            ...
            try
            {
                ...
                while (drdShowRecord.Read())
                {
                    ...
                    //---
                    intStateId = Convert.ToInt32(drdShowRecord["STATE_ID"]);
                    intRPStateId = Convert.ToInt32(drdShowRecord["P_STATE_ID"]);
                    intDeductorTypeId      = Convert.ToInt32(drdShowRecord["D_CATEGORY_ID"]);
                    intMinistryId          = Convert.ToInt32(drdShowRecord["MINISTRY_ID"]);
                    intDeductorStateId = Convert.ToInt32(drdShowRecord["D_STATE_ID"]);
                    //
                    strTanNo              = Convert.ToString(drdShowRecord["TAN_NO"]);
                    strPanNo              = Convert.ToString(drdShowRecord["PAN_NO"]);
                    strDeductorType       = Convert.ToString(drdShowRecord["CATEGORY_CODE"]) + " - " + Convert.ToString(drdShowRecord["CATEGORY_DESCRIPTION"]);
                    strBranch             = Convert.ToString(drdShowRecord["BRANCH_DIV"]);
                    strCompanyPrefix      = Convert.ToString(drdShowRecord["FILE_PREFIX"]);
                    strAddress1           = Convert.ToString(drdShowRecord["ADDRESS1"]);
                    strAddress2           = Convert.ToString(drdShowRecord["ADDRESS2"]);
                    strAddress3           = Convert.ToString(drdShowRecord["ADDRESS3"]);
                    strAddress4           = Convert.ToString(drdShowRecord["ADDRESS4"]);
                    strAddress5           = Convert.ToString(drdShowRecord["ADDRESS5"]);
                    strStateName          = Convert.ToString(drdShowRecord["STATE_NAME"]);                    
                    strStateCode          = Convert.ToString(drdShowRecord["STATE_CODE"]);
                    strPinCode            = Convert.ToString(drdShowRecord["PIN_CODE"]);
                    strStd                = Convert.ToString(drdShowRecord["STD"]);
                    strPhone              = Convert.ToString(drdShowRecord["PHONE"]);
                    strEmail              = Convert.ToString(drdShowRecord["EMAIL"]);
                    //
                    strRPName             = Convert.ToString(drdShowRecord["PERSON_NAME"]);                    
                    strRPDesignation      = Convert.ToString(drdShowRecord["DESIGNATION"]);
                    strRPFatherName       = Convert.ToString(drdShowRecord["FATHER_NAME"]);
                    strRPAddress1         = Convert.ToString(drdShowRecord["P_ADDRESS1"]);
                    strRPAddress2         = Convert.ToString(drdShowRecord["P_ADDRESS2"]);
                    strRPAddress3         = Convert.ToString(drdShowRecord["P_ADDRESS3"]);
                    strRPAddress4         = Convert.ToString(drdShowRecord["P_ADDRESS4"]);
                    strRPAddress5         = Convert.ToString(drdShowRecord["P_ADDRESS5"]);                    
                    strRPStateName        = Convert.ToString(drdShowRecord["RP_STATE_NAME"]);
                    strRPStateCode        = Convert.ToString(drdShowRecord["RP_STATE_CODE"]);
                    strRPPin              = Convert.ToString(drdShowRecord["P_PIN_CODE"]);
                    strRPStd              = Convert.ToString(drdShowRecord["P_STD"]);
                    strRPPhone            = Convert.ToString(drdShowRecord["P_PHONE"]);
                    strRPMobileNo         = Convert.ToString(drdShowRecord["P_MOBILE"]);
                    strRPEmail            = Convert.ToString(drdShowRecord["P_EMAIL"]);
                    strPAOCode            = Convert.ToString(drdShowRecord["PAO_CODE"]);
                    strPAORegNo           = Convert.ToString(drdShowRecord["PAO_REG_NO"]);
                    strDDOCode            = Convert.ToString(drdShowRecord["DDO_CODE"]);
                    strDDORegNo           = Convert.ToString(drdShowRecord["DDO_REG_NO"]);
                    //
                    strDStateName         = Convert.ToString(drdShowRecord["D_STATE_NAME"]);
                    strDStateCode         = Convert.ToString(drdShowRecord["D_STATE_CODE"]);
                    //
                    strMinistryCode       = Convert.ToString(drdShowRecord["MINISTRY_CODE"]);
                    strMinistryName       = Convert.ToString(drdShowRecord["MINISTRY_NAME"]);
                    strOtherMinistry      = Convert.ToString(drdShowRecord["MINISTRY_OTHER"]);
                    //
                    ...
                    strAltRPSTD           = Convert.ToString(drdShowRecord["P_ALT_STD"]);
                    strAltRPPhone         = Convert.ToString(drdShowRecord["P_ALT_PHONE"]);
                    strAltRPEmail         = Convert.ToString(drdShowRecord["P_ALT_EMAIL"]);
            ...
            try
            {
                ...
                while (drdShowChallanRecord.Read())
                {
                    ...
                    blnSectionDisplay = false;                    
                        ...
                        lblChallanSectionDisplay.Visible = true;
                        lblChallanSectionDisplay.Text = Convert.ToString(drdShowChallanRecord["SECTION_DESCRIPTION"]);                    
                    ...
                    if (Convert.ToString(drdShowChallanRecord["BOOK_ENTRY"]) == "1")
                        chkChallanBookEntry.Checked = true;
        ...
        private bool ShowDeducteeRecord(long Id)
        {
            ...
            try
            {
                ...
                while (drdShowDeducteeRecord.Read())
                {
                    ...
                    txtDeducteeName.Text = Convert.ToString(drdShowDeducteeRecord["EMPLOYEE_NAME"]);
                    txtDeducteeName.SelectionStart = 0;

                    txtOldDeducteeName.Text = Convert.ToString(drdShowDeducteeRecord["EMPLOYEE_NAME"]);
                    txtDeducteePAN.Text = Convert.ToString(drdShowDeducteeRecord["EMPLOYEE_PAN"]);
                    //cmbEmployeeCategory.Text = Convert.ToString(drdShowDeducteeRecord["EMPLOYEE_CATEGORY"]);
                    ...
                    //if (Convert.ToString(drdShowDeducteeRecord["NON_DEDUCTION_FLAG"]) == "B")
                    //    rbnNoDeduction.Checked = true;
                    //if (Convert.ToString(drdShowDeducteeRecord["NON_DEDUCTION_FLAG"]) == "C")
                    //{
                    //    //rbnNormal.Checked = true;
                    //    //grpRemarks.Enabled = false;
                    //    rbnHigherRate.Checked = true;
        ...
        {
            string strUserMessageCaption="";
            string strReasonID = "";
            try
            {   
                if (strFormNo == T_FormNo.F24Q)
                {
                    strUserMessageCaption = "Employee ";
                }
                else
                {
                    strUserMessageCaption = "Deductee ";
                }
                ...
                else if (lblSearchMode.Text == Tabbed_Mode.Challan)
                {
                    ...
                    if (dtService.J_ConvertToIntYYYYMMDD(mskDateOfPayment.Text) < dtService.J_ConvertToIntYYYYMMDD(TdsMan.T_ReturnStartDateFinancialYear(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) - 1)))
                    {
                        ...
                        cmnService.J_UserMessage("Date prior to immediate previous Financial Year is not allowed.");
                        mskDateOfPayment.Select();
                        return false;
                    }
                    //----------------------------------------------------------
                    ...
                    //    {
                    //        if (txtBSRCode.Text.Trim() == "" && txtChallanNo.Text.Trim() == "" && txtTransferVoucherNo.Text.Trim() == "")
                    //        {
                    //            if (cmnService.J_UserMessage("It is automatically Book Entry.. - Proceed?", MessageBoxButtons.YesNo) == DialogResult.No)
                    //            {
                    //                txtBSRCode.Select();
                    //                return false;
                    //            }
                    ...
                    //-----------------------------------------------------------------------
                    if (chkChallanBookEntry.Checked == false)
                    {
                    ...
                    if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2013_14ID)
                    {
                        ...
                        else if (chkChallanBookEntry.Checked == true)
                        {
                            //--
                            if (cmbMinorHead.SelectedIndex > 0)
                            {
                ...
                else if (lblSearchMode.Text == Tabbed_Mode.Deductee)
                {
                    ...
                    if (txtDeducteeName.Text.Trim() == "")
                    {
                        cmnService.J_UserMessage(strUserMessageCaption  + "Name - Cannot be Blank");
                        txtDeducteeName.Select();
                        return false;
                    }
                    // CHECK '^'
                    ...
                    {
                        cmnService.J_UserMessage(strUserMessageCaption + "Name - '^' not allowed");
                        txtDeducteeName.Select();
                        return false;
                    }
                    //-----------------------------------------------------------------------
                    ...
                    {
                        cmnService.J_UserMessage(strUserMessageCaption + "PAN - Cannot be Blank");
                        txtDeducteePAN.Select();
                        return false;
                    }
                    ...
                    if (txtDeducteePAN.Text.Length != 10)
                    {
                        cmnService.J_UserMessage(strUserMessageCaption + "PAN - Should be of 10 characters");
                        txtDeducteePAN.Select();
                        return false;
                    }
                    ...
                    if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2013_14ID)
                    {
                        if (cmbDDSection.SelectedIndex <= 0)
                        {
                            ...
                            { 
                                //-- ANIK @ 2013/09/26 for FVU 4.0
                                if (cmbDDSection.Text == "194LC")
                                {
                                    if (strDeductorType.Substring(0, 1).ToUpper() != "M" && strDeductorType.Substring(0, 1).ToUpper() != "K")
                                    {
                                        cmnService.J_UserMessage("Section 194LC is applicable only for deductor category 'Company' and 'Branch / Division of Company'.");
                                        cmbDDSection.Select();
                                        return false;
                                    }
                                }
                    ...
                    //Commented by Shrey Kejriwal on 05/10/2012
                    ...
                    //-- QUARTER
                    //----------------------------------------------------------
                    //Form Wise Checking the Quarter[ 24Q,27Q,27EQ ]
                    if ((strFormNo == T_FormNo.F24Q)||(strFormNo == T_FormNo.F27Q)||(strFormNo == T_FormNo.F27EQ))
                    {
                    ...
                    //-- TAX DEPOSITED
                    //-----------------------------------------------------------------------
                    //Modified by Shrey Kejriwal on 24/01/2012
                    ...
                    if ((strFormNo == T_FormNo.F27EQ)||(strFormNo == T_FormNo.F27EQ))
                    {
                        if (Convert.ToInt32(Support.GetItemData(cmbRemarks, cmbRemarks.SelectedIndex)) == Convert.ToInt32(strReasonID))
                        {
                    ...
                    //-----------------------------------------------------------------------
                    ////RATE FOR 24Q
                    //// RATE CAN NOT BE ZERO IF INCOME TAX  IS PRESENT [NOT APPLICABLE FOR 24Q]
                    ...
                    //
                    //
                    //RATE FOR 26Q &  27Q & 27EQ
                    // RATE CAN NOT BE ZERO IF INCOME TAX  IS PRESENT
                    if ((strFormNo == T_FormNo.F26Q)||(strFormNo == T_FormNo.F27Q)||(strFormNo == T_FormNo.F27EQ))
                    {
                        if (Convert.ToDouble(cmnService.J_NumericData(txtDeducteeIncometax)) != 0)
                        {
                            if (Convert.ToDouble(cmnService.J_NumericData(txtDeducteeRate)) == 0)
                    ...
                    //{
                        ...
                        if ((strFormNo == T_FormNo.F24Q) || (strFormNo == T_FormNo.F26Q))
                        {
                            if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) >= T_FinancialYearID.F2010_11ID)
                            {
                                if (txtDeducteePAN.Text == "PANNOTAVBL" && Convert.ToInt32(Support.GetItemData(cmbRemarks, cmbRemarks.SelectedIndex)) != Convert.ToInt32(strReasonID))//&& rbnHigherRate.Checked == false)
                                {
                        ...
                        if (Convert.ToInt32(Support.GetItemData(cmbRemarks, cmbRemarks.SelectedIndex)) == Convert.ToInt32(strReasonID) && txtDeducteePAN.Text != "PANNOTAVBL")
                        {
                        ...
                        //}
                        //else 
                        //if (rbnNoDeduction.Checked == true)
                        ...
                        if (strFormNo == T_FormNo.F24Q)
                        {
                            ...
                            {
                                // SECTION NO.
                                //if (txtViewChallanDetailsSection.Text != "194" && 
                                //    txtViewChallanDetailsSection.Text != "194A" && 
                                //    txtViewChallanDetailsSection.Text != "194EE" && 
                                //    txtViewChallanDetailsSection.Text != "193")
                                //{
                                ...
                                //    return false;
                                //}
                                // IF 'NO DEDUCTION' – INCOME TAX, SURCHARGE, CESS, TOTAL, TAX DEPOSITED ALL VALUE SHOULD BE 0.
                                //
                        ...
                        if (strFormNo == T_FormNo.F27Q)
                        {
                            if (Convert.ToInt32(Support.GetItemData(cmbRemarks, cmbRemarks.SelectedIndex)) == 14)
                            {
                                //Added by Shrey Kejriwal on 24/01/2012
                                //Checking if Higher Rate is selected for Invalid Pan entered
                                if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) >= T_FinancialYearID.F2010_11ID)
                                {
                                    if (txtDeducteePAN.Text == "PANNOTAVBL" && Convert.ToInt32(Support.GetItemData(cmbRemarks, cmbRemarks.SelectedIndex)) != 15)
                                    {
                                        cmnService.J_UserMessage("For Deductees with Invalid PAN it is mandatory to deduct TDS under Higher Rate.");
                                        cmbRemarks.Select();
                                        return false;
                                ...
                                //Added by Shrey Kejriwal on 24/01/2012
                                //Giving a warning message for selecting Higher rate for the valid PAN entered
                                if (Convert.ToInt32(Support.GetItemData(cmbRemarks, cmbRemarks.SelectedIndex)) == 15 && txtDeducteePAN.Text != "PANNOTAVBL")
                                {
                                    if (cmnService.J_UserMessage("Are you sure you want to select Higher Rate for the Valid PAN entered", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2) == DialogResult.No)
                                    {
                                        cmbRemarks.Select();
                    ...
                    {
                        cmnService.J_UserMessage("Total Employee deposit can never be more than the Challan deposit");
                        txtDeducteeTaxDeposited.Select();
                        return false;
                    }
                    ...
                    {
                        // CHECK '^'
                        if (TdsMan.T_DetectCaret(txtUniqueAckn.Text.Trim(), "^") == true)
                        {
                            cmnService.J_UserMessage("Unique Ack (Form 15CA)- '^' not allowed");
                            txtUniqueAckn.Select();
                            return false;
                        }                    
                    }
                    ////-----------------------------------------------------------------------
                ...
                else if (lblSearchMode.Text == Tabbed_Mode.GenerateTDS)
                {
                    ...
                    if (chkCSIFileDownload.Checked == true)
                    {
                        ...
                        if (TdsMan.T_DateDiff(T_DATE_DIFF.MONTH, dtService.J_ConvertddMMyyyy(mskViewCSIFileDownloadFrom), dtService.J_ConvertddMMyyyy(mskViewCSIFileDownloadTo)) > 24)
                        {
                            cmnService.J_UserMessage("CSI file cannot be downloaded as the Challans period exceed 24 months");
                            mskViewCSIFileDownloadFrom.Select();
                            return false;
                        }
                    }
        ...
        private void Insert_Update_Delete_Data(object sender, EventArgs e)
        {
            try
            {
                IDataReader drdGetDeducteeAmounts = null;
                //--------------------------------------------
                ...
                switch (lblMode.Text)
                {
                    ...
                    case J_Mode.Add:
                        ...
                        if (lblSearchMode.Text == Tabbed_Mode.Challan)
                        {
                            ...
                            //-----------------------------------------------------------
                            //Commented by Indrajit on 13-02-2013 to bypass company common information
                            //
                            //if (lngBasicInfoID == 0)
                            //{
                            //    if (TdsMan.T_GenerateBasicInfoId(dmlService.J_pCommand, Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)),
                            ...
                            //Added by Indrajit on 23-08-2012 and blocked
                            ...
                            strSQL = "UPDATE TRN_CHALLAN " +
                                     "SET SL_NO = DMAX('SL_NO','TRN_CHALLAN','BASIC_INFO_ID =" + lngBasicInfoID + "')+1 " +
                                     "WHERE CHALLAN_ID = DMAX('CHALLAN_ID','TRN_CHALLAN','')";
                            ...
                            if (TdsMan.T_UpdateSrlNo(dmlService.J_pCommand, lngBasicInfoID, Updt_SrlNo.ChlnSrlNo) == false)
                            {
                                dmlService.J_Rollback();
                                return;
                            }
                        ...
                        else if (lblSearchMode.Text == Tabbed_Mode.Deductee)
                        {
                            dmlService.J_BeginTransaction();
                            //-----------------------------------------------------------
                            //Added by Indrajit on 13-02-2013 to check the existance of Challan
                            //-----------------------------------------------------------
                            if (dmlService.J_IsRecordExist("TRN_CHALLAN", "CHALLAN_ID", lngChallanID) == false)
                            {
                                cmnService.J_UserMessage("Challan has been deleted.");
                            ...
                            //    "     AND    EMPLOYEE_NAME ='" + cmnService.J_ReplaceQuote(txtDeducteeName.Text) + "'" +
                            //    "     AND    EMPLOYEE_PAN  ='" + cmnService.J_ReplaceQuote(txtDeducteePAN.Text) + "'";

                            //lngDeducteeID = cmnService.J_NullToZero(dmlService.J_ExecSqlReturnScalar(strSQL));

                            //MessageBox.Show(lngDeducteeID.ToString() + DateTime.Now.ToString());
                            ...
                            if (lngDeducteeID == 0)
                            {
                                ...
                                //    "     VALUES      ('" + cmnService.J_ReplaceQuote(txtDeducteeName.Text.Trim()) + "'," +
                                //    "                  '" + cmnService.J_ReplaceQuote(txtDeducteePAN.Text.Trim()) + "'," +
                                //    "                  '" + cmnService.J_ReplaceQuote(cmbDeducteeCode.Text.Trim()) + "')";
                                if (dmlService.J_ExecSql(dmlService.J_pCommand, strSQL) == false)
                                {
                                    dmlService.J_Rollback();
                                    return;
                                ...
                                //--
                                lngDeducteeID = dmlService.J_ReturnMaxValue(dmlService.J_pCommand, "MST_EMPLOYEE", "EMPLOYEE_ID");
                            }
                            //
                            ...
                            //Added by Indrajit on 13-02-2013 and blocked
                            //To update the last deductee serial no
                            //============================================================
                            long lngDeducteeSrl;
                            lngDeducteeSrl = 0;
                            ...
                            strSQL = "INSERT INTO TRN_DEDUCTEE_DETAILS " +
                                     ...
                                     "     VALUES(" + lngChallanID + "," +
                                     "            " + lngBasicInfoID + "," +
                                     //"            " + cmnService.J_ReturnDoubleValue(txtDeducteeSrlNo.Text) + "," +   //Modified by Indrajit on 13-02-2013
                                     "            " + lngDeducteeSrl + "," +
                                     "            " + lngDeducteeID + "," +
                            ...
                            strSQL = "UPDATE TRN_DEDUCTEE_DETAILS " +
                                     "SET SL_NO = DMAX('SL_NO','TRN_DEDUCTEE_DETAILS','CHALLAN_ID =" + lngChallanID + "')+1 " +
                                     "WHERE DEDUCTEE_DETAIL_ID = DMAX('DEDUCTEE_DETAIL_ID','TRN_DEDUCTEE_DETAILS','')";
                            ...
                            // RECALCALUTING SRL NO.
                            if (TdsMan.T_UpdateSrlNo(dmlService.J_pCommand, lngChallanID, Updt_SrlNo.DeducteeSrlNo) == false)
                            {
                                dmlService.J_Rollback();
                                return;
                            }
                            ...
                            //Blocked by Indrajit on 19-02-2013
                            //// UPDATE TDS
                            ...
                            double dblTOT_TAX_AMOUNT = 0;
                            double dblTOT_SURCHARGE_AMOUNT = 0;
                            double dblTOT_CESS_AMOUNT = 0;
                            double dblTOT_TOTAL_AMOUNT = 0;
                            double dblTOT_TAX_DEPOSITED_AMOUNT = 0;


                            strSQL =@"SELECT  IIF(ISNULL(SUM(TAX_AMOUNT)),'0',SUM(TAX_AMOUNT))                     AS TOT_TAX_AMOUNT,
                                              IIF(ISNULL(SUM(SURCHARGE_AMOUNT)),'0',SUM(SURCHARGE_AMOUNT))         AS TOT_SURCHARGE_AMOUNT, 
                                              IIF(ISNULL(SUM(CESS_AMOUNT)),'0',SUM(CESS_AMOUNT))                   AS TOT_CESS_AMOUNT,
                                              IIF(ISNULL(SUM(TOTAL_AMOUNT)),'0',SUM(TOTAL_AMOUNT))                 AS TOT_TOTAL_AMOUNT,
                                              IIF(ISNULL(SUM(TAX_DEPOSITED_AMOUNT)),'0',SUM(TAX_DEPOSITED_AMOUNT)) AS TOT_TAX_DEPOSITED_AMOUNT 
                                      FROM TRN_DEDUCTEE_DETAILS 
                                      HAVING CHALLAN_ID = " + lngChallanID;

                            drdGetDeducteeAmounts = dmlService.J_ExecSqlReturnReader(strSQL);
                            if (drdGetDeducteeAmounts == null) 
                            {
                                dmlService.J_Rollback();
                                return;
                            }
                            while (drdGetDeducteeAmounts.Read())
                            {
                                dblTOT_TAX_AMOUNT           = Convert.ToDouble(drdGetDeducteeAmounts["TOT_TAX_AMOUNT"]);
                                dblTOT_SURCHARGE_AMOUNT     = Convert.ToDouble(drdGetDeducteeAmounts["TOT_SURCHARGE_AMOUNT"]);
                                dblTOT_CESS_AMOUNT          = Convert.ToDouble(drdGetDeducteeAmounts["TOT_CESS_AMOUNT"]);
                                dblTOT_TOTAL_AMOUNT         = Convert.ToDouble(drdGetDeducteeAmounts["TOT_TOTAL_AMOUNT"]);
                                dblTOT_TAX_DEPOSITED_AMOUNT = Convert.ToDouble(drdGetDeducteeAmounts["TOT_TAX_DEPOSITED_AMOUNT"]);
                            }
                            drdGetDeducteeAmounts.Close();
                            drdGetDeducteeAmounts.Dispose();

                            strSQL = @"UPDATE TRN_CHALLAN
                                       SET 
                                       TRN_CHALLAN.CTRL_TDS       = " + dblTOT_TAX_AMOUNT + @",
                                       TRN_CHALLAN.CTRL_SURCHARGE = " + dblTOT_SURCHARGE_AMOUNT + @",
                                       TRN_CHALLAN.CTRL_EDU_CESS  = " + dblTOT_CESS_AMOUNT + @",
                                       TRN_CHALLAN.CTRL_TOT       = " + dblTOT_TOTAL_AMOUNT + @",
                                       TRN_CHALLAN.CTRL_TOT_TAX   = " + dblTOT_TAX_DEPOSITED_AMOUNT + @"
                                       WHERE  TRN_CHALLAN.CHALLAN_ID   = " + lngChallanID;
                            ...
                            strSQL = "SELECT (TRN_CHALLAN.TOT_TAX - " +
                                     "       TRN_CHALLAN.INTEREST_ALLOCATED - " +
                                     "       TRN_CHALLAN.OTHERS_ALLOCATED) - TRN_CHALLAN.CTRL_TOT_TAX AS DIFF " +
                                     "FROM   TRN_CHALLAN " +
                                     "WHERE  CHALLAN_ID = " + lngChallanID;

                            if (Convert.ToInt64(dmlService.J_ExecSqlReturnScalar(dmlService.J_pCommand, strSQL)) < 0)
                            {
                                //cmnService.J_UserMessage("Besi Hoye Gechhe...");    
                                cmnService.J_UserMessage("Total Employee deposit can never be more than the Challan deposit..");
                                dmlService.J_Rollback();
                                return;
                            }
                            ...
                            //Modified by Indrajit on 13-02-2013
                            //if (lngDeducteeDetailID == 0) return;
                    ...
                    case J_Mode.Edit:
                        ...
                        if (lblSearchMode.Text == Tabbed_Mode.Challan)
                        {
                            ...
                            //Added by Indrajit on 13-02-2013 to check the existance of Challan
                            //-----------------------------------------------------------
                            if (Check_Record(lngChallanID, "TRN_CHALLAN", "CHALLAN_ID", ref dgcViewChallan) == 0)
                            {
                                dmlService.J_Rollback();
                                return;
                            }
                            ...
                            {
                                cmnService.J_UserMessage("Challan total can not be less than Employee total");
                                txtTotalTax.Select();
                                dmlService.J_Rollback();
                                return;
                            }
                        ...
                        else if (lblSearchMode.Text == Tabbed_Mode.Deductee)
                        {
                            ...
                            //Added by Indrajit on 13-02-2013 to check the existance of Challan
                            //-----------------------------------------------------------
                            if (dmlService.J_IsRecordExist(dmlService.J_pCommand, "TRN_CHALLAN", "CHALLAN_ID", lngChallanID) == false)
                            {
                                cmnService.J_UserMessage("Challan has been deleted.");
                                dmlService.J_Rollback();
                                //=============================================================
                            ...
                            //--------------------------------------------------
                            //if (Check_Record(lngDeducteeID) == 0)
                            if (Check_Record(lngDeducteeID, "TRN_DEDUCTEE_DETAILS", "DEDUCTEE_DETAIL_ID", ref dgcViewDeductee) == 0)
                            {
                                dmlService.J_Rollback();
                                return;
                            }
                            ...
                            if (lngDeducteeID == 0)
                            {
                                if (cmnService.J_UserMessage("New Employee will be created - Proceed?", MessageBoxButtons.YesNo) == DialogResult.No)
                                {
                                    txtDeducteeName.Select();
                                    dmlService.J_Rollback();
                                    return;
                                ...
                                //    "     VALUES      ('" + cmnService.J_ReplaceQuote(txtDeducteeName.Text.Trim()) + "'," +
                                //    "                  '" + cmnService.J_ReplaceQuote(txtDeducteePAN.Text) + "'," +
                                //    "                  '" + cmnService.J_ReplaceQuote(cmbEmployeeCategory.Text) + "')";
                                if (dmlService.J_ExecSql(dmlService.J_pCommand, strSQL) == false)
                                {
                                    dmlService.J_Rollback();
                                    return;
                    ...
                    case J_Mode.Delete:
                        ...
                        if (lblSearchMode.Text == Tabbed_Mode.Challan)
                        {
                            ...
                            //Added by Indrajit on 13-02-2013 to check the existance of Deductee
                            //-----------------------------------------------------------
                            if (Check_Record(lngChallanID, "TRN_CHALLAN", "CHALLAN_ID", ref dgcViewChallan) == 0)
                            {
                                dmlService.J_Rollback();
                            ...
                            if (lngChallanID <= 0)
                            {
                                cmnService.J_UserMessage("No Challan Selected ");
                                lblMode.Text = J_Mode.Add;
                                cmbSection.Select();
                                dmlService.J_Rollback();
                                return;
                            ...
                            long lngNoOfDeductees = cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar(dmlService.J_pCommand, strSQL)));

                            // WHEN EMPLOYEE ENTRY EXISTS
                            if (lngNoOfDeductees > 0)
                            {
                                strDeleteMessage = "The Challan selected contains [" + lngNoOfDeductees + "]  nos. Employee Detail records." +
                                    "Please note that if this Challan is deleted all the corresponding Employee Detail records will also be deleted." +
                                    "Proceed Deletion?";
                            }
                            else
                            {
                            ...
                            //Blocked by Indrajit on 13-02-2013
                            // RECALCALUTING SRL NO.
                            if (TdsMan.T_UpdateSrlNo(dmlService.J_pCommand, cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT SL_NO FROM TRN_CHALLAN WHERE  CHALLAN_ID = " + lngChallanID))),
                                lngBasicInfoID, Updt_SrlNo.ChlnSrlNo) == false) return;
                        ...
                        else if (lblSearchMode.Text == Tabbed_Mode.Deductee)
                        {
                            ...
                            if (TdsMan.T_UpdateSrlNo(dmlService.J_pCommand, cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar(dmlService.J_pCommand, "SELECT SL_NO FROM TRN_DEDUCTEE_DETAILS WHERE DEDUCTEE_DETAIL_ID = " + lngDeducteeID))),
                                lngChallanID, Updt_SrlNo.DeducteeSrlNo) == false) return;
        ...
        private void LoadDeducteeGrid(long ChallanDetailID)
        {
            ...
            dsetGridClone = dmlService.J_ShowDataInGrid(dmlService.J_pCommand, ref dgcViewDeductee, strSQL, strMatrixViewDeductee);       //Show Data into the Grid                
        }
        #endregion       
        ...
        private bool GenerateTextFile(string strInputFilePath)
        {
            ...
            //    cmnService.J_WriteLine(ref StreamWriter, TdsMan.T_WriteField(intPageNumber.ToString()) + TdsMan.T_WriteField("FH") + TdsMan.T_WriteField("SL1") +
            ...
            //    // BATCH HEADER
            //    string strCountSalaryDetailRecords = Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COUNT(*) AS COUNT_SALARY_DETAILS FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID));
            //    string strGrossTotalIncome = Convert.ToString(cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT SUM(GROSS_TOTAL_INCOME) AS SUM_GROSS_TOTAL_INCOME FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID))));
            ...
            //                            TdsMan.T_WriteField(strRPMobileNo) + TdsMan.T_WriteField(txtRPSTD.Text) +
            //                            TdsMan.T_WriteField(txtRPPhone.Text) + TdsMan.T_WriteField(strPChangeAddress) +
            //                            TdsMan.T_WriteField(txtTotalChallanAmount.Text) +
            //                            TdsMan.T_WriteBlankField(1, "^") +
            //                            TdsMan.T_WriteField(strCountSalaryDetailRecords) +
            //                            TdsMan.T_WriteField(strGrossTotalIncome, "0.00") +
            ...
            //                            TdsMan.T_WriteField(txtPAOCode.Text) + TdsMan.T_WriteField(txtDDOCode.Text) +
            //                            TdsMan.T_WriteField(strMinistryCode) + TdsMan.T_WriteField(txtOtherMinistry.Text) +
            //                           TdsMan.T_WriteField(TANRegNo) + TdsMan.T_WriteField(txtPAORegNo.Text) +
            //                            TdsMan.T_WriteField(txtDDORegNo.Text) +
            ...
            //        strArray[lngArrayCounter, (int)enmCHALLANIndex.SECTION_NAME] = drdGenChallan["SECTION_NAME"].ToString();
            ...
            //            #region DEDUCTEE DETAIL


            //            // DEDUCTEE DETAIL
            //            string[,] strGenerateTextFile = {{"MST_EMPLOYEE.EMPLOYEE_PAN = 'PANNOTAVBL'", "F", "C", "T"},
            //                                             {"MST_EMPLOYEE.EMPLOYEE_PAN <> 'PANNOTAVBL'", "F", "TRN_DEDUCTEE_DETAILS.NON_DEDUCTION_FLAG", "F"}};
            ...
            //                                TdsMan.T_WriteField(strArray[lngCounter, (int)enmCHALLANIndex.SL_NO]) +
            //                                TdsMan.T_WriteBlankField(2, "^") +
            //                                TdsMan.T_WriteField(Convert.ToString(drdGenDeductee["EMPLOYEE_PAN"])) +
            //                                TdsMan.T_WriteBlankField(2, "^") +
            //                                TdsMan.T_WriteField(TdsMan.RemoveSpecialCharacters(Convert.ToString(drdGenDeductee["EMPLOYEE_NAME"]))) +
            ...
            //        // CHALLAN HEADER
            //        strSQL = "SELECT COUNT(*) AS SD_ROW_COUNT " +
            //            "     FROM   TRN_SALARY_DETAILS " +
            //            "     WHERE  BASIC_INFO_ID = " + lngBasicInfoID + " ";

            //        lngSDRowCount = Convert.ToInt32(dmlService.J_ExecSqlReturnScalar(strSQL));
            //        strArray = new string[lngSDRowCount, 22];

            //        strSQL = "SELECT TRN_SALARY_DETAILS.SALARY_DETAILS_ID     AS SALARY_DETAILS_ID," +
            //            "            TRN_SALARY_DETAILS.SL_NO                 AS SL_NO," +
            //            "            MST_EMPLOYEE.EMPLOYEE_NAME               AS EMPLOYEE_NAME," +
            //            "            MST_EMPLOYEE.EMPLOYEE_PAN                AS EMPLOYEE_PAN," +
            //            "            MST_EMPLOYEE.CATEGORY                    AS EMPLOYEE_CATEGORY," +
            ...
            //        {
            //            drdGenDeductee.Close();
            //            drdGenDeductee.Dispose();
            //            return false;
            //        }
            //        lngArrayCounter = 0;
            //        while (drdGenDeductee.Read())
            //        {
            //            strFromDate = TdsMan.J_ConvertToIntDDMMYYYY(Convert.ToString(drdGenDeductee["FROM_DATE"]));
            //            strToDate = TdsMan.J_ConvertToIntDDMMYYYY(Convert.ToString(drdGenDeductee["TO_DATE"]));
            //            //---    
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.SD_ID] = drdGenDeductee["SALARY_DETAILS_ID"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.SL_NO] = drdGenDeductee["SL_NO"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.EMPLOYEE_NAME] = drdGenDeductee["EMPLOYEE_NAME"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.EMPLOYEE_PAN] = drdGenDeductee["EMPLOYEE_PAN"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.EMPLOYEE_CATEGORY] = drdGenDeductee["EMPLOYEE_CATEGORY"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.FROM_DATE] = strFromDate; //drdGenDeductee["FROM_DATE"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.TO_DATE] = strToDate; //drdGenDeductee["TO_DATE"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.TS_BALANCE] = drdGenDeductee["TS_BALANCE"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.US_16_AGGREGATE] = drdGenDeductee["US_16_AGGREGATE"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.INCOME_CHARGEABLE] = drdGenDeductee["INCOME_CHARGEABLE"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.AIS_TOTAL] = drdGenDeductee["AIS_TOTAL"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.GROSS_TOTAL_INCOME] = drdGenDeductee["GROSS_TOTAL_INCOME"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.CVIA_DED_TOTAL] = drdGenDeductee["CVIA_DED_TOTAL"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.TOTAL_INCOME] = drdGenDeductee["TOTAL_INCOME"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.TAX_TOTAL_INCOME] = drdGenDeductee["TAX_TOTAL_INCOME"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.SCHG_TOTAL_INCOME] = drdGenDeductee["SCHG_TOTAL_INCOME"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.ECESS_TOTAL_INCOME] = drdGenDeductee["ECESS_TOTAL_INCOME"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.TAX_PAYABLE_AGGREGATE] = drdGenDeductee["TAX_PAYABLE_AGGREGATE"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.US_89_LESS] = drdGenDeductee["US_89_LESS"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.TAX_PAYABLE] = drdGenDeductee["TAX_PAYABLE"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.TOTAL_TDS_DEDUCTED] = drdGenDeductee["TOTAL_TDS_DEDUCTED"].ToString();
            //            strArray[lngArrayCounter, (int)enmSALARY_DETAILIndex.SHORTFALL_TAX] = drdGenDeductee["SHORTFALL_TAX"].ToString();
            ...
            //                // Count of 'Salary Details  - Section 16 Detail'
            //                int intCountS16 = 0;
            //                strSQL = "SELECT COUNT(*) FROM TRN_SALARY_DETAILS WHERE US_16_EA > 0 AND BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID];
            //                if (cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL))) > 0)
            //                    intCountS16 = intCountS16 + 1;
            //                //
            //                if (cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COUNT(*) FROM TRN_SALARY_DETAILS WHERE US_16_TE > 0 AND BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID]))) > 0)
            //                    intCountS16 = intCountS16 + 1;
            //                //Count of 'Salary Details  - Chapter VI-A Detail'
            //                int intCountCVIA = 0;
            //                if (cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COUNT(*) FROM TRN_SALARY_DETAILS WHERE CVIA_SEC80CCE_TOTAL_DED_AMOUNT > 0 AND BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID]))) > 0)
            //                    intCountCVIA = intCountCVIA + 1;
            //                //
            //                if (cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COUNT(*) FROM TRN_SALARY_DETAILS WHERE CVIA_SEC80CCF_DED_AMOUNT > 0 AND BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID]))) > 0)
            //                    intCountCVIA = intCountCVIA + 1;
            //                //
            //                //## Anik 2013/04/25
            //                if (cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COUNT(*) FROM TRN_SALARY_DETAILS WHERE CVIA_SEC80CCG_DED_AMOUNT > 0 AND BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID]))) > 0)
            //                    intCountCVIA = intCountCVIA + 1;
            //                //##
            //                if (cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COUNT(*) FROM TRN_SALARY_DETAILS WHERE CVIA_OTH_DED_TOTAL > 0 AND BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID]))) > 0)
            //                    intCountCVIA = intCountCVIA + 1;
            //                //--------------------------------------------
            //                cmnService.J_WriteLine(ref StreamWriter, TdsMan.T_WriteField(intPageNumber.ToString()) +
            //                    TdsMan.T_WriteField("SD") +
            //                    TdsMan.T_WriteField("1") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SL_NO]) +
            //                    TdsMan.T_WriteField("A") +
            //                    TdsMan.T_WriteBlankField(1, "^") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.EMPLOYEE_PAN]) +
            //                    TdsMan.T_WriteBlankField(1, "^") +
            //                    TdsMan.T_WriteField(TdsMan.RemoveSpecialCharacters(strArray[lngCounter, (int)enmSALARY_DETAILIndex.EMPLOYEE_NAME])) +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.EMPLOYEE_CATEGORY]) +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.FROM_DATE]) +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.TO_DATE]) +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.TS_BALANCE], "0.00") +
            //                    TdsMan.T_WriteBlankField(1, "^") +
            //                    TdsMan.T_WriteField(Convert.ToString(intCountS16)) +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.US_16_AGGREGATE], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.INCOME_CHARGEABLE], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.AIS_TOTAL], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.GROSS_TOTAL_INCOME], "0.00") +
            //                    TdsMan.T_WriteBlankField(1, "^") +
            //                    TdsMan.T_WriteField(Convert.ToString(intCountCVIA)) +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.CVIA_DED_TOTAL], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.TOTAL_INCOME], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.TAX_TOTAL_INCOME], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SCHG_TOTAL_INCOME], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.ECESS_TOTAL_INCOME], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.US_89_LESS], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.TAX_PAYABLE], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.TOTAL_TDS_DEDUCTED], "0.00") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SHORTFALL_TAX], "0.00") +
            //                    TdsMan.T_WriteBlankField(3, "^") +
            //                    TdsMan.T_WriteBlankField(5, "^"));

            //                // SD-S16
            //                int intS16SerialNo = 0;
            //                double dblUS_16_EA = cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT US_16_EA FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID])));
            //                if (dblUS_16_EA > 0)
            //                {
            //                    intPageNumber = intPageNumber + 1;
            //                    intS16SerialNo = intS16SerialNo + 1;

            //                    cmnService.J_WriteLine(ref StreamWriter, TdsMan.T_WriteField(intPageNumber.ToString()) +
            //                    TdsMan.T_WriteField("S16") +
            //                    TdsMan.T_WriteField("1") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SL_NO]) +
            //                    TdsMan.T_WriteField(Convert.ToString(intS16SerialNo)) +
            //                    TdsMan.T_WriteField("16(ii)") +
            //                    TdsMan.T_WriteField(Convert.ToString(dblUS_16_EA), "0.00"));
            //                }
            //                //
            //                double dblUS_16_TE = cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT US_16_TE FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID])));
            //                if (dblUS_16_TE > 0)
            //                {
            //                    intPageNumber = intPageNumber + 1;
            //                    intS16SerialNo = intS16SerialNo + 1;
            ...
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SL_NO]) +
            //                    TdsMan.T_WriteField(Convert.ToString(intS16SerialNo)) +
            //                    TdsMan.T_WriteField("16(iii)") +
            //                    TdsMan.T_WriteField(Convert.ToString(dblUS_16_TE), "0.00"));
            //                }
            //                //80CCE
            //                int intC6ASerialNo = 0;
            //                double dbl80CCE = cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT CVIA_SEC80CCE_TOTAL_DED_AMOUNT FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID])));
            //                if (dbl80CCE > 0)
            //                {
            //                    intPageNumber = intPageNumber + 1;
            //                    intC6ASerialNo = intC6ASerialNo + 1;

            //                    cmnService.J_WriteLine(ref StreamWriter, TdsMan.T_WriteField(intPageNumber.ToString()) +
            //                    TdsMan.T_WriteField("C6A") +
            //                    TdsMan.T_WriteField("1") +
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SL_NO]) +
            //                    TdsMan.T_WriteField(Convert.ToString(intC6ASerialNo)) +
            //                    TdsMan.T_WriteField("80CCE") +
            //                    TdsMan.T_WriteField(Convert.ToString(dbl80CCE), "0.00"));
            //                }
            //                // 80CCF
            //                double dbl80CCF = cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT CVIA_SEC80CCF_DED_AMOUNT FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID])));
            //                if (dbl80CCF > 0)
            //                {
            //                    intPageNumber = intPageNumber + 1;
            //                    intC6ASerialNo = intC6ASerialNo + 1;
            ...
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SL_NO]) +
            //                    TdsMan.T_WriteField(Convert.ToString(intC6ASerialNo)) +
            //                    TdsMan.T_WriteField("80CCF") +
            //                    TdsMan.T_WriteField(Convert.ToString(dbl80CCF), "0.00"));
            //                }
            //                //## Anik 2013/04/25
            //                // 80CCG
            //                double dbl80CCG = cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT CVIA_SEC80CCG_DED_AMOUNT FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID])));
            //                if (dbl80CCG> 0)
            //                {
            //                    intPageNumber = intPageNumber + 1;
            //                    intC6ASerialNo = intC6ASerialNo + 1;
            ...
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SL_NO]) +
            //                    TdsMan.T_WriteField(Convert.ToString(intC6ASerialNo)) +
            //                    TdsMan.T_WriteField("80CCG") +
            //                    TdsMan.T_WriteField(Convert.ToString(dbl80CCG), "0.00"));
            //                }
            //                //##
            //                // OTHERS
            //                double dblOTHERS = cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT CVIA_OTH_DED_TOTAL FROM TRN_SALARY_DETAILS WHERE BASIC_INFO_ID = " + lngBasicInfoID + " AND SALARY_DETAILS_ID = " + strArray[lngCounter, (int)enmSALARY_DETAILIndex.SD_ID])));
            //                if (dblOTHERS > 0)
            //                {
            //                    intPageNumber = intPageNumber + 1;
            //                    intC6ASerialNo = intC6ASerialNo + 1;
            ...
            //                    TdsMan.T_WriteField(strArray[lngCounter, (int)enmSALARY_DETAILIndex.SL_NO]) +
            //                    TdsMan.T_WriteField(Convert.ToString(intC6ASerialNo)) +
            //                    TdsMan.T_WriteField("OTHERS") +
            //                    TdsMan.T_WriteField(Convert.ToString(dblOTHERS), "0.00"));
            //                }
        ...
        private void ValidateTextFile(string FVU_Version)
        {
            ...
            try
            {
                ...
                // CHECKING IF THE CSI FILE PATH EXISTS
        ...
        {   
            if (chkNILReturn.Checked == true)
            {
                ...
                //Blocked by Indrajit on 27-02-2012 to impose Locking
                //if (lngBasicInfoID == 0)
                //{
                //    if (TdsMan.T_GenerateBasicInfoId(dmlService.J_pCommand, Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)),
                //                                        cmbQuarter.Text,
        ...
        private void EnabilityReturnFilingStatus(long BasicInfoId)
        {
            ...
            try
            {
                ...
                // CHECK IF CHALLAN EXISTS
                strSQL = "SELECT RECEIPT_NO AS RECEIPT_NO," +
        ...
        private void EnabilityGrpButtons(long BasicInfoId)
        {
            ...
            try
            {
                ...
                // RECEIPT NO.
                if (lblReturnFilingReceiptNo.Visible == true)
                {
                    BtnCancel.Enabled = false;
                    BtnCancel.BackColor = Color.LightGray;
                }
        ...
        private void ControlSummaryVisible(bool bVisible0, bool bVisible1, bool bVisible2, bool bVisible3, bool bVisible4, bool bVisible5, bool bVisible6)
        {
            ...
            lblTotalChallanRecords.Visible   = bVisible1;
            txtTotalChallanRecords.Visible = bVisible1;
            lblTotalDeducteeRecords.Visible   = bVisible2;
            txtTotalDeducteeRecords.Visible = bVisible2;
            lblTotalChallanAmount.Visible   = bVisible3;
            txtTotalChallanAmount.Visible = bVisible3;
            lblTotalTDSDeducted.Visible   = bVisible4;
            txtTotalDeducteeTDS.Visible = bVisible4;
            lblAmountPaid.Visible = bVisible5;
        ...
        {
            lblReturnFilingReceiptNo.Visible = bVisible0;
            txtReceiptNo.Visible = bVisible1;
            lblReturnFilingTokenNo.Visible = bVisible2;
            txtTokenNo.Visible = bVisible3;
            lblDateofFiling.Visible = bVisible4;
        ...
        {
            if (Mode.ToUpper() == J_Mode.Add.ToUpper())
            {
            ...
            else if (Mode.ToUpper() == J_Mode.Edit.ToUpper())
            {
        ...
        private void LoadChallanFormComponents()
        {
            if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) <= T_FinancialYearID.F2012_13ID)
            {
                #region Load Challan Components For 24Q && 26Q When F.Y Less than equal 2012_13
                if ((strFormNo == T_FormNo.F24Q) || (strFormNo == T_FormNo.F26Q))
                {
                    lblChallanMinorHead.Visible = false;
                    cmbMinorHead.Visible = false;

                    lblChallanChequeNo.Visible = true;
                    txtChequeNo.Visible = true;

                    lblChallanSection.Visible = true;
                    cmbSection.Visible = true;
                    lblChallanSectionDisplay.Visible = false;

                    lblChallanSrlNo.Location = new Point(80, 4);
                    txtChallanSrlNo.Location = new Point(165, 1);

                    chkChallanBookEntry.Location = new Point(226, 3);

                    //lblChallanSection.Location = new Point(lblSection.Location.X, 32);
                    //cmbSection.Location = new Point(cmbSection.Location.X, 28);
                    //lblChallanSectionDisplay.Location = new Point(lblChallanSectionDisplay.Location.X, 54);

                    lblChallanDateOfPayment.Location = new Point(lblChallanDateOfPayment.Location.X, 10);
                    mskDateOfPayment.Location = new Point(mskDateOfPayment.Location.X, 8);
                    ...
                    lblChallanChequeNo.Location = new Point(lblChallanChequeNo.Location.X, 79);
                    txtChequeNo.Location = new Point(164, 76);

                    lblChallanTDS.Location = new Point(lblChallanTDS.Location.X, 101);
                    txtTDS.Location = new Point(164, 98);

                    lblChallanSurchage.Location = new Point(lblChallanSurchage.Location.X, 123);
                    txtSurcharge.Location = new Point(164, 120);

                    lblChallanEduCess.Location = new Point(lblChallanEduCess.Location.X, 145);
                    txtEducationCess.Location = new Point(164, 142);

                    lblChallanInterest.Location = new Point(lblChallanInterest.Location.X, 167);
                    txtInterests.Location = new Point(164, 164);

                    lblChallanFee.Location = new Point(lblChallanFee.Location.X, 189);
                    txtFee.Location = new Point(164, 186);

                    lblChallanOthers.Location = new Point(lblChallanOthers.Location.X, 212);
                    txtOthers.Location = new Point(164, 209);

                    lblChallanTotal.Location = new Point(lblChallanTotal.Location.X, 234);
                    txtTotalTax.Location = new Point(164, 231);

                    lblChallanRemarks.Location = new Point(lblChallanRemarks.Location.X, 258);
                    txtRemarks.Location = new Point(68, 255);

                    lblChallanInterestAllocated.Location = new Point(lblChallanInterestAllocated.Location.X, 285);
                    txtInterestAllocated.Location = new Point(164, 281);

                    lblChallanOthersAllocated.Location = new Point(lblChallanOthersAllocated.Location.X, 306);
                ...
                #region Load Challan Components For 27Q When F.Y Less than equal 2012_13
                else if ((strFormNo == T_FormNo.F27Q)||(strFormNo == T_FormNo.F27EQ))
                {
                    lblChallanMinorHead.Visible = false;
                    cmbMinorHead.Visible = false;
                    ...
                    //lblSection.Location = new Point(lblSection.Location.X, 32);
                    //cmbSection.Location = new Point(cmbSection.Location.X, 28);
                    //lblChallanSectionDisplay.Location = new Point(lblChallanSectionDisplay.Location.X, 54);

                    //lblDOP.Location = new Point(31, 10);
                    //mskDateOfPayment.Location = new Point(168, 8);

                    lblChallanDateOfPayment.Location = new Point(lblChallanDateOfPayment.Location.X, 4);
                    mskDateOfPayment.Location = new Point(mskDateOfPayment.Location.X, 2);
                    lblDOPFormat.Location = new Point(lblDOPFormat.Location.X, 4);

                    //lblBSRCode.Location = new Point(8, 35);
                    //txtBSRCode.Location = new Point(164, 32);


                    lblChallanBSRCode.Location = new Point(lblChallanBSRCode.Location.X, 28);
                    txtBSRCode.Location = new Point(txtBSRCode.Location.X, 25);

                    //lblTrVchNo.Location = new Point(8, 57);
                    //txtTransferVoucherNo.Location = new Point(164, 54);
                    //txtChallanNo.Location = new Point(164, 54);

                    lblChallanTrVchNo.Location = new Point(lblChallanTrVchNo.Location.X, 51);
                    txtTransferVoucherNo.Location = new Point(txtTransferVoucherNo.Location.X, 48);
                    txtChallanNo.Location = new Point(txtChallanNo.Location.X, 48);

                    //lblCheque.Location = new Point(51, 79);
                    //txtChequeNo.Location = new Point(164, 76);

                    lblChallanChequeNo.Location = new Point(lblChallanChequeNo.Location.X, 74);
                    txtChequeNo.Location = new Point(txtChequeNo.Location.X, 71);

                    //lblChallanTDS.Location = new Point(93, 101);
                    //txtTDS.Location = new Point(164, 98);

                    lblChallanTDS.Location = new Point(lblChallanTDS.Location.X, 97);
                    txtTDS.Location = new Point(txtTDS.Location.X, 94);

                    //lblSurc.Location = new Point(93, 123);
                    //txtSurcharge.Location = new Point(164, 120);

                    lblChallanSurchage.Location = new Point(lblChallanSurchage.Location.X, 119);
                    txtSurcharge.Location = new Point(txtSurcharge.Location.X, 116);

                    //lblEduCess.Location = new Point(63, 145);
                    //txtEducationCess.Location = new Point(164, 142);

                    lblChallanEduCess.Location = new Point(lblChallanEduCess.Location.X, 142);
                    txtEducationCess.Location = new Point(txtEducationCess.Location.X, 139);

                    //lblInt.Location = new Point(75, 167);
                    //txtInterests.Location = new Point(164, 164);

                    lblChallanInterest.Location = new Point(lblChallanInterest.Location.X, 165);
                    txtInterests.Location = new Point(txtInterests.Location.X, 162);

                    //label66.Location = new Point(75, 189);
                    //txtFee.Location = new Point(164, 186);

                    lblChallanFee.Location = new Point(lblChallanFee.Location.X, 188);
                    txtFee.Location = new Point(txtFee.Location.X, 185);

                    //lblOthers.Location = new Point(81, 212);
                    //txtOthers.Location = new Point(164, 209);

                    lblChallanOthers.Location = new Point(lblChallanOthers.Location.X, 211);
                    txtOthers.Location = new Point(txtOthers.Location.X, 208 );

                    //lblTotal.Location = new Point(29, 234);
                    //txtTotalTax.Location = new Point(164, 231);

                    lblChallanTotal.Location = new Point(lblChallanTotal.Location.X, 234);
                    txtTotalTax.Location = new Point(txtTotalTax.Location.X, 231);

                    //lblRemarks.Location = new Point(8, 258);
                    //txtRemarks.Location = new Point(68, 255);

                    lblChallanRemarks.Location = new Point(lblChallanRemarks.Location.X, 258);
                    txtRemarks.Location = new Point(txtRemarks.Location.X, 255);

                    //lblInterestAllocated.Location = new Point(43, 285);
                    //txtInterestAllocated.Location = new Point(164, 281);

                    lblChallanInterestAllocated.Location = new Point(lblChallanInterestAllocated.Location.X, 275);
                    txtInterestAllocated.Location = new Point(txtInterestAllocated.Location.X, 272);

                    //lblOthersAllocated.Location = new Point(49, 306);
                    //txtOthersAllocated.Location = new Point(164, 303);
                    lblChallanOthersAllocated.Location = new Point(lblChallanOthersAllocated.Location.X, 298);
                    txtOthersAllocated.Location = new Point(txtOthersAllocated.Location.X, 295);

                    pnlTop.Height = 67;

                    pnlBottom.Location = new Point(1, 67);
                    pnlBottom.Height = 354;
            ...
            {
                #region Load Challan Components For 24Q && 26Q When F.Y not less than equal 2012_13
                if ((strFormNo == T_FormNo.F24Q) || (strFormNo == T_FormNo.F26Q))
                {

                    lblChallanMinorHead.Visible = true;
                    cmbMinorHead.Visible = true;

                    lblChallanChequeNo.Visible = false;
                    txtChequeNo.Visible = false;

                    lblChallanSection.Visible = false;
                    ...
                    lblChallanTDS.Location = new Point(lblChallanTDS.Location.X, 110);
                    txtTDS.Location = new Point(164, 107);

                    lblChallanSurchage.Location = new Point(lblChallanSurchage.Location.X, 134);
                    txtSurcharge.Location = new Point(164, 131);

                    lblChallanEduCess.Location = new Point(lblChallanEduCess.Location.X, 158);
                    txtEducationCess.Location = new Point(164, 155);

                    lblChallanInterest.Location = new Point(lblChallanInterest.Location.X, 183);
                    txtInterests.Location = new Point(164, 180);

                    lblChallanFee.Location = new Point(lblChallanFee.Location.X, 205);
                    txtFee.Location = new Point(164, 202);

                    lblChallanOthers.Location = new Point(lblChallanOthers.Location.X, 228);
                    txtOthers.Location = new Point(164, 225);

                    lblChallanTotal.Location = new Point(lblChallanTotal.Location.X, 252);
                    txtTotalTax.Location = new Point(164, 249);

                    lblChallanMinorHead.Location = new Point(lblChallanMinorHead.Location.X, 277);
                    cmbMinorHead.Location = new Point(164, 273);

                    lblChallanRemarks.Location = new Point(lblChallanRemarks.Location.X, 310);
                    txtRemarks.Location = new Point(68, 307);

                    lblChallanInterestAllocated.Location = new Point(lblChallanInterestAllocated.Location.X, 341);
                    txtInterestAllocated.Location = new Point(164, 337);

                    lblChallanOthersAllocated.Location = new Point(lblChallanOthersAllocated.Location.X, 362);
                ...
                #region Load Challan Components For 27Q && 27EQ When F.Y not less than equal 2012_13
                else if ((strFormNo == T_FormNo.F27Q) || (strFormNo == T_FormNo.F27EQ))
                {
                    lblChallanMinorHead.Visible = true;
                    cmbMinorHead.Visible = true;
                    ...
                    lblChallanDateOfPayment.Location = new Point(lblChallanDateOfPayment.Location.X, 27);
                    mskDateOfPayment.Location = new Point(mskDateOfPayment.Location.X, 27);
                    lblDOPFormat.Location = new Point(lblDOPFormat.Location.X, 27);

                    //lblBSRCode.Location = new Point(8, 37);
                    //txtBSRCode.Location = new Point(164, 34);

                    lblChallanBSRCode.Location = new Point(lblChallanBSRCode.Location.X, 50);
                    txtBSRCode.Location = new Point(txtBSRCode.Location.X, 50);
                    ...
                    lblChallanTrVchNo.Location = new Point(lblChallanTrVchNo.Location.X, 72);
                    txtTransferVoucherNo.Location = new Point(txtTransferVoucherNo.Location.X, 72);
                    txtChallanNo.Location = new Point(txtChallanNo.Location.X, 72);


                    //lblTDS.Location = new Point(93, 110);
                    //txtTDS.Location = new Point(164, 107);

                    lblChallanTDS.Location = new Point(lblChallanTDS.Location.X, 94);
                    txtTDS.Location = new Point(txtTDS.Location.X, 94);

                    //lblSurc.Location = new Point(93, 134);
                    //txtSurcharge.Location = new Point(164, 131);

                    lblChallanSurchage.Location = new Point(lblChallanSurchage.Location.X, 116);
                    txtSurcharge.Location = new Point(txtSurcharge.Location.X, 116);

                    //lblEduCess.Location = new Point(63, 158);
                    //txtEducationCess.Location = new Point(164, 155);

                    lblChallanEduCess.Location = new Point(lblChallanEduCess.Location.X, 138);
                    txtEducationCess.Location = new Point(txtEducationCess.Location.X, 138);

                    //lblInt.Location = new Point(75, 183);
                    //txtInterests.Location = new Point(164, 180);

                    lblChallanInterest.Location = new Point(lblChallanInterest.Location.X, 160);
                    txtInterests.Location = new Point(txtInterests.Location.X, 160);

                    //label66.Location = new Point(75, 205);
                    //txtFee.Location = new Point(164, 202);

                    lblChallanFee.Location = new Point(lblChallanFee.Location.X, 182);
                    txtFee.Location = new Point(txtFee.Location.X, 182);

                    //lblOthers.Location = new Point(81, 228);
                    //txtOthers.Location = new Point(164, 225);

                    lblChallanOthers.Location = new Point(lblChallanOthers.Location.X, 204);
                    txtOthers.Location = new Point(txtOthers.Location.X, 204);

                    //lblTotal.Location = new Point(89, 252);
                    //txtTotalTax.Location = new Point(164, 249);

                    lblChallanTotal.Location = new Point(lblChallanTotal.Location.X, 226);
                    txtTotalTax.Location = new Point(txtTotalTax.Location.X, 226);

                    //lblMinor.Location = new Point(-2, 277);
                    //cmbMinorHead.Location = new Point(164, 273);

                    lblChallanMinorHead.Location = new Point(lblChallanMinorHead.Location.X, 248);
                    cmbMinorHead.Location = new Point(cmbMinorHead.Location.X, 248);

                    //lblRemarks.Location = new Point(8, 310);
                    //txtRemarks.Location = new Point(68, 307);

                    lblChallanRemarks.Location = new Point(lblChallanRemarks.Location.X, 270);
                    txtRemarks.Location = new Point(txtRemarks.Location.X, 270);

                    //lblInterestAllocated.Location = new Point(43, 341);
                    //txtInterestAllocated.Location = new Point(164, 337);

                    lblChallanInterestAllocated.Location = new Point(lblChallanInterestAllocated.Location.X, 292);
                    txtInterestAllocated.Location = new Point(txtInterestAllocated.Location.X, 292);

                    //lblOthersAllocated.Location = new Point(49, 362);
                    //txtOthersAllocated.Location = new Point(164, 359);

                    lblChallanOthersAllocated.Location = new Point(lblChallanOthersAllocated.Location.X, 314);
                    txtOthersAllocated.Location = new Point(txtOthersAllocated.Location.X, 314);
            ...
            //Challan Label Caption 

            #region Challan Lable Number
            if (strFormNo == T_FormNo.F24Q)
            {
                lblChallanSrlNo.Text = "Srl No. [301]";
                lblChallanSection.Text = "Section ";
                lblChallanDateOfPayment.Text = "Date of Payment [311]";
                lblChallanBSRCode.Text = "BSR Code [309]";
                lblChallanTrVchNo.Text = "Challan No. [310]";
                lblChallanChequeNo.Text = "Cheque No. [308]";
                lblChallanTDS.Text = "TDS [302]";
                lblChallanSurchage.Text = "Surcharge ";
                lblChallanEduCess.Text = "Education Cess [303]";
                lblChallanInterest.Text = "Interest [304]";
                lblChallanFee.Text = "Fee [305]";
                lblChallanOthers.Text = "Others [306]";
                lblChallanTotal.Text = "Total [307]";
                lblChallanMinorHead.Text = "Minor Head [312]";
                lblChallanInterestAllocated.Text = "Interest (Allocated) ";
                lblChallanOthersAllocated.Text = "Others (Allocated) ";
                lblChallanRemarks.Text = "Remarks ";
            }

            //Challan Lable Caption 
            else if (strFormNo == T_FormNo.F26Q)
            {
                lblChallanSrlNo.Text = "Srl No. [401]";
                lblChallanSection.Text = "Section  ";
                lblChallanDateOfPayment.Text = "Date of Payment [410]";
                lblChallanBSRCode.Text = "BSR Code [408]";
                lblChallanTrVchNo.Text = "Challan No. [409]";
                lblChallanChequeNo.Text = "Cheque No.  ";
                lblChallanTDS.Text = "TDS [402]";
                lblChallanSurchage.Text = "Surcharge  ";
                lblChallanEduCess.Text = "Education Cess ";
                lblChallanInterest.Text = "Interest [403]";
                lblChallanFee.Text = "Fee [404]";
                lblChallanOthers.Text = "Others [405]";
                lblChallanTotal.Text = "Total [406]";
                lblChallanMinorHead.Text = "Minor Head [411]";
                lblChallanInterestAllocated.Text = "Interest (Allocated) ";
                lblChallanOthersAllocated.Text = "Others (Allocated) ";
                lblChallanRemarks.Text = "Remarks ";
            }
            ...
            {
                lblChallanSrlNo.Text = "Srl No. [701]";
                lblChallanSection.Text = "Section  ";
                lblChallanDateOfPayment.Text = "Date of Payment [712]";
                lblChallanBSRCode.Text = "BSR Code [710]";
                lblChallanTrVchNo.Text = "Challan No. [711]";
                lblChallanChequeNo.Text = "Cheque No. [709]";
                lblChallanTDS.Text = "TDS [702]";
                lblChallanSurchage.Text = "Surcharge  [703]";
                lblChallanEduCess.Text = "Education Cess [704]";
                lblChallanInterest.Text = "Interest [705]";
                lblChallanFee.Text = "Fee [706]";
                lblChallanOthers.Text = "Others [707]";
                lblChallanTotal.Text = "Total [708]";
                lblChallanMinorHead.Text = "Minor Head [713]";
                lblChallanInterestAllocated.Text = "Interest (Allocated) ";
                lblChallanOthersAllocated.Text = "Others (Allocated) ";
                lblChallanRemarks.Text = "Remarks ";
            }
            ...
            {
                lblChallanSrlNo.Text = "Srl No. [651]";
                lblChallanSection.Text = "Section [652]";
                lblChallanDateOfPayment.Text = "Date of Payment [662]";
                lblChallanBSRCode.Text = "BSR Code [660]";
                lblChallanTrVchNo.Text = "Challan No. [661]";
                lblChallanChequeNo.Text = "Cheque No. [659]";
                lblChallanTDS.Text = "TDS [652]";
                lblChallanSurchage.Text = "Surcharge  [653]";
                lblChallanEduCess.Text = "Education Cess [654]";
                lblChallanInterest.Text = "Interest [655]";
                lblChallanFee.Text = "Fee [656]";
                lblChallanOthers.Text = "Others [657]";
                lblChallanTotal.Text = "Total [658]";
                lblChallanMinorHead.Text = "Minor Head [663]";
                lblChallanInterestAllocated.Text = "Interest (Allocated) ";
                lblChallanOthersAllocated.Text = "Others (Allocated) ";
                lblChallanRemarks.Text = "Remarks ";
            }
        ...
        #region LoadDeducteeFormComponents
        private void LoadDeducteeFormComponents()
        {
            if (Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex) <= T_FinancialYearID.F2012_13ID)
            {
                #region 24Q <= F.Y [2012-2013]
                if (strFormNo == T_FormNo.F24Q)
                {
                    lblDeducteeSrlNo.Text = "Employee Srl No [313]";
                    lblDeducteeName.Text = "Employee Name [316]";
                    lblDeducteePAN.Text = "PAN [315]";
                    lblSection.Text = "Section [317]";
                    lblDeducteeDate.Text = "Date [318]";
                    lblDeducteeAmtofPayment.Text = "Amount of Payment [320]";
                    lblDeducteeIncomeTax.Text = "Income Tax [321]";
                    lblDeducteeSurcharge.Text = "Surcharge";
                    lblDeducteeCess.Text = "Cess [322]";
                    lbldeducteeTotal.Text = "Total [323]";
                    lblDeducteeTaxDeposited.Text = "Tax Deposited [323]";
                    lblDeducteeRemarks.Text = "Remarks [326]"; ;
                    lblDeducteeCertificateNo.Text = "Certificate No. [327]";
                    //------------------------------------------------------------
                    lblDeducteeSrlNo.Location = new Point(lblChallanOthersAllocated.Location.X, 10);
                    txtDeducteeSrlNo.Location = new Point(txtDeducteeSrlNo.Location.X, 7);
                    //---
                    lblDeducteeName.Location = new Point(lblDeducteeName.Location.X, 32);
                    txtDeducteeName.Location = new Point(txtDeducteeName.Location.X, 30);
                    //----
                    chkCashBookEntry.Location = new Point(chkCashBookEntry.Location.X, 10);
                    //---
                    lblDeducteeCode.Location = new Point(lblDeducteeCode.Location.X, 53);
                    cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 52);
                    //---
                    lblDeducteePAN.Location = new Point(lblDeducteePAN.Location.X, 55);
                    txtDeducteePAN.Location = new Point(txtDeducteePAN.Location.X, 52);
                    lstPANHelp.Location = new Point(lstPANHelp.Location.X, 75);
                    //---
                    //lblDeducteeCode.Location = new Point(lblDeducteeCode.Location.X, 55);
                    //cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 52);
                    //---
                    lblSection.Location = new Point(lblSection.Location.X, 79);
                    cmbDDSection.Location = new Point(cmbDDSection.Location.X, 76);
                    lblDDSectionDisplay.Location = new Point(lblDDSectionDisplay.Location.X, 94);
                    //---
                    lblDeducteeDate.Location = new Point(lblDeducteeDate.Location.X, 110);
                    mskDeducteeDate.Location = new Point(mskDeducteeDate.Location.X, 108);
                    lblDeducteeDateFormat.Location = new Point(lblDeducteeDateFormat.Location.X, 112);
                    //---

                    lblDeducteeAmtofPayment.Location = new Point(lblDeducteeAmtofPayment.Location.X, 133);
                    txtDeducteeAmountOfPayment.Location = new Point(txtDeducteeAmountOfPayment.Location.X, 131);
                    //---
                    //lblDeducteeRate.Location =new Point(lblDeducteeRate.Location.X,155);
                    //txtDeducteeRate.Location =new Point(txtDeducteeRate.Location.X,153);
                    //lblDisplayRate.Location =new Point(lblDisplayRate.Location.X,153);
                    //---
                    lblDeducteeIncomeTax.Location = new Point(lblDeducteeIncomeTax.Location.X, 155);//177
                    txtDeducteeIncometax.Location = new Point(txtDeducteeIncometax.Location.X, 153);//175
                    //---
                    lblDeducteeSurcharge.Location = new Point(lblDeducteeSurcharge.Location.X, 177);//199
                    txtDeducteeSurcharge.Location = new Point(txtDeducteeSurcharge.Location.X, 175);//197
                    //---
                    lblDeducteeCess.Location = new Point(lblDeducteeCess.Location.X, 199);//221
                    txtDeducteeCess.Location = new Point(txtDeducteeCess.Location.X, 197);//219
                    //---
                    lbldeducteeTotal.Location = new Point(lbldeducteeTotal.Location.X, 221);//241
                    txtDeducteeTotal.Location = new Point(txtDeducteeTotal.Location.X, 219);//241
                    //---
                    lblDeducteeTaxDeposited.Location = new Point(lblDeducteeTaxDeposited.Location.X, 241);//263
                    txtDeducteeTaxDeposited.Location = new Point(txtDeducteeTaxDeposited.Location.X, 241);//263
                    //---
                    //lblGrossingUpIndicator.Location =new Point(lblGrossingUpIndicator.Location.X,291);
                    //cmbGrossingUpIndicator.Location =new Point(cmbGrossingUpIndicator.Location.X,291);
                    //---
                    //lblDeductedDate.Location =new Point(lblDeductedDate.Location.X,308);
                    //mskDeductedDate.Location =new Point(mskDeductedDate.Location.X,308);
                    //lblDeductedDateFormat.Location =new Point(lblDeductedDateFormat.Location.X,310);
                    //---                    
                    lblDeducteeRemarks.Location = new Point(lblDeducteeRemarks.Location.X, 263);//333
                    cmbRemarks.Location = new Point(cmbRemarks.Location.X, 263);//331
                    //---
                    lblDeducteeCertificateNo.Location = new Point(lblDeducteeCertificateNo.Location.X, 291);//356
                    txtCertificateNo.Location = new Point(txtCertificateNo.Location.X, 291);//354
                    //---
                    //lblTDSRate.Location =new Point(lblTDSRate.Location.X,379);
                    //cmbTDSRate.Location =new Point(cmbTDSRate.Location.X,376);
                    //---
                    //lblRemittance.Location =new Point(lblRemittance.Location.X,402);
                    //cmbRemittance.Location =new Point(cmbRemittance.Location.X,399);
                    //---
                    //lblUniqueAckn.Location =new Point(lblUniqueAckn.Location.X,425);
                    //txtUniqueAckn.Location =new Point(txtUniqueAckn.Location.X,423);
                    //---
                    //lblCountry.Location =new Point (lblCountry.Location.X,447);
                    //cmbCountry.Location = new Point(cmbCountry.Location.X, 445);
                }
                #endregion 

                #region 26Q <= F.Y [2013-2013]
                else if (strFormNo == T_FormNo.F26Q)
                {

                    lblDeducteeSrlNo.Text = "Deductee SrlNo. [412]";
                    lblDeducteeName.Text = "Deductee Name [416]";
                    lblDeducteePAN.Text = "PAN [415]";
                    lblDeducteeCode.Text = "Code [414]";
                    lblSection.Text = "Section [417]";
                    lblDeducteeDate.Text = "Date [418]";
                    lblDeducteeAmtofPayment.Text = "Amount of Payment [419]";
                    lblDeducteeRate.Text = "Rate [423]";
                    lblDeducteeIncomeTax.Text = "Income Tax [420]";
                    lblDeducteeSurcharge.Text = "Surcharge";
                    lblDeducteeCess.Text = "Cess";
                    lbldeducteeTotal.Text = "Total [424]";
                    lblDeducteeTaxDeposited.Text = "Tax Deposited [421]";
                    lblDeducteeRemarks.Text = "Remarks [424]";
                    lblDeducteeCertificateNo.Text = "Cretificate No.[425]";
                    //---------------------------------------------------
                    lblDeducteeSrlNo.Location = new Point(lblChallanOthersAllocated.Location.X, 10);
                    txtDeducteeSrlNo.Location = new Point(txtDeducteeSrlNo.Location.X, 7);
                    //---
                    ...
                    //---
                    lblDeducteeCode.Location = new Point(lblDeducteeCode.Location.X, 55);
                    cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 51);
                    //---
                    lblDeducteePAN.Location = new Point(lblDeducteePAN.Location.X, 53);
                    txtDeducteePAN.Location = new Point(txtDeducteePAN.Location.X, 53);
                    lstPANHelp.Location = new Point(lstPANHelp.Location.X, 75);
                    //---
                    lblDeducteeCode.Location = new Point(lblDeducteeCode.Location.X, 55);
                    cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 51);
                    ...
                    lblSection.Location = new Point(lblSection.Location.X, 79);
                    cmbDDSection.Location = new Point(cmbDDSection.Location.X, 76);
                    lblDDSectionDisplay.Location = new Point(lblDDSectionDisplay.Location.X, 88);
                    //---
                    lblDeducteeDate.Location = new Point(lblDeducteeDate.Location.X, 110);
                    mskDeducteeDate.Location = new Point(mskDeducteeDate.Location.X, 108);
                    lblDeducteeDateFormat.Location = new Point(lblDeducteeDateFormat.Location.X, 112);
                    ...
                    lblDisplayRate.Location =new Point(lblDisplayRate.Location.X,153);
                    //---
                    lblDeducteeIncomeTax.Location = new Point(lblDeducteeIncomeTax.Location.X, 177);
                    txtDeducteeIncometax.Location = new Point(txtDeducteeIncometax.Location.X, 175);
                    //---
                    lblDeducteeSurcharge.Location = new Point(lblDeducteeSurcharge.Location.X, 199);
                    txtDeducteeSurcharge.Location = new Point(txtDeducteeSurcharge.Location.X, 197);
                    //---
                    lblDeducteeCess.Location = new Point(lblDeducteeCess.Location.X, 221);
                    txtDeducteeCess.Location = new Point(txtDeducteeCess.Location.X, 219);
                    //---
                    lbldeducteeTotal.Location = new Point(lbldeducteeTotal.Location.X, 241);
                    txtDeducteeTotal.Location = new Point(txtDeducteeTotal.Location.X, 241);
                    //---
                    lblDeducteeTaxDeposited.Location = new Point(lblDeducteeTaxDeposited.Location.X, 263);
                    txtDeducteeTaxDeposited.Location = new Point(txtDeducteeTaxDeposited.Location.X, 263);
                    //---
                    //lblGrossingUpIndicator.Location =new Point(lblGrossingUpIndicator.Location.X,291);
                    //cmbGrossingUpIndicator.Location =new Point(cmbGrossingUpIndicator.Location.X,291);
                    //---
                    ...
                    //lblDeductedDateFormat.Location =new Point(lblDeductedDateFormat.Location.X,310);
                    //---                    
                    lblDeducteeRemarks.Location = new Point(lblDeducteeRemarks.Location.X, 291);//333
                    cmbRemarks.Location = new Point(cmbRemarks.Location.X, 291);//331
                    //---
                    lblDeducteeCertificateNo.Location = new Point(lblDeducteeCertificateNo.Location.X, 314);//356
                    txtCertificateNo.Location = new Point(txtCertificateNo.Location.X, 314);//354
                    //---
                    //lblTDSRate.Location =new Point(lblTDSRate.Location.X,379);
                    //cmbTDSRate.Location =new Point(cmbTDSRate.Location.X,376);
                    //---
                ...
                #region 27Q <= F.Y [2012-2013]
                else if (strFormNo == T_FormNo.F27Q) 
                {
                    lblDeducteeSrlNo.Text = "Deductee SrlNo. [714]";
                    lblDeducteeName.Text = "Deductee Name [718]";
                    lblDeducteePAN.Text = "PAN [717]";
                    lblDeducteeCode.Text = "Code [716]";
                    lblSection.Text = "Section [417]";
                    lblDeducteeDate.Text = "Date [720]";
                    lblDeducteeAmtofPayment.Text = "Amount of Payment [721]";
                    lblDeducteeRate.Text = "Rate [728]";
                    lblDeducteeIncomeTax.Text = "Income Tax [722]";
                    lblDeducteeSurcharge.Text = "Surcharge [723]";
                    lblDeducteeCess.Text = "Cess [724]";
                    lbldeducteeTotal.Text = "Total [725]";
                    lblDeducteeTaxDeposited.Text = "Tax Deposited [726]";
                    lblGrossingUpIndicator.Text = "Grossing Up Indicator";
                    lblDeducteeRemarks.Text = "Remarks [729]";
                    lblDeducteeCertificateNo.Text = "Cretificate No.[730]";
                    lblTDSRate.Text = "TDS Rate Act[731]";
                    lblRemittance.Text = "Remittance [732]";
                    lblUniqueAckn.Text = "Unique Ack (Form 15CA) [733]";
                    lblCountry.Text = "Country [734]";
                    //--------------------------------------------------------------------
                    lblDeducteeSrlNo.Location = new Point(lblChallanOthersAllocated.Location.X, 10);
                    txtDeducteeSrlNo.Location = new Point(txtDeducteeSrlNo.Location.X, 7);
                    //---
                    ...
                    //lblDeductedDateFormat.Location =new Point(lblDeductedDateFormat.Location.X,310);
                    //---                    
                    lblDeducteeRemarks.Location = new Point(lblDeducteeRemarks.Location.X, 308);//333
                    cmbRemarks.Location = new Point(cmbRemarks.Location.X, 308);//331
                    //---
                    lblDeducteeCertificateNo.Location = new Point(lblDeducteeCertificateNo.Location.X, 333);//356
                    txtCertificateNo.Location = new Point(txtCertificateNo.Location.X, 331);//354
                    //---
                    //lblTDSRate.Location =new Point(lblTDSRate.Location.X,379);
                    //cmbTDSRate.Location =new Point(cmbTDSRate.Location.X,376);
                    //---
                ...
                #region 27EQ <= F.Y [2012-2013]
                else if (strFormNo == T_FormNo.F27EQ)
                {
                    lblDeducteeSrlNo.Text = "Collectee SrlNo. ";
                    lblDeducteeName.Text = "Collectee Name [668]";
                    lblDeducteePAN.Text = "PAN [667]";
                    lblDeducteeCode.Text = "Code [666]";
                    lblSection.Text = "Section [417]";
                    lblDeducteeDate.Text = "Date [671]";
                    lblDeducteeAmtofPayment.Text = "Amount of Payment [670]";
                    lblDeducteeRate.Text = "Rate [679]";
                    lblDeducteeIncomeTax.Text = "Income Tax [673]";
                    lblDeducteeSurcharge.Text = "Surcharge [674]";
                    lblDeducteeCess.Text = "Cess [675]";
                    lbldeducteeTotal.Text = "Total [676]";
                    lblDeducteeTaxDeposited.Text = "Tax Deposited [679]";
                    lblTotValueOfPurchase.Text = "Tot value of Purchase [669]";
                    lblDeducteeRemarks.Text = "Remarks [680]";
                    lblDeducteeCertificateNo.Text = "Cretificate No.[681]";
                    //----------------------------------------------------------
                    //lblDeducteeSrlNo.Location = new Point(lblChallanOthersAllocated.Location.X, 9);
                    //txtDeducteeSrlNo.Location = new Point(txtDeducteeSrlNo.Location.X, 7);
                    ////----
                    //chkCashBookEntry.Location = new Point(chkCashBookEntry.Location.X, 10);
                    ////---
                    ...
                    //cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 52);
                    ////---
                    //lblDeducteeName.Location = new Point(lblDeducteeName.Location.X, 31);
                    //txtDeducteeName.Location = new Point(txtDeducteeName.Location.X, 30);
                    ////---
                    //lblDeducteePAN.Location = new Point(lblDeducteePAN.Location.X, 54);
                    //txtDeducteePAN.Location = new Point(txtDeducteePAN.Location.X, 52);
                    //lstPANHelp.Location = new Point(lstPANHelp.Location.X, 75);
                    ////---
                    ////lblDeducteeCode.Location = new Point(lblDeducteeCode.Location.X, 79);
                    ////cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 76);
                    ////---
                    //lblSection.Location = new Point(lblSection.Location.X, 78);
                    //cmbDDSection.Location = new Point(cmbDDSection.Location.X, 75);
                    //lblDDSectionDisplay.Location = new Point(lblDDSectionDisplay.Location.X, 98);
                    ////---
                    //lblDeducteeDate.Location = new Point(lblDeducteeDate.Location.X, 122);
                    //mskDeducteeDate.Location = new Point(mskDeducteeDate.Location.X, 120);
                    //lblDeducteeDateFormat.Location = new Point(lblDeducteeDateFormat.Location.X, 124);
                    ////---

                    //lblDeducteeAmtofPayment.Location = new Point(lblDeducteeAmtofPayment.Location.X, 145);
                    //txtDeducteeAmountOfPayment.Location = new Point(txtDeducteeAmountOfPayment.Location.X, 143);
                    ////---
                    ////lblDeducteeRate.Location =new Point(lblDeducteeRate.Location.X,167);
                    ////txtDeducteeRate.Location =new Point(txtDeducteeRate.Location.X,165);
                    ////lblDisplayRate.Location =new Point(lblDisplayRate.Location.X,165);
                    ////---
                    //lblDeducteeIncomeTax.Location = new Point(lblDeducteeIncomeTax.Location.X, 189);
                    //txtDeducteeIncometax.Location = new Point(txtDeducteeIncometax.Location.X, 187);
                    ////---
                    //lblDeducteeSurcharge.Location = new Point(lblDeducteeSurcharge.Location.X, 211);
                    //txtDeducteeSurcharge.Location = new Point(txtDeducteeSurcharge.Location.X, 209);
                    ////---
                    //lblDeducteeCess.Location = new Point(lblDeducteeCess.Location.X, 233);
                    //txtDeducteeCess.Location = new Point(txtDeducteeCess.Location.X, 231);
                    ////---
                    //lbldeducteeTotal.Location = new Point(lbldeducteeTotal.Location.X, 253);
                    //txtDeducteeTotal.Location = new Point(txtDeducteeTotal.Location.X, 253);
                    ////---
                    //lblDeducteeTaxDeposited.Location = new Point(lblDeducteeTaxDeposited.Location.X, 275);
                    //txtDeducteeTaxDeposited.Location = new Point(txtDeducteeTaxDeposited.Location.X, 275);
                    ////---
                    ////lblGrossingUpIndicator.Location =new Point(lblGrossingUpIndicator.Location.X,292);
                    ////cmbGrossingUpIndicator.Location =new Point(cmbGrossingUpIndicator.Location.X,286);
                    ////---
                    ////lblDeductedDate.Location =new Point(lblDeductedDate.Location.X,320);
                    ////mskDeductedDate.Location =new Point(mskDeductedDate.Location.X,320);
                    ////lblDeductedDateFormat.Location =new Point(lblDeductedDateFormat.Location.X,322);
                    ////---                    
                    //lblTotValueOfPurchase.Location = new Point(lblTotValueOfPurchase.Location.X, 292);
                    //txtTotalValueOfPurchase.Location = new Point(txtTotalValueOfPurchase.Location.X, 286);
                    ////---
                    //lblDeducteeRemarks.Location = new Point(lblDeducteeRemarks.Location.X, 345);
                    //cmbRemarks.Location = new Point(cmbRemarks.Location.X, 343);
                    ////---
                    //lblDeducteeCertificateNo.Location = new Point(lblDeducteeCertificateNo.Location.X, 368);
                    //txtCertificateNo.Location = new Point(txtCertificateNo.Location.X, 366);
                    ////---
                    ////lblTDSRate.Location =new Point(lblTDSRate.Location.X,391);
                    ////cmbTDSRate.Location =new Point(cmbTDSRate.Location.X,388);
                    ////---
                    ////lblRemittance.Location =new Point(lblRemittance.Location.X,414);
                    ////cmbRemittance.Location =new Point(cmbRemittance.Location.X,411);
                    ////---
                    ////lblUniqueAckn.Location =new Point(lblUniqueAckn.Location.X,437);
                    ////txtUniqueAckn.Location =new Point(txtUniqueAckn.Location.X,435);
                    ////---
                    ////txtDeducteeCess.Location =new Point(txtDeducteeCess.Location.X,459);
                    ////txtDeducteeCess.Location =new Point(txtDeducteeCess.Location.X,457);
                    ////---
                    ////lblCountry.Location =new Point (lblCountry.Location.X,101);
                    ////cmbCountry.Location = new Point(cmbCountry.Location.X, 101);
                }                
                #endregion
            }
            else
            {
                #region 24Q Not <= F.Y [2012-2013]
                if (strFormNo == T_FormNo.F24Q)
                {
                    lblDeducteeSrlNo.Text = "Employee Srl No [313]";
                    lblDeducteeName.Text = "Employee Name [316]";
                    ...
                    cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 52);
                    //---
                    lblDeducteePAN.Location = new Point(98, 55);
                    txtDeducteePAN.Location = new Point(167, 52);
                    lstPANHelp.Location = new Point(lstPANHelp.Location.X, 75);
                    //---
                    //lblDeducteeCode.Location = new Point(lblDeducteeCode.Location.X, 55);
                    //cmbDeducteeCode.Location = new Point(cmbDeducteeCode.Location.X, 52);
                    //---
                    lblSection.Location = new Point(lblSection.Location.X, 75);
                    cmbDDSection.Location = new Point(cmbDDSection.Location.X, 74);
                    lblDDSectionDisplay.Location = new Point(lblDDSectionDisplay.Location.X, 94);
                    //---
                    lblDeducteeDate.Location = new Point(lblDeducteeDate.Location.X, 110);
                    mskDeducteeDate.Location = new Point(mskDeducteeDate.Location.X, 108);
                ...
                if (strFormNo == T_FormNo.F27Q)
                {
                    ...
                    txtCertificateNo.Location = new Point(txtCertificateNo.Location.X, 314);//354
                    //---
                    lblTDSRate.Location = new Point(lblTDSRate.Location.X, 362);//379
                    cmbTDSRate.Location = new Point(cmbTDSRate.Location.X, 360);//376
                    //---
                    lblRemittance.Location = new Point(lblRemittance.Location.X, 385);//402
                    cmbRemittance.Location = new Point(cmbRemittance.Location.X, 382);//399
                    //---
                    lblUniqueAckn.Location = new Point(lblUniqueAckn.Location.X, 408);//425
                    txtUniqueAckn.Location = new Point(txtUniqueAckn.Location.X, 405);//423
                    //---
                    lblCountry.Location = new Point(lblCountry.Location.X, 431);//447
                    cmbCountry.Location = new Point(cmbCountry.Location.X, 429);//445
        ...
        #region ChallanControlsVisibility
        private void ChallanControlsVisibility(string SearchMode)
        {
        ...
        #region Check_Record[1]
        private long Check_Record(long lngSrchId)
        {
            if (lblSearchMode.Text == Tabbed_Mode.Challan)
            {
                if (dmlService.J_IsRecordExist(dmlService.J_pCommand, "TRN_CHALLAN", "CHALLAN_ID", lngSrchId) == true) return lngSrchId;
            ...
            {
                if (dmlService.J_IsRecordExist(dmlService.J_pCommand, "TRN_DEDUCTEE_DETAILS", "DEDUCTEE_DETAIL_ID", lngSrchId) == true) return lngSrchId;
        ...
        private long Check_Record(long SrchId, string SrchTable, string SrchColumn, ref DGControl.DGControl DGControl)
        {
            ...
            //Added by indrajit on 18-02-2013 to refresh challan info on deductee screen
            if (ShowChallanDetailsRecord(lngChallanID) == false)
            {
                if (dsetGridClone == null) return 0;
                dmlService.J_setGridPosition(ref dgcViewChallan, dsetGridClone, "CHALLAN_ID", lngChallanID);
