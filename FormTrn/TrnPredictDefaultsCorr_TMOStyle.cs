...
{
    public partial class TrnPredictDefaultsCorr_TMOStyle : TDSMAN.FormGen.GenForm
    {
        ResizeForm _form_resize;

        #region System Generated Code
        public TrnPredictDefaultsCorr_TMOStyle()
        {
            InitializeComponent();
            //--
        ...
        ToolTip tllChallanVerify = new ToolTip();
        //-----------------------------------------
        string strSmilyImage = "smily.png";
        bool blResize = true;
        ...
        //-- CHALLAN GRID COLUMNS
        int intGridChallanID = 0, intGridSl = 1, intGridSection = 2, intGridChallanNo = 3, intGridDepositDate = 4, intGridBSRCode = 5, intGridTax = 6;
        int intGridDeducteeTotal = 7, intGridDifference = 8, intGridReceiptNo = 9, intGridAvailableBalance = 10, intGridOverBooked = 11;
        //
        string strShortDeduction27QTable = "";
        ...
        private void TrnHealthCheckup_Load(object sender, EventArgs e)
        {
            ...
            //
            strShortDeduction27QTable = "TMP_SHORT_DEDUCTION_27Q_" + TDSMAN.Classes.TDSMAN.T_pProductSerial.Replace("-","_");
            //
            ...
            else if (strFromModule == T_OTHERMODULENAME.STANDALONE)
            {
                pnlSelection.Visible = true;
                rbnSelectRegular_CheckedChanged(sender, e);
                //
                pnlControls.Enabled = false;
                //
                btnXit.Text = "&Back";//E&xit
                //
                return;
            }
            blnRadioExit = false;
        ...
        private void rbnRegularCorrectionReturn_CheckedChanged(object sender, EventArgs e)
        {
            ...
            //
            //lblChallanCheckLabel.Visible = false;
            //btnChallanVerify.Visible = false;
            //--
            if (rbnRegularReturn.Checked == true)
            {
                ...
                //--
                if (strFromModule != T_OTHERMODULENAME.STANDALONE)
                {
                ...
                if (TDSMAN.Classes.TDSMAN.T_pPredictDefaultsId > 0)
                {
                    ...
                    while (drdGetDetails.Read())
                    {
                        ...
                        //--
                        if(Convert.ToString(drdGetDetails["COMPANY_NAME"]).Length > 47)
                            lblValueTAN.Text = Convert.ToString(drdGetDetails["TAN_NO"]) + " [ " + cmnService.J_Left(Convert.ToString(drdGetDetails["COMPANY_NAME"]), 47) + " ]";    
                        else
                            lblValueTAN.Text = Convert.ToString(drdGetDetails["TAN_NO"]) + " [ " + Convert.ToString(drdGetDetails["COMPANY_NAME"]) + " ]";
                        //
                        lblValueFAYear.Text = Convert.ToString(drdGetDetails["FA_YEAR"]);
                        lblValueQtr.Text = Convert.ToString(drdGetDetails["QTR"]);
                        lblValueFormNo.Text = Convert.ToString(drdGetDetails["FORM_NO"]);
                        lblValueFormNoDisplay.Text = TdsMan.GetFormNoIT2025(lblValueFormNo.Text) + " (" + lblValueFormNo.Text + ")";
                        //--
                ...
                else
                {
                    if (TDSMAN.Classes.TDSMAN.T_pBookMarkOption == true)
                    {
                        if (dmlService.J_IsRecordExist("MST_BOOKMARK_DETAIL") == true)
                        {
                            ...
                            //--
                            if (strCompanyName.Length > 47)
                                lblValueTAN.Text = strTAN + " [ " + cmnService.J_Left(strCompanyName, 47) + " ]";
                            else
                                lblValueTAN.Text = strTAN + " [ " + strCompanyName + " ]";
                            //
                            lblValueFAYear.Text = strFAYear;
                            lblValueQtr.Text = strQuarter;
                            lblValueFormNo.Text = strFormNoBkmark;
                            //--
                            blnSelectComboExit = false;
        ...
        private void cmbMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            LoadDetailsRegular(lngBasicInfoID, sender, e);
            //
        ...
        private void btnViewShortDeductions_Click(object sender, EventArgs e)
        {
            ...
            //
            if (rbnRegularReturn.Checked == true && Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2014_15ID && cmbFormNo.Text == T_FormNo.F24Q && cmbQuarter.Text == T_Qtr.Q4)
            {
                tbcView.SelectTab(tbpShortDeductionsSD);
                lblSection.Visible = false;
                cmbSection.Visible = false;
        ...
        private void btnPrintGrid_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (tbcView.SelectedTab == tbpShortDeductions)
                {
                    if (rbnRegularReturn.Checked == true)
                    {
                        rptDialog.PrintShortDeductionRegular_RDLC(lngBasicInfoID,
                                                 cmbFormNo.Text,
                                                 cmbQuarter.Text,
                                                 ...
                                                 strShortDeduction27QTable);
                    }
                    else if (rbnCorrectionReturn.Checked == true)
                    {
                        blActivate = false;
                        rptDialog.PrintShortDeductionCorr_RDLC(lngBasicInfoID,
                                                 Convert.ToInt32(cmnService.J_GetComboBoxItemId(ref cmbSection, cmbSection.SelectedIndex)),
                                                 Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteeName.Text)),
                                                 Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteePAN.Text)));
                    }
                ...
                else if (tbcView.SelectedTab == tbpLatePayments)
                {
                    if (rbnRegularReturn.Checked == true)
                    {
                        rptDialog.PrintLatePaymentRegular_RDLC(lngBasicInfoID,
                                              cmbFormNo.Text,
                                              cmbQuarter.Text,
                                              cmbFinancialYear.Text,
                                              Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteeName.Text)),
                    ...
                    {
                        blActivate = false;
                        rptDialog.PrintLatePaymentCorr_RDLC(lngBasicInfoID,
                                                 Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteeName.Text)),
                                                 Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteePAN.Text)),
                                                 lblTotalLabelLatePayments.Text + " Interest Amount Reported : " + lblInterestAmountReported.Text + " Interest Payable : " + lblInterestPayable.Text);
                    }
                ...
                else if (tbcView.SelectedTab == tbpLateDeductions) //-- 2015/07/20
                {
                    if (rbnRegularReturn.Checked == true)
                    {
                        rptDialog.PrintLateDeductionRegular_RDLC(lngBasicInfoID,
                                              cmbFormNo.Text,
                                              cmbQuarter.Text,
                                              cmbFinancialYear.Text,
                                              Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteeName.Text)),
                    ...
                    {
                        blActivate = false;
                        rptDialog.PrintLateDeductionCorr_RDLC(lngBasicInfoID,
                                                 Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteeName.Text)),
                                                 Convert.ToString(cmnService.J_ReplaceQuote(txtDeducteePAN.Text)),
                                                   lblTotalLabelLateDeductions.Text + " Interest Amount Reported : " + lblInterestAmountReportedLD.Text + " Interest Payable : " + lblInterestPayableLD.Text);
                    }
                ...
                else if (tbcView.SelectedTab == tbpShortDeductionsSD)
                {
                    if (rbnRegularReturn.Checked == true)
                    {
                        rptDialog.PrintShortDeductionRegularSD_RDLC(lngBasicInfoID,
                                                 cmbFormNo.Text,
                                                 cmbQuarter.Text,
                                                 cmbFinancialYear.Text,
                                                 Convert.ToString(cmbCompany.Text));
                    ...
                    else if (rbnCorrectionReturn.Checked == true)
                    {
                        ...
                        rptDialog.PrintShortDeductionCorrSD_RDLC(lngBasicInfoID);//,
                                                 //strFormNo,
                                                 //strQtr,
                                                 //cmbFinancialYear.Text,
                                                 //Convert.ToString(cmbCompany.Text));
        ...
        private void LoadDetailsRegular(long BasicInfoID, object sender, EventArgs e)
        {
            long lngShortDeductionsNos = 0;
            double dblShortDeductionsAmount = 0;
            intCOMPLIANCE_HIGHER_RATE_FLAG = 0;
            //
            //--
            string strShortDeductionsRoundOffValue = "", strShortDeductionsFormatValue = "";
            if (TDSMAN.Classes.TDSMAN.T_ROUND_OFF_TAXABLE_AMOUNT_26Q_27Q_27EQ == true)
            {
                strShortDeductionsRoundOffValue = "0";
                strShortDeductionsFormatValue = "0";
            }
            else
            {
                strShortDeductionsFormatValue = "0.00";
                strShortDeductionsRoundOffValue = "2";
            }
            //--
            try
            {
                ...
                if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2014_15ID && cmbFormNo.Text == T_FormNo.F24Q && cmbQuarter.Text == T_Qtr.Q4)
                {
                    ...
                    if (dmlServiceSD2.J_IsDatabaseObjectExist(TDSMAN.Classes.TDSMAN.T_tblTEMP_PREDICT_SHORT_DED_SD) == false)
                    {
                        ...
                        strSQL = "CREATE TABLE " + TDSMAN.Classes.TDSMAN.T_tblTEMP_PREDICT_SHORT_DED_SD + @" (
                             ...
                             " + cmnService.J_GetDataType("VERIFIED_PAN_STATUS", J_ColumnType.String, J_DefaultValue.YES) + @")";
                    ...
                    strSQL = @"INSERT INTO  " + TDSMAN.Classes.TDSMAN.T_tblTEMP_PREDICT_SHORT_DED_SD + @" 
                               ...
                               SELECT      TRN_SALARY_DETAILS.BASIC_INFO_ID             AS BASIC_INFO_ID, 
                                          " + Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) + @",
                                           ...
                                           TRN_SALARY_DETAILS.US_89_LESS                AS RELIEF_AMOUNT,
                                           TRN_SALARY_DETAILS.TAXABLE_AMOUNT            AS CURRENT_SALARY,
                                           TRN_SALARY_DETAILS.TOTAL_TAX_DEDUCTED_AMOUNT + TRN_SALARY_DETAILS.PREVIOUS_TAX_DEDUCTED_TOTAL + TRN_SALARY_DETAILS.SUPER_ANN_TAX + TRN_SALARY_DETAILS.TDS_US_192_2B_AMOUNT AS TDS_ENTERED,
...
//                    }
                    ...
                    foreach (DataTable DtTable in ds.Tables)
                    {
                        foreach (DataRow dr in DtTable.Rows)
                        {
                            ...
                            if (dr["EMPLOYEE_PAN"].ToString() == "AFVPH8262B")
                            {
                                return;
                            }
                            ...
                            else
                            {
                                ...
                                if (dblTaxPaid < dblTotalTaxTobePaid)
                                {
                                    dblShortDeductionsAmt = dblTotalTaxTobePaid - cmnService.J_ReturnDoubleValue(dr["RELIEF_AMOUNT"].ToString()) - dblTaxPaid;
                                    //
                                    strSQL = @"UPDATE " + TDSMAN.Classes.TDSMAN.T_tblTEMP_PREDICT_SHORT_DED_SD + @" 
                                                                       SET   SHORT_DEDUCTION_AMOUNT = " + dblShortDeductionsAmt + @",
                    ...
                    if (lngShortDeductionsNos == 0)
                    {
                        ...
                        btnViewShortDeductions.BackColor = Color.LightGray;
                        //
                        btnViewShortDeductions.Visible = false;
                        //-- 2023/02/20
                        lblShortDeductionsNos.Text =""; //"✔";// "\u263A";// 
                        if (File.Exists(Path.Combine(Application.StartupPath, strSmilyImage)) == true)
                            lblShortDeductionsNos.Image = new Bitmap(Path.Combine(Application.StartupPath, strSmilyImage));
                        //lblShortDeductionsNos.Tag = strSmilyImage;
                        lblSDTick.Text = "\u2714";
                        lblSDTick.ForeColor = Color.Green;
                        lblSDTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                    }
                    else
                    {
                        //strSQL = "SELECT COUNT(*) AS COUNT_PREDICT_SHORT_DED_SD FROM " + TDSMAN.Classes.TDSMAN.T_tblTEMP_PREDICT_SHORT_DED_SD + " WHERE SHORT_DEDUCTION_AMOUNT > 0";
                        //dblShortDeductionsAmount = cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar(dmlServiceSD.J_pCommand, strSQL)));
                        //
                        btnViewShortDeductions.Enabled = true;
                        btnViewShortDeductions.BackColor = Color.Lavender;
                        //
                        btnViewShortDeductions.Visible = true;
                        //
                        lblShortDeductionsNos.Image = null;
                        lblShortDeductionsNos.Text = Convert.ToString(lngShortDeductionsNos) + " nos.";
                        //
                        lblSDTick.Text = "\u0058";
                        lblSDTick.ForeColor = Color.Red;
                        lblSDTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                    }
                    //
                    ...
                    //     @"AND    TRN_DEDUCTEE_DETAILS.TAX_DEPOSITED_AMOUNT < " + strActualAmtCreation + " " +
                    //     @"AND    MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2)";

                    //lngShortDeductionsNos = lngShortDeductionsNos + cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL)));
                    //if (lngShortDeductionsNos == 0)
                    //{
                    //    btnViewShortDeductions.Enabled = false;
                    //    btnViewShortDeductions.BackColor = Color.LightGray;
                ...
                else if (cmbFormNo.Text == T_FormNo.F27Q) //-- DTAA
                {
                    #region F27Q
                    //
                    //string strShortDeduction27QTable = "TRN_SHORT_DEDUCTION_27Q_" + TDSMAN.Classes.TDSMAN.T_pProductSerial;
                    if (dmlService.J_IsDatabaseObjectExist(strShortDeduction27QTable) == true)
                    {
                        strSQL = @"DROP TABLE " + strShortDeduction27QTable;
                        dmlService.J_ExecSql(strSQL);
                    }
                    //--
                    strSQL = @"SELECT *
                               INTO " + strShortDeduction27QTable + @"
                               FROM TRN_DEDUCTEE_DETAILS WHERE DEDUCTEE_DETAIL_ID = 0 ";
                    dmlService.J_ExecSql(strSQL);
                    //--
                    strSQL = @"DELETE FROM " + strShortDeduction27QTable;
                    dmlService.J_ExecSql(strSQL);
                    //-- INSERT THE DATA
                    if (J_Var.J_pDatabaseType == J_DatabaseType.MsAccess)
                        strSQL = @"INSERT INTO " + strShortDeduction27QTable + " SELECT * FROM TRN_DEDUCTEE_DETAILS WHERE TRN_DEDUCTEE_DETAILS.CERTIFICATE_NO = '' AND TRN_DEDUCTEE_DETAILS.BASIC_INFO_ID = " + BasicInfoID;
                    else
                    {
                        strSQL = @"INSERT INTO " + strShortDeduction27QTable + @" (
                                                    CHALLAN_ID,
                                                    BASIC_INFO_ID,
                                                    SL_NO,
                                                    PARTY_ID,
                                                    ...
                                                    SEC194N_EXCESS_1CRORE_AMOUNT,
                                                    EMPLOYEE_SERIAL_NO,
                                                    SEC194N_20LAKH_1CRORE_PAYMENT_AMOUNT,
                                                    SEC194N_GREATER_1CRORE_PAYMENT_AMOUNT,
                                                    TDS_CLAUSE_YN,
                                                    TDS_CLAUSE_CHALLAN_NO,
                                                    TDS_CLAUSE_PAYMENT_DATE,
                                                    SEC194NC_EXCESS_3CRORE,
                                                    SEC194NFT_20L_3CRORE,
                                                    SEC194NFT_EXCESS_3CRORE,
                                                    SECTION_115BAC_FLAG,
                                                    DEDUCTEE_CODE, TAX_ID_NUM_COUNTRY_OF_RESIDENCE
                                                )
                                                SELECT 
                                                    CHALLAN_ID,
                                                    BASIC_INFO_ID,
                                                ...
                                                AND    BASIC_INFO_ID = " + BasicInfoID;
                    }
                    dmlService.J_ExecSql(strSQL);
                    //--
                    strSQL = dmlService.ReturnALTERSyntaxSequelServer(strShortDeduction27QTable, "SCHEDULED_RATE", "MONEY", "", "", "0");
                    dmlService.J_ExecSql(strSQL);
                    strSQL = @"UPDATE " + strShortDeduction27QTable + " SET SCHEDULED_RATE = 0";
                    dmlService.J_ExecSql(strSQL);
                    strSQL = dmlService.ReturnALTERSyntaxSequelServer(strShortDeduction27QTable, "DTAA_RATE", "MONEY", "", "", "0");
                    dmlService.J_ExecSql(strSQL);
                    strSQL = @"UPDATE " + strShortDeduction27QTable + " SET DTAA_RATE = 0";
                    dmlService.J_ExecSql(strSQL);
                    strSQL = dmlService.ReturnALTERSyntaxSequelServer(strShortDeduction27QTable, "SCHEDULED_TAX_AMOUNT", "MONEY", "", "", "0");
                    dmlService.J_ExecSql(strSQL);
                    strSQL = @"UPDATE " + strShortDeduction27QTable + " SET SCHEDULED_TAX_AMOUNT = 0";
                    dmlService.J_ExecSql(strSQL);
                    //-- UPDATE ALL SECTIONS EXCEPT 195 WITH STIPULATED RATES
                    if (J_Var.J_pDatabaseType == J_DatabaseType.SqlServer)
                        strSQL = @"UPDATE " + strShortDeduction27QTable + @"
                              SET SCHEDULED_RATE = 
                              CASE WHEN MST_DEDUCTEE.DEDUCTEE_PAN = 'PANNOTAVBL'                 
                                        THEN MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE                         
                                        ELSE CASE WHEN SUBSTRING(MST_DEDUCTEE.DEDUCTEE_PAN, 4, 1) = 'P' OR SUBSTRING(MST_DEDUCTEE.DEDUCTEE_PAN, 4, 1) = 'H'                        
                                                  THEN MST_NONSALARY_TAX_SLAB.NON_COMPANY_RATE                        
                                                  ELSE MST_NONSALARY_TAX_SLAB.COMPANY_RATE                         
                                              END                         
                              END 
                              FROM " + strShortDeduction27QTable + @"
                              INNER JOIN MST_DEDUCTEE 
			                        ON MST_DEDUCTEE.DEDUCTEE_ID = " + strShortDeduction27QTable + @".PARTY_ID
                              INNER JOIN MST_NONSALARY_TAX_SLAB 
			                        ON MST_NONSALARY_TAX_SLAB.SECTION_ID = " + strShortDeduction27QTable + @".SECTION_ID  
                              WHERE " + strShortDeduction27QTable + @".PAYMENT_DATE BETWEEN MST_NONSALARY_TAX_SLAB.FROM_DATE AND MST_NONSALARY_TAX_SLAB.TO_DATE";
                    else
                        strSQL = @"UPDATE ((" + strShortDeduction27QTable + @" 
                                        INNER JOIN MST_DEDUCTEE 
                                            ON MST_DEDUCTEE.DEDUCTEE_ID = " + strShortDeduction27QTable + @".PARTY_ID)
                                        INNER JOIN MST_NONSALARY_TAX_SLAB 
                                            ON MST_NONSALARY_TAX_SLAB.SECTION_ID = " + strShortDeduction27QTable + @".SECTION_ID)
                                    SET " + strShortDeduction27QTable + @".SCHEDULED_RATE =
                                        IIF(MST_DEDUCTEE.DEDUCTEE_PAN='PANNOTAVBL',
                                            MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE,
                                            IIF(MID(MST_DEDUCTEE.DEDUCTEE_PAN,4,1) = 'P' OR MID(MST_DEDUCTEE.DEDUCTEE_PAN,4,1)= 'H',
                                                MST_NONSALARY_TAX_SLAB.NON_COMPANY_RATE,
                                                MST_NONSALARY_TAX_SLAB.COMPANY_RATE
                                            )
                                        )
                                    WHERE " + strShortDeduction27QTable + @".PAYMENT_DATE 
                                          BETWEEN MST_NONSALARY_TAX_SLAB.FROM_DATE 
                                              AND MST_NONSALARY_TAX_SLAB.TO_DATE";
                    dmlService.J_ExecSql(strSQL);
                    //-- UPDATE RATE 20% AGAINST SECTION 195 (FOR DTAA ONLY) //-- 2026/01/15
                    strSQL = @"UPDATE " + strShortDeduction27QTable + @"
                                SET SCHEDULED_RATE = 20
				                WHERE SECTION_ID = 30 AND TDS_APPLICABILITY_ID = 2";
                    dmlService.J_ExecSql(strSQL);
                    //-- UPDATING SECTION 195 WITH NON_DTAA RATES //-- 2026/01/15 //-- ######################
                    if (J_Var.J_pDatabaseType == J_DatabaseType.SqlServer)
                        strSQL = @"UPDATE " + strShortDeduction27QTable + @"
                              SET SCHEDULED_RATE = PAR_NON_DTAA_RATE.NON_DTAA_RATE
                              FROM " + strShortDeduction27QTable + @"
                              INNER JOIN MST_DEDUCTEE 
			                        ON MST_DEDUCTEE.DEDUCTEE_ID = " + strShortDeduction27QTable + @".PARTY_ID
                              INNER JOIN PAR_NON_DTAA_RATE 
			                        ON PAR_NON_DTAA_RATE.REMITTANCE_ID = " + strShortDeduction27QTable + @".REMITTANCE_ID  
                              WHERE " + strShortDeduction27QTable + @".PAYMENT_DATE BETWEEN PAR_NON_DTAA_RATE.FROM_DATE AND PAR_NON_DTAA_RATE.TO_DATE 
                              AND   " + strShortDeduction27QTable + @".TDS_APPLICABILITY_ID = 1 
                        ...
                        strSQL = @"UPDATE ((" + strShortDeduction27QTable + @" 
                                        ...
                                        INNER JOIN PAR_NON_DTAA_RATE 
                                            ON PAR_NON_DTAA_RATE.REMITTANCE_ID = " + strShortDeduction27QTable + @".REMITTANCE_ID)
                                    SET " + strShortDeduction27QTable + @".SCHEDULED_RATE = PAR_NON_DTAA_RATE.NON_DTAA_RATE
                                    WHERE " + strShortDeduction27QTable + @".PAYMENT_DATE BETWEEN PAR_NON_DTAA_RATE.FROM_DATE AND PAR_NON_DTAA_RATE.TO_DATE
                                    AND   " + strShortDeduction27QTable + @".TDS_APPLICABILITY_ID = 1 
                                    AND   MST_DEDUCTEE.DEDUCTEE_PAN <> 'PANNOTAVBL'"; 
                    dmlService.J_ExecSql(strSQL);
                    //-- ######################
                    //-- Update DTAA_RATE by matching DTAA Rate
                    if (J_Var.J_pDatabaseType == J_DatabaseType.SqlServer)
                        strSQL = @"UPDATE " + strShortDeduction27QTable + @"
                              SET DTAA_RATE = ISNULL(PAR_DTAA_RATE.DTAA_RATE,0)
                              FROM " + strShortDeduction27QTable + @"
                              INNER JOIN MST_SECTION 
				                    ON MST_SECTION.SECTION_ID = " + strShortDeduction27QTable + @".SECTION_ID
                              LEFT JOIN PAR_DTAA_RATE
                                   ON  " + strShortDeduction27QTable + @".COUNTRY_ID    = PAR_DTAA_RATE.COUNTRY_ID
                                  AND " + strShortDeduction27QTable + @".REMITTANCE_ID = PAR_DTAA_RATE.REMITTANCE_ID
                              WHERE " + strShortDeduction27QTable + @".SECTION_ID = 30 
                              AND " + strShortDeduction27QTable + @".TDS_APPLICABILITY_ID = 2
                             AND " + strShortDeduction27QTable + @".PAYMENT_DATE BETWEEN PAR_DTAA_RATE.FROM_DATE AND PAR_DTAA_RATE.TO_DATE";
                    else
                        strSQL = @"UPDATE (" + strShortDeduction27QTable + @" 
                                    INNER JOIN MST_SECTION 
                                        ON MST_SECTION.SECTION_ID = " + strShortDeduction27QTable + @".SECTION_ID)
                                    LEFT JOIN PAR_DTAA_RATE
                                        ON " + strShortDeduction27QTable + @".COUNTRY_ID = PAR_DTAA_RATE.COUNTRY_ID
                                       AND " + strShortDeduction27QTable + @".REMITTANCE_ID = PAR_DTAA_RATE.REMITTANCE_ID
                                SET " + strShortDeduction27QTable + @".DTAA_RATE = IIF(ISNULL(PAR_DTAA_RATE.DTAA_RATE), 0, PAR_DTAA_RATE.DTAA_RATE)                                    
                                WHERE " + strShortDeduction27QTable + @".SECTION_ID = 30
                                  AND " + strShortDeduction27QTable + @".TDS_APPLICABILITY_ID = 2
                                  AND " + strShortDeduction27QTable + @".PAYMENT_DATE 
                                        BETWEEN PAR_DTAA_RATE.[FROM_DATE] AND PAR_DTAA_RATE.[TO_DATE]";
                    dmlService.J_ExecSql(strSQL);
                    //-- Update SCHEDULED_RATE with DTAA Rate if DTAA Rate found
                    strSQL = @"UPDATE " + strShortDeduction27QTable + @"
                                 SET SCHEDULED_RATE = DTAA_RATE
                                 WHERE SECTION_ID = 30
				                 AND DTAA_RATE > 0";
                    dmlService.J_ExecSql(strSQL);
                    //-- Update SCHEDULED TAX AMOUNT
                    strSQL = @"UPDATE " + strShortDeduction27QTable + @"
                              SET SCHEDULED_TAX_AMOUNT = ROUND(PAYMENT_AMOUNT * SCHEDULED_RATE /100,0)
                              WHERE DEDUCTED_DATE IS NOT NULL";
                    dmlService.J_ExecSql(strSQL);
                    //-- Delete Records without Short Deduction 
                    if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2026_27ID)
                        strSQL = @"DELETE FROM  " + strShortDeduction27QTable + @"
                               WHERE TOTAL_AMOUNT >= SCHEDULED_TAX_AMOUNT ";
                    else
                        strSQL = @"DELETE FROM  " + strShortDeduction27QTable + @"
                               WHERE TAX_AMOUNT >= SCHEDULED_TAX_AMOUNT ";
                    dmlService.J_ExecSql(strSQL);
                    //--
                    strSQL = @"SELECT COUNT(*) FROM " + strShortDeduction27QTable;
                ...
                else if (cmbFormNo.Text != T_FormNo.F27Q)
                {
                    ...
                    else //-- 26Q & 27EQ
                    {
                        if (J_Var.J_pDatabaseType == J_DatabaseType.SqlServer)
                        {
                            ...
                            //                       @"OR " + strTableName + "." + strFieldPAN + " = 'PANINVALID'  " +
                            //                       @"OR MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2)  " +
                            //                       @"THEN ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE ) / 100, " + strShortDeductionsRoundOffValue + @")
                            //                   ELSE  CASE
                            ...
                            //                                                 mst_nonsalary_tax_slab.COMPLIANCE_HIGHER_RATE ) / 100, " + strShortDeductionsRoundOffValue + @")
                            //                   ELSE CASE WHEN SUBSTRING(" + strTableName + "." + strFieldPAN + ", 4, 1) = 'P' OR SUBSTRING(" + strTableName + "." + strFieldPAN + ", 4, 1) = 'H'  " +
                            //                       @" THEN ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.NON_COMPANY_RATE ) / 100," + strShortDeductionsRoundOffValue + @")
                            //                   ELSE ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.COMPANY_RATE ) / 100," + strShortDeductionsRoundOffValue + @") END END END";
                            strActualAmtCreation = @"CASE WHEN " + strTableName + "." + strFieldPAN + " = 'PANNOTAVBL' " +
                                                   @"OR " + strTableName + "." + strFieldPAN + " = 'PANAPPLIED' " +
                                                   @"OR " + strTableName + "." + strFieldPAN + " = 'PANINVALID' " +
                                                   @"OR MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2) " +
                                                   @"THEN (TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE) / 100.0
                                                   ELSE CASE
                                                       WHEN " + strTableName + @".COMPLIANCE_HIGHER_RATE_FLAG = 1
                                                       AND TRN_DEDUCTEE_DETAILS.PAYMENT_DATE >= '07/01/2021'
                                                       THEN (TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.COMPLIANCE_HIGHER_RATE) / 100.0
                                                   ELSE CASE
                                                       WHEN SUBSTRING(" + strTableName + "." + strFieldPAN + @", 4, 1) = 'P'
                                                       OR SUBSTRING(" + strTableName + "." + strFieldPAN + @", 4, 1) = 'H'
                                                       THEN (TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.NON_COMPANY_RATE) / 100.0
                                                       ELSE (TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.COMPANY_RATE) / 100.0
                                                   END END END";
                        }
                        else if (J_Var.J_pDatabaseType == J_DatabaseType.MsAccess)
                        {
                            if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2026_27ID)
                            {
                                ...
                                strActualAmtCreation = @"IIF(" + strTableName + "." + strFieldPAN + " = 'PANNOTAVBL' " +
                                                       ...
                                                       @"OR " + strTableName + "." + strFieldPAN + " = 'PANINVALID' " +
                                                       @"OR MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2), " +
                                                       @"(TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE) / 100, " +
                                                       @"IIF(" + strTableName + @".COMPLIANCE_HIGHER_RATE_FLAG = 1 " +
                                                       @"AND TRN_DEDUCTEE_DETAILS.PAYMENT_DATE >= " + cmnService.J_DateOperator() + "07/01/2021" + cmnService.J_DateOperator() + @", " +
                                                       @"(TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.COMPLIANCE_HIGHER_RATE) / 100, " +
                                                       @"IIF(MID(" + strTableName + "." + strFieldPAN + @", 4, 1) = 'P' " +
                                                       @"OR MID(" + strTableName + "." + strFieldPAN + @", 4, 1) = 'H', " +
                                                       @"(TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.NON_COMPANY_RATE) / 100, " +
                                                       @"(TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.COMPANY_RATE) / 100" +
                                                       @")))";
                            }
                            else
                            {
                                ...
                                strActualAmtCreation = @"IIF(" + strTableName + "." + strFieldPAN + " = 'PANNOTAVBL' " +
                                                       ...
                                                       @"(TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE) / 100, " +
                                                       @"IIF(" + strTableName + @".COMPLIANCE_HIGHER_RATE_FLAG = 1 " +
                                                       @"AND TRN_DEDUCTEE_DETAILS.PAYMENT_DATE >= " +
                                                       cmnService.J_DateOperator() + "07/01/2021" + cmnService.J_DateOperator() + @", " +
                                                       @"(TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.COMPLIANCE_HIGHER_RATE) / 100, " +
                                                       @"IIF(MID(" + strTableName + "." + strFieldPAN + @", 4, 1) = 'P' " +
                                                       @"OR MID(" + strTableName + "." + strFieldPAN + @", 4, 1) = 'H', " +
                                                       @"(TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.NON_COMPANY_RATE) / 100, " +
                        ...
                        //                       @"OR " + strTableName + "." + strFieldPAN + " = 'PANINVALID' " +
                        //                       @"OR MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2), " +
                        //                       @"ROUND(((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE ) / 100)," + strShortDeductionsRoundOffValue + @"), 
                        //                   iif(" + strTableName + @".COMPLIANCE_HIGHER_RATE_FLAG = 1 AND TRN_DEDUCTEE_DETAILS.PAYMENT_DATE >= " + cmnService.J_DateOperator() + "07/01/2021" + cmnService.J_DateOperator() + @", Round(( trn_deductee_details.payment_amount * mst_nonsalary_tax_slab.COMPLIANCE_HIGHER_RATE) / 100, " + strShortDeductionsRoundOffValue + @"),
                        //                    IIF(MID(" + strTableName + "." + strFieldPAN + ", 4, 1) = 'P' OR MID(" + strTableName + "." + strFieldPAN + ", 4, 1) = 'H', " +
                        //                       @"ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.NON_COMPANY_RATE ) / 100," + strShortDeductionsRoundOffValue + @"), 
                        //                    ROUND((TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * MST_NONSALARY_TAX_SLAB.COMPANY_RATE ) / 100," + strShortDeductionsRoundOffValue + @"))))";
                        //
                        //strSQL = @"SELECT COUNT(*) 
                        //           FROM   TRN_DEDUCTEE_DETAILS, 
                        //                  " + strTableName + "," +
...
//                         @"AND    TRN_CHALLAN.DEPOSIT_DATE        > " + strLastDateCreation + " " +
                ...
                if (lngLatePaymentsNos == 0)
                {
                    ...
                    btnViewLatePayments.BackColor = Color.LightGray;
                    //
                    btnViewLatePayments.Visible = false;
                    //-- 2023/02/20
                    lblLatePaymentsNos.Text = "";  
                    if (File.Exists(Path.Combine(Application.StartupPath, strSmilyImage)) == true)
                        lblLatePaymentsNos.Image = new Bitmap(Path.Combine(Application.StartupPath, strSmilyImage));
                    lblLPTick.Text = "\u2714";
                    lblLPTick.ForeColor = Color.Green;
                    lblLPTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                }
                else
                {
                    ...
                    //
                    btnViewLatePayments.Visible = true;
                    //
                    lblLatePaymentsNos.Image = null;
                    lblLatePaymentsNos.Text = Convert.ToString(lngLatePaymentsNos) + " nos.";
                    lblLPTick.Text = "\u0058";
                    lblLPTick.ForeColor = Color.Red;
                    lblLPTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                }
                ...
                if (lngLateDeductions == 0)
                {
                    ...
                    //
                    btnViewLateDeductions.Visible = false;
                    //-- 2023/02/20
                    lblLateDeductionsNos.Text = "";
                    if (File.Exists(Path.Combine(Application.StartupPath, strSmilyImage)) == true)
                        lblLateDeductionsNos.Image = new Bitmap(Path.Combine(Application.StartupPath, strSmilyImage));
                    //
                    lblLDTick.Text = "\u2714";
                    lblLDTick.ForeColor = Color.Green;
                    lblLDTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                }
                else
                {
                    ...
                    //
                    btnViewLateDeductions.Visible = true;
                    //
                    lblLateDeductionsNos.Image = null;
                    lblLateDeductionsNos.Text = Convert.ToString(lngLateDeductions) + " nos.";
                    //
                    lblLDTick.Text = "\u0058";
                    lblLDTick.ForeColor = Color.Red;
                    lblLDTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                }
                ////
                //lblLateDeductionsNos.Text = Convert.ToString(lngLateDeductions) + " nos.";
                ...
                if (cmnService.J_ReturnDoubleValue(lblInterestAmountReported.Text) > cmnService.J_ReturnDoubleValue(lblTotalInterest.Text))
                    ...
                    lblInterestPayable.Text = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(lblTotalInterest.Text) - cmnService.J_ReturnDoubleValue(lblInterestAmountReported.Text));
                if (lblLatePaymentsNos.Text != "")
                {
                    if(cmnService.J_ReturnDoubleValue(lblInterestPayable.Text) == 0)
                    {
                        lblLPTick.Text = "\u2714";
                        lblLPTick.ForeColor = Color.Green;
                        lblLPTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                    ...
                    //
                    lblLatePaymentsNos.Text = lblLatePaymentsNos.Text + "\nPayable : " + string.Format("{0:0.00}", lblInterestPayable.Text);// TotalInterestLP);// lblInterestPayable.Text;
                }
                //-- 2024/05/14
                if(lblLateDeductionsNos.Text != "")
                {
                    if (cmnService.J_ReturnDoubleValue(lblInterestPayableLD.Text) == 0)
                    {
                        lblLDTick.Text = "\u2714";
                        lblLDTick.ForeColor = Color.Green;
                        lblLDTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                    ...
                    //
                    lblLateDeductionsNos.Text = lblLateDeductionsNos.Text + "\nPayable : " + string.Format("{0:0.00}", lblInterestPayableLD.Text);
                }
                //-- 2023/10/10
                ////if (lblShortDeductionsNos.Text != "")
                ////{
                ////    lblShortDeductionsNos.Text = lblShortDeductionsNos.Text + "\nTotal : " + lblInterestPayableLD.Text;// lblTotalShortPayments.Text;
                ////}
                //--
                //--
                if (lngShortDeductionsNos > 0)
                ...
                //--
                //@@@@@@@@@@ 2024/07/22
                if (lblFilingDateLabel.Visible == true) //-- 2025/05/07
                {
                    mskFilingDate.Text = J_ReturnServerDate();
                    btnViewLateFine_Click(sender, e);
                }
                //@@@@@@@@@@
                //--
            }
        ...
        private void LoadDetailsCorr(long BasicInfoID)
        {
            ...
            long lngFAYearId = 0;
            long lngShortDeductionsNos = 0;
            string strFAYear = "", strTAN = "", strCompany = "";
            try
            {
                //--
                if (BasicInfoID == 0) return;
                ...
                //
                //--
                if (Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COMPANY_NAME FROM COR_HDR_COMPANY WHERE BATCH_HEADER_ID = " + BasicInfoID)).Length > 47)
                    lblValueTAN.Text = Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT TAN_NO FROM COR_HDR_COMPANY WHERE BATCH_HEADER_ID = " + BasicInfoID)) + " [ " + cmnService.J_Left(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COMPANY_NAME FROM COR_HDR_COMPANY WHERE BATCH_HEADER_ID = " + BasicInfoID)), 47) + " ]";
                else
                    lblValueTAN.Text = Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT TAN_NO FROM COR_HDR_COMPANY WHERE BATCH_HEADER_ID = " + BasicInfoID)) + " [ " + Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT COMPANY_NAME FROM COR_HDR_COMPANY WHERE BATCH_HEADER_ID = " + BasicInfoID)) + " ]";
                //
                lblValueFAYear.Text = Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT MST_ASSESSMENT.FA_YEAR FROM COR_HDR_BATCH, MST_ASSESSMENT WHERE COR_HDR_BATCH.ASST_ID = MST_ASSESSMENT.ASST_ID  AND BATCH_HEADER_ID = " + BasicInfoID));
                lblValueQtr.Text = strQtr;
                lblValueFormNo.Text = strFormNo;
                lblValueFormNo.Text = lblValueFormNo.Text + " :: " + Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT IMPORTED_DATE FROM COR_HDR_BATCH WHERE BATCH_HEADER_ID = " + BasicInfoID));
                lblValueFormNoDisplay.Text = TdsMan.GetFormNoIT2025(lblValueFormNo.Text) + " (" + lblValueFormNo.Text + ")" + " :: " + Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT IMPORTED_DATE FROM COR_HDR_BATCH WHERE BATCH_HEADER_ID = " + BasicInfoID));

                lblFDTick.Visible = false;
                //--
                //--
                #region SUMMARY SHORT DEDUCTIONS
                //--
                    ...
                    strSQL = @"INSERT INTO  " + TDSMAN.Classes.TDSMAN.T_tblTEMP_PREDICT_SHORT_DED_CORR_SD + @" 
                               ...
                               SELECT      BATCH_HEADER_ID           AS BATCH_HEADER_ID, 
                                          " + lngFAYearId + @"       AS ASST_ID,    
                                           ...
                                           TOTAL_TAX_DEDUCTED_AMOUNT + PREVIOUS_TAX_DEDUCTED_TOTAL + SUPER_ANN_TAX + TDS_US_192_2B_AMOUNT AS TDS_ENTERED,                                           
                ...
                else if (strFormNo != T_FormNo.F27Q)
                {
                    ...
                    if (strFormNo == T_FormNo.F24Q)
                    {
                        if (J_Var.J_pDatabaseType == J_DatabaseType.SqlServer)
                            strActualAmtCreation = @"CASE WHEN   COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN = 'PANNOTAVBL' " +
                                                               ...
                                                               @"OR COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN = 'PANINVALID' " +
                                                               @"OR MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2)  " +
                                                        @"THEN ROUND((COR_TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * 20 ) / 100,0) END";
                        else if (J_Var.J_pDatabaseType == J_DatabaseType.MsAccess)
                            strActualAmtCreation = @"IIF(COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN = 'PANNOTAVBL' " +
                                                   @"OR COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN  = 'PANAPPLIED' " +
                                                   @"OR COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN  = 'PANINVALID' " +
                                                   @"OR MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2), " +
                                                   @"ROUND(VAL(FORMAT((COR_TRN_DEDUCTEE_DETAILS.PAYMENT_AMOUNT * 20) / 100, '0')),0))";
                        //--
                        strSQL = @"SELECT COUNT(*) 
                           FROM   (COR_TRN_DEDUCTEE_DETAILS 
                          LEFT JOIN MST_VERIFIED_PAN
                                  ON  COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN = MST_VERIFIED_PAN.PAN)
                           WHERE  COR_TRN_DEDUCTEE_DETAILS.BATCH_HEADER_ID = " + BasicInfoID + " " +
                    ...
                    else
                    {
                        ...
                        strSQL = @"SELECT COUNT(*) 
                           FROM   ((COR_TRN_DEDUCTEE_DETAILS 
                           INNER JOIN MST_NONSALARY_TAX_SLAB 
                                  ON  COR_TRN_DEDUCTEE_DETAILS.SECTION_ID = MST_NONSALARY_TAX_SLAB.SECTION_ID)
                          LEFT JOIN MST_VERIFIED_PAN
                                  ON  COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN = MST_VERIFIED_PAN.PAN)
                           WHERE  COR_TRN_DEDUCTEE_DETAILS.PAYMENT_DATE BETWEEN MST_NONSALARY_TAX_SLAB.FROM_DATE AND MST_NONSALARY_TAX_SLAB.TO_DATE
                           AND    COR_TRN_DEDUCTEE_DETAILS.BATCH_HEADER_ID = " + BasicInfoID + " " +
                         @"AND    COR_TRN_DEDUCTEE_DETAILS.TAX_DEPOSITED_AMOUNT < " + strActualAmtCreation + " " +
                         @"AND    (MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2) OR COR_TRN_DEDUCTEE_DETAILS.REASON_ID IN (3,7,15,4,12,17,20))";
                    }
...
//        }
        ...
        public void PopulateLatePayments(long BasicInfoID, string TableName, string FieldId, string FieldPAN, string FieldName, string Argumnets, string SearchConditions, string FormNo)
        {
            ...
            try
            {
                if (FormNo == T_FormNo.F27EQ)
                {
                    ...
                    strInterest = "1%";
                    //-- 2025/05/07
                    if(Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2025_26ID)
                    {
                        dblInterest = 0.015;
                        strInterest = "1.5%";
                    }
                ...
                strOrderBy = " ORDER  BY TRN_CHALLAN.SL_NO, TRN_DEDUCTEE_DETAILS.SL_NO ";
                //--
                string[,] strMatrixViewLatePayments = {{"Chl sl",   "30", "", "", "", "", "T"},
                                                      {"Ded sl",  "30", "", "", "", "", "T"},
                                                      {"PAN",          "100", "", "", "", "", "T"},
                                                      {"Name","230", "S", "", "", "", "T"},
                                                      {"Amount",     "80", "0.00", "R", "", "", "T"},
                                                      {"TDS Deposited","70", "0.00", "R", "", "", "T"},
                                                      {"Ded Date","50", "d", "", "", "", "T"},
                                                      {"Due Date","50", "d", "", "", "", "T"},
                                                      {"Deposit Date","60", "d", "", "", "", "T"},
                                                      {"Delay in Month","80", "", "R", "", "", "T"},
                                                      {"Interest @" + strInterest,"60", "0.00", "R", "", "", "T"}};
                if (dsetLatePayments != null) dsetLatePayments.Clear();
                dsetLatePayments = dmlService.J_ShowDataInGrid(dmlService.J_pCommand, ref dgvLatePayments, strSQL + SearchConditions + strOrderBy, strMatrixViewLatePayments);
                //--
                double TotalInterest = 0; string InterestAmountReported = "";
                ...
                //
                //if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2026_27ID)
                //    strSQL = "SELECT SUM(INTEREST) FROM TRN_CHALLAN, TRN_BASIC_INFO WHERE TRN_CHALLAN.BASIC_INFO_ID = TRN_BASIC_INFO.BASIC_INFO_ID AND TRN_BASIC_INFO.BASIC_INFO_ID = " + BasicInfoID + " ";
                //else
                    strSQL = "SELECT SUM(INTEREST_ALLOCATED) FROM TRN_CHALLAN, TRN_BASIC_INFO WHERE TRN_CHALLAN.BASIC_INFO_ID = TRN_BASIC_INFO.BASIC_INFO_ID AND TRN_BASIC_INFO.BASIC_INFO_ID = " + BasicInfoID + " ";
                lblInterestAmountReported.Text = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL))));
                //lblInterestAmountReported.Text = string.Format("{0:0.00}", Convert.ToDouble(Convert.ToString(InterestAmountReported) == "" ? "0" : Convert.ToString(InterestAmountReported))); 
                ...
                //lblTotalLabelLatePayments.Text = "Total Interest :";
                //if (lblLatePaymentsNos.Text != "")
                //    lblLatePaymentsNos.Text = lblLatePaymentsNos.Text + "\nPayable : " + lblInterestPayable.Text;
            }
            catch (Exception err)
            {
                cmnService.J_UserMessage(err.Message);
        ...
        public void PopulateShortDeductions(long BasicInfoID, string TableName, string FieldId, string FieldPAN,  string FieldName, string Argumnets, string SearchConditions)
        {
            ...
            string iTDSRate = "0", iTDStobeDeducted = "0", iShortPayment = "0",  iV = "";
            try
            {
                if (cmbFormNo.Text != T_FormNo.F27Q)
                {
                    ...
                    {
                        #region 26Q - 27EQ 
                        iTDSRate = "70"; iTDStobeDeducted = "80"; iShortPayment = "70"; iV = "";
                        if (J_Var.J_pDatabaseType == J_DatabaseType.SqlServer)
                        {
                            #region COMMENT
                            //strSQL = @"SELECT TRN_CHALLAN.SL_NO                                        AS CHALLAN_SL_NO,
                    ...
                    strOrderBy = " ORDER  BY TRN_CHALLAN.SL_NO, TRN_DEDUCTEE_DETAILS.SL_NO ";
                    //--
                    string[,] strMatrixViewShortDeductions = {{"Chl Sl",   "30", "", "", "", "", "T"},
                                                  {"Ded Sl",  "30", "", "", "", "", "T"},
                                                  {"PAN",          "90", "", "", "", "", "T"},
                                                  {"Ded Name","230", "S", "", "", "", "T"},
                                                  {"Section",      "45", "", "", "", "", "T"},
                                                  ...
                                                  {"Amount",     "90", "0.00", "R", "", "", "T"},
                                                  {"TDS Deposited","70", "0.00", "R", "", "", "T"},
                                                  {"TDS Rate", iTDSRate , "0.0000", "R", "", iV, "T"},
                                                  {"TDS to be Deducted", iTDStobeDeducted , "0.00", "R", "", iV, "T"},
                                                  {"Short Payment", iShortPayment , "0.00", "R", "", iV, "T"},
                                                  {"COMPLIANCE_HIGHER_RATE_FLAG","0", "", "", "", "F", ""},
                                                  {"VERIFIED_PAN_STATUS","0", "", "", "", "F", ""} };
                    ...
                    if (cmbFormNo.Text == T_FormNo.F24Q)
                    {
                        lblTotalShortPaymentsCaption.Visible = false;
                        lblTotalShortPayments.Visible = false;
                        //lblShortDeductionsNos.Visible = false;
                        lblPANMarkedYellow.Visible = true;
                    }
                    else
                    {
                        lblPANMarkedYellow.Visible = false;
                        ...
                        //if(lblShortDeductionsNos.Text != "\u2714")
                        //if(lblShortDeductionsNos.Text != "")
                        //    lblShortDeductionsNos.Text = lblShortDeductionsNos.Text + "\nTotal : " + lblTotalShortPayments.Text;
                        //-- 2023/10/10
                        if (TotalShortPayments > 0)
                            lblShortDeductionsNos.Text = lblShortDeductionsNos.Text + "\nTotal : " + lblTotalShortPayments.Text;
                    }
                    #endregion
                }
                else //-- 27Q - DTAA
                {
                    ...
                    {
                        strSQL = @"SELECT TRN_CHALLAN.SL_NO                                        AS CHALLAN_SL_NO,
                                      " + strShortDeduction27QTable + @".SL_NO                     AS DEDUCTEE_SL_NO," +
                                          TableName + "." + FieldPAN + ", " +
                                          TableName + "." + FieldName + ", " +
                                        @"MST_SECTION.SECTION_NO,
                                      " + strShortDeduction27QTable + @".PAYMENT_DATE, 
                                      " + strShortDeduction27QTable + @".PAYMENT_AMOUNT,
                                      " + strShortDeduction27QTable + @".TAX_DEPOSITED_AMOUNT, 
                                     SCHEDULED_RATE AS RATE," +
                                    "SCHEDULED_TAX_AMOUNT                         AS ACTUAL_TAX_AMOUNT," +
                                    "SCHEDULED_TAX_AMOUNT - TAX_DEPOSITED_AMOUNT  AS SHORT_PAYMENT," +
                                    TableName + ".COMPLIANCE_HIGHER_RATE_FLAG  AS COMPLIANCE_HIGHER_RATE_FLAG," +
                                    "MST_VERIFIED_PAN.VERIFIED_STATUS AS VERIFIED_PAN_STATUS " +
                                 @"FROM  (((((" + strShortDeduction27QTable + @" 
                                   INNER JOIN " + TableName + @"
                                        ON " + strShortDeduction27QTable + @".PARTY_ID = " + TableName + "." + FieldId + @") 
                                   INNER JOIN TRN_CHALLAN
                                        ON  " + strShortDeduction27QTable + @".CHALLAN_ID = TRN_CHALLAN.CHALLAN_ID)
                                   INNER JOIN MST_NONSALARY_TAX_SLAB
                                        ON  " + strShortDeduction27QTable + @".SECTION_ID = MST_NONSALARY_TAX_SLAB.SECTION_ID)  
                                   INNER JOIN MST_SECTION
                                        ON " + strShortDeduction27QTable + @".SECTION_ID  = MST_SECTION.SECTION_ID ) 
                                   LEFT JOIN MST_VERIFIED_PAN
                                        ON  " + TableName + "." + FieldPAN + @" = MST_VERIFIED_PAN.PAN)
                               WHERE  " + strShortDeduction27QTable + @".PAYMENT_DATE BETWEEN MST_NONSALARY_TAX_SLAB.FROM_DATE AND MST_NONSALARY_TAX_SLAB.TO_DATE  
                               AND    " + strShortDeduction27QTable + @".PAYMENT_DATE IS NOT NULL 
                               AND    " + strShortDeduction27QTable + @".BASIC_INFO_ID    = " + BasicInfoID + " ";// +
                         //@"AND   (MST_VERIFIED_PAN.VERIFIED_STATUS IN(1, 2) OR  " + strShortDeduction27QTable + @".REASON_ID IN (3,7,15,4,12,17,20,13))";
                    }
                    else if (J_Var.J_pDatabaseType == J_DatabaseType.MsAccess)
                    {
                        strSQL = @"SELECT TRN_CHALLAN.SL_NO                                        AS CHALLAN_SL_NO,
                    ...
                    strOrderBy = " ORDER  BY TRN_CHALLAN.SL_NO, " + strShortDeduction27QTable + @".SL_NO ";
                    //--
                    string[,] strMatrixViewShortDeductions = {{"Chl Sl",   "30", "", "", "", "", "T"},
                                                  {"Ded Sl",  "30", "", "", "", "", "T"},
                                                  {"PAN",          "90", "", "", "", "", "T"},
        ...
        public void PopulateShortDeductionsSD( long BasicInfoID, string SearchConditions)
        {
            ...
            try
            {
                strSQL = @"SELECT SL_NO,
                                  ...
                                  RELIEF_AMOUNT,
                                  TDS_ENTERED,
                                  SHORT_DEDUCTION_AMOUNT, VERIFIED_PAN_STATUS
                           FROM   " + TDSMAN.Classes.TDSMAN.T_tblTEMP_PREDICT_SHORT_DED_SD + @"
                           WHERE  SHORT_DEDUCTION_AMOUNT > 0";
                //--
                strOrderBy = " ORDER BY SL_NO";
                //--
                string[,] strMatrixViewShortDeductionsSD = {{"Sl No.",   "30", "", "", "", "", "T"},
                                                          {"PAN",          "30", "", "", "", "", "T"},
                                                          {"Employee Name","200", "S", "", "", "", "T"},
                                                          {"Category",      "45", "", "", "", "", ""},
                                                          {"Current Employer's Salary",     "90", "0.00", "R", "", "", "T"},
                                                          {"Taxable Income as per Salary","80", "0.00", "R", "", "", "T"},
                                                          {"Total Tax as per Salary",     "70", "0.00", "R", "", "", "T"},
                                                          {"Relief u/s 89","80", "0.00", "R", "", "", "T"},
                                                          {"Current Employer's TDS","80", "0.00", "R", "", "", "T"},
                                                          {"Short Payment","70", "0.00", "R", "", "", "T"},
                                                          {"VERIFIED_PAN_STATUS","0", "", "", "", "F", ""} };
                ...
                //--
                if (dgvShortDeductionsSD.RowCount > 0)
                    lblShortDeductionsNos.Text = lblShortDeductionsNos.Text + "\nTotal : " + lblTotalShortPaymentsSD.Text;
                //--
            }
            catch (Exception err)
            {
        ...
        public void PopulateLateDeductions(long BasicInfoID, string TableName, string FieldId, string FieldPAN, string FieldName, string SearchConditions)
        {
            ...
            try
            {
                ...
                strOrderBy = " ORDER  BY TRN_CHALLAN.SL_NO, TRN_DEDUCTEE_DETAILS.SL_NO ";
                //--
                string[,] strMatrixViewLateDeductions = {{"Chl Sl",   "30", "", "", "", "", "T"},
                                                      {"Ded Sl",  "30", "", "", "", "", "T"},
                                                      {"PAN",          "100", "", "", "", "", "T"},
                                                      {"Name","200", "S", "", "", "", "T"},
                                                      {"Amount",     "80", "0.00", "R", "", "", "T"},
                ...
                //lblTotalLabelLateDeductions.Text = "Total Interest :";
                //if (lblLateDeductionsNos.Text != "")
                //    lblLateDeductionsNos.Text = lblLateDeductionsNos.Text + "\nPayable : " + lblTotalInterestLD.Text;//-- 2023/07/25 //--lblTotalShortPayments.Text;
            }
            catch (Exception err)
            {
                cmnService.J_UserMessage(err.Message);
...
//        }
        ...
        public void PopulateLatePaymentsCorr(long BasicInfoID, string Argumnets, string SearchConditions, string FormNo)
        {
            ...
            try
            {
                ...
                lblTotalInterest.Text = string.Format("{0:0.00}", Convert.ToDouble(Convert.ToString(TotalInterest) == "" ? "0" : Convert.ToString(TotalInterest)));
                //
                long lngFAYearId = cmnService.J_ReturnInt64Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT ASST_ID     FROM COR_HDR_BATCH WHERE BATCH_HEADER_ID = " + BasicInfoID)));
                //if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2026_27ID)
                //if (lngFAYearId >= T_FinancialYearID.F2026_27ID)
                //        strSQL = "SELECT SUM(INTEREST) FROM COR_TRN_CHALLAN  WHERE COR_TRN_CHALLAN.BATCH_HEADER_ID = " + BasicInfoID + " ";
                //else
                    strSQL = "SELECT SUM(INTEREST_ALLOCATED) FROM COR_TRN_CHALLAN  WHERE COR_TRN_CHALLAN.BATCH_HEADER_ID = " + BasicInfoID + " ";
                //
                lblInterestAmountReported.Text = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL))));
        ...
        public void PopulateShortDeductionsCorr(long BasicInfoID, string Argumnets, string SearchConditions)
        {
            ...
            try
            {
                string strTDSRateGridWidth = "70", strTDSToBeDeductedGridWidth = "80", strShortPaymentGridWidth = "70";
                //---------------------------------------------------------
                ...
                if (Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT FORM_NO FROM COR_HDR_BATCH WHERE BATCH_HEADER_ID = " + BasicInfoID)) != T_FormNo.F27Q)
                {
                    ...
                    #endregion
                    if (Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT FORM_NO FROM COR_HDR_BATCH WHERE BATCH_HEADER_ID = " + BasicInfoID)) == T_FormNo.F24Q)
                    {
                        strTDSRateGridWidth = "0"; strTDSToBeDeductedGridWidth = "0"; strShortPaymentGridWidth = "0";
                        if (J_Var.J_pDatabaseType == J_DatabaseType.MsAccess)
                        {
                            strSQL = @"SELECT COR_TRN_CHALLAN.SL_NO                                        AS CHALLAN_SL_NO,
                                      COR_TRN_DEDUCTEE_DETAILS.SL_NO                               AS DEDUCTEE_SL_NO,
                                     ...
                                     @"FROM   (((COR_TRN_DEDUCTEE_DETAILS 
                                      ...
                                      LEFT JOIN MST_VERIFIED_PAN
                                        ON  COR_TRN_DEDUCTEE_DETAILS.DEDUCTEE_PAN  = MST_VERIFIED_PAN.PAN)                                
                               WHERE  COR_TRN_DEDUCTEE_DETAILS.PAYMENT_DATE IS NOT NULL 
                               AND    COR_TRN_DEDUCTEE_DETAILS.BATCH_HEADER_ID = " + BasicInfoID + " " +
                             @"AND    COR_TRN_DEDUCTEE_DETAILS.TAX_DEPOSITED_AMOUNT < " + Argumnets + " ";
                        }
                        else if (J_Var.J_pDatabaseType == J_DatabaseType.SqlServer)
                        ...
                        if (J_Var.J_pDatabaseType == J_DatabaseType.MsAccess)
                        {
                            strSQL = @"SELECT COR_TRN_CHALLAN.SL_NO                                        AS CHALLAN_SL_NO,
                                      ...
                                      MST_NONSALARY_TAX_SLAB.INVALID_PAN_RATE,  
                                              ...
                                              Argumnets + " - COR_TRN_DEDUCTEE_DETAILS.TAX_DEPOSITED_AMOUNT  AS SHORT_PAYMENT," +
                                              "MST_VERIFIED_PAN.VERIFIED_STATUS AS VERIFIED_PAN_STATUS " +
                                     @"FROM   ((((COR_TRN_DEDUCTEE_DETAILS INNER JOIN 
                                          MST_NONSALARY_TAX_SLAB
                                        ON COR_TRN_DEDUCTEE_DETAILS.SECTION_ID     = MST_NONSALARY_TAX_SLAB.SECTION_ID)
                                      INNER JOIN COR_TRN_CHALLAN
                                        ON COR_TRN_DEDUCTEE_DETAILS.TRN_CHALLAN_ID = COR_TRN_CHALLAN.TRN_CHALLAN_ID )
                    ...
                    string[,] strMatrixViewShortDeductions = {{"Chl Sl",   "30", "", "", "", "", "T"},
                                                  ...
                                                  {"Payment Date", "70", "d", "", "", "", "T"},
                                                  {"Amount",     "90", "0.00", "R", "", "", "T"},
                                                  {"TDS Deposited","80", "0.00", "R", "", "", "T"},
                                                  {"TDS Rate",     strTDSRateGridWidth, "0.0000", "R", "", "", "T"},
                                                  {"TDS To Be Deducted",strTDSToBeDeductedGridWidth, "0.00", "R", "", "", "T"},
                                                  {"Short Payment",strShortPaymentGridWidth, "0.00", "R", "", "", "T"},
                                                  {"VERIFIED_PAN_STATUS","0", "", "", "", "F", ""}};
                    if (dsetShortDeductions != null) dsetShortDeductions.Clear();
                    dsetShortDeductions = dmlService.J_ShowDataInGrid(dmlService.J_pCommand, ref dgvShortDeductions, strSQL + SearchConditions + strOrderBy, strMatrixViewShortDeductions);
                    //--
        ...
        public void PopulateLateFilingDetailsRegular(long BasicInfoID)
        {
            ...
            try
            {
                ...
                //{
                    ...
                    string strReturnLastDate = Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL));
                //-- 2025/05/07
                strSQL = "SELECT " + cmnService.J_SQLDBFormat("DATE_OF_FILING", J_SQLColFormat.DateFormatDDMMYYYY) + " FROM TRN_BASIC_INFO WHERE BASIC_INFO_ID = " + lngBasicInfoID;
                string strDateofFiling = Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL));
                //}
                //--
                if(strDateofFiling != "")
                {
                ...
                else if (dtService.J_ConvertToIntYYYYMMDD(J_ReturnServerDate()) > dtService.J_ConvertToIntYYYYMMDD(strReturnLastDate))
                {
                ...
                else
                {
                    ...
                    //
                    //lblFDTick.Text = "\u2714";
                    //lblFDTick.ForeColor = Color.Green;
                }
                //--
                ...
                if (dtService.J_IsBlankDateCheck(ref mskFilingDate, J_ShowMessage.NO) == true)
                    ...
                    mskFilingDate.ReadOnly = true;
                //
                if (dtService.J_IsBlankDateCheck(ref mskFilingDate, J_ShowMessage.NO) == false)
                {
                    if (dtService.J_ConvertToIntYYYYMMDD(mskFilingDate.Text) > dtService.J_ConvertToIntYYYYMMDD(strReturnLastDate))
                    {
                        lblFDTick.Text = "\u0058";
                        lblFDTick.ForeColor = Color.Red;
                        lblFDTick.Font = new Font("Microsoft Sans Serif", 12, FontStyle.Bold);
                    }
                    else
                    {
                        lblFDTick.Text = "\u2714";
        ...
        private void PnlReturnSummary_Paint(object sender, PaintEventArgs e)
        {

        }


        #region BtnCHallanVerify_Click
        private void BtnCHallanVerify_Click(object sender, EventArgs e)
        {
            try
            {
                if (btnChallanVerify.Text == "Check")
                {
                    #region Check
                    TDSMAN.Classes.TDSMAN.T_pVerifyChallanId = 0;
                    TDSMAN.Classes.TDSMAN.T_pVerifyChallanId = lngBasicInfoID;
                    //
                    //cmnService.J_ShowChildForm(new TrnBulkChallanVerification(), J_Var.frmMain, "Bulk Challan Verification");
                    //
                    //--
                    ...
                    //
                    btnChallanVerify.Text = "Stop";
                    btnChallanVerify.ForeColor = Color.Red;
                    //--
                    ...
                    //
                    lblPRN_NO.Text = Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT PRN_NO FROM TRN_BASIC_INFO WHERE BASIC_INFO_ID = " + lngBasicInfoID));
                    ...
                    strSQL = @"SELECT  TOP 1 MST_TAN_ACCOUNT.TAN_NO,
                           ...
                           WHERE   MST_TAN_ACCOUNT.TAN_NO = '" + cmnService.J_Left(lblValueTAN.Text.Trim(), 10) + "'";
                    //
                    IDataReader drdShowRecord = null;
                    drdShowRecord = dmlService.J_ExecSqlReturnReader(strSQL);
                    ...
                    InitializeCaptcha();
                    //
                    grpTracesLoginDetails.Visible = true;
                    //
                    tbcView.Visible = true;
                    tbcView.SelectTab(tbpChallanDetails);
                    //
                    if (txtTANNo.Text == "")
                        txtTANNo.Select();
                    else
                ...
                else if (btnChallanVerify.Text == "Stop")
                {
                    //
                    bgwChallanValidation.CancelAsync();
                    bgwChallanValidation.Dispose();
                    //
                    GC.Collect();
                    //
                    this.Cursor = Cursors.Default;
            ...
            {
                btnChallanVerify.Text = "Check";
                btnChallanVerify.ForeColor = Color.Black;
            }
        }
        #endregion
        ...
        #region rbnSelectRegular_CheckedChanged
        private void rbnSelectRegular_CheckedChanged(object sender, EventArgs e)
        {
            if (rbnSelectRegular.Checked == true)
            {
                //--
                strDeducteeDetailsTable = "TRN_DEDUCTEE_DETAILS";
                strBasicInfoID = "BASIC_INFO_ID";
                ...
                strSQL = " SELECT ASST_ID," +
                    ...
                    "      WHERE  VISIBILITY_FLAG = 0 " +
                    "      ORDER BY ASST_ID DESC";
                if (dmlService.J_PopulateComboBox(strSQL, ref cmbSelectFAYear, 1, J_ComboBoxSelectedIndex.YES) == false) return;
                //-----------
                //-- QUARTER
                //-----------
                string[] strQtr = { T_Qtr.Q1, T_Qtr.Q2, T_Qtr.Q3, T_Qtr.Q4 };
                dmlService.J_PopulateComboBox(strQtr, ref cmbSelectQtr, 1);
                //-----------
                //-- FORM NO
                //-----------
                string[] strFormNo1 = { T_FormNo.F24Q, T_FormNo.F26Q, T_FormNo.F27Q, T_FormNo.F27EQ };
                dmlService.J_PopulateComboBox(strFormNo1, ref cmbSelectFormNo, 1);
                //-----------
                //-- COMPANY
                //-----------
                strSQL = " SELECT COMPANY_ID," +
                    ...
                    "      FROM   MST_COMPANY " +
                    "      ORDER BY COMPANY_NAME";
                if (dmlService.J_PopulateComboBox(strSQL, ref cmbSelectCompany) == false) return;
                //-----------
                #endregion
                //--
                #region BOOKMARK
                if (TDSMAN.Classes.TDSMAN.T_pPredictDefaultsId > 0)
                {
                    ...
                    {
                        blnSelectComboExit = true;
                        cmbSelectQtr.Text = Convert.ToString(drdGetDetails["QTR"]);
                        cmbSelectCompany.Text = Convert.ToString(drdGetDetails["COMPANY_NAME"]) + " [" + Convert.ToString(drdGetDetails["TAN_NO"]) + "]";
                        cmbSelectFAYear.Text = Convert.ToString(drdGetDetails["FA_YEAR"]);
                        cmbSelectFormNo.Text = Convert.ToString(drdGetDetails["FORM_NO"]);
                        //--
                        //if (Convert.ToString(drdGetDetails["COMPANY_NAME"]).Length > 47)
                        //    lblValueTAN.Text = Convert.ToString(drdGetDetails["TAN_NO"]) + " [ " + cmnService.J_Left(Convert.ToString(drdGetDetails["COMPANY_NAME"]), 47) + " ]";
                        //else
                ...
                else
                {
                    if (TDSMAN.Classes.TDSMAN.T_pBookMarkOption == true)
                    {
                        if (dmlService.J_IsRecordExist("MST_BOOKMARK_DETAIL") == true)
                        {
                            ...
                            //                    cmbFinancialYear.Text = Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL));//Convert.ToInt32(Support.SetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex, intAsstId));
                            blnSelectComboExit = true;
                            cmbSelectQtr.Text = strQuarter;
                            cmbSelectCompany.Text = strCompanyName + " [" + strTAN + "]";
                            cmbSelectFAYear.Text = strFAYear;
                            ////--
                            //if (strCompanyName.Length > 47)
                            //    lblValueTAN.Text = strTAN + " [ " + cmnService.J_Left(strCompanyName, 47) + " ]";
                            //else
                            ...
                            //--
                            blnSelectComboExit = false;
                            cmbSelectFormNo.Text = strFormNoBkmark;
                        }
                    }
                }
                #endregion
            ...
            else if (rbnSelectCorrection.Checked == true)
            {
        ...
        #region btnExitSelection_Click
        private void btnExitSelection_Click(object sender, EventArgs e)
        {
            GC.Collect();
            //
            dmlService.Dispose();
        ...
        private void btnGo_Click(object sender, EventArgs e)
        {
            if(rbnSelectRegular.Checked==true)
            {
                ...
                #endregion
                blnSelectComboExit = true;
                cmbQuarter.Text = cmbSelectQtr.Text;
                cmbCompany.Text = cmbSelectCompany.Text;
                cmbFinancialYear.Text = cmbSelectFAYear.Text;
                cmbFormNo.Text = cmbSelectFormNo.Text;
                blnSelectComboExit = false;
                //--
                if (cmnService.J_Left(cmbSelectCompany.Text, cmbSelectCompany.Text.Length - 13).Length > 47)
                    lblValueTAN.Text = cmnService.J_Left(cmnService.J_Right(cmbSelectCompany.Text,11),10) + " [ " + cmnService.J_Left(cmnService.J_Left(cmbSelectCompany.Text, cmbSelectCompany.Text.Length - 13), 47) + " ]";
                else
                    lblValueTAN.Text = cmnService.J_Left(cmnService.J_Right(cmbSelectCompany.Text, 11), 10) + " [ " + cmnService.J_Left(cmbSelectCompany.Text, cmbSelectCompany.Text.Length - 13) + " ]";
                //
                lblValueFAYear.Text = cmbSelectFAYear.Text;
                lblValueQtr.Text = cmbSelectQtr.Text;
                lblValueFormNo.Text = cmbSelectFormNo.Text;
                lblValueFormNoDisplay.Text = TdsMan.GetFormNoIT2025(lblValueFormNo.Text) + " (" + lblValueFormNo.Text + ")";
                //--
                if (cmbSelectFAYear.SelectedIndex <= 0)
                {
                    cmnService.J_UserMessage("Select FA Year !!");
                    return;
                }
                if (cmbSelectCompany.SelectedIndex <= 0)
                {
                    cmnService.J_UserMessage("Select Company !!");
                    return;
                }
                if (cmbSelectFormNo.Text == "")
                {
                    cmnService.J_UserMessage("Select Form No !!");
                    return;
                }
                if (cmbSelectQtr.Text == "")
                {
                    cmnService.J_UserMessage("Select Quarter !!");
                    return;
                }
                //--
                lngBasicInfoID = TdsMan.T_ReturnBasicInfoId(dmlService.J_pCommand, Convert.ToInt32(Support.GetItemData(cmbSelectFAYear, cmbSelectFAYear.SelectedIndex)),
                                                        cmbSelectQtr.Text,
                                                        Convert.ToInt32(Support.GetItemData(cmbSelectCompany, cmbSelectCompany.SelectedIndex)),
                                                        cmbSelectFormNo.Text);

                if (lngBasicInfoID == 0)
                {
                    cmnService.J_UserMessage( "No records found for the selected return");
                    return;
                }
                //--
                cmbMain_SelectedIndexChanged(sender, e);
                ...
                btnViewLateFine.Visible = true;
                //
                lblChallanCheckLabel.Visible = true;
                btnChallanVerify.Visible = true;
                //--
                //rbnRegularCorrectionReturn_CheckedChanged(sender, e);
                //
            }
            ...
            {
                if (dgvSelectCorrection.RowCount < 0)
                {
                    lngBasicInfoID = 0;
                    ClearControls();
                    return;
                ...
                {
                    if (dgvSelectCorrection.CurrentRow == null) return;
                    lngBasicInfoID = Convert.ToInt64(dgvSelectCorrection.Rows[dgvSelectCorrection.CurrentRow.Index].Cells[0].Value);
                }
                //-- 2019/03/14
                strCompanyDeductorType = Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT MST_CATEGORY.CATEGORY_CODE FROM COR_TRN_COMPANY, MST_CATEGORY WHERE COR_TRN_COMPANY.D_CATEGORY_ID = MST_CATEGORY.CATEGORY_ID AND COR_TRN_COMPANY.BATCH_HEADER_ID = " + lngBasicInfoID));
                //
        ...
        private void bgwChallanValidation_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            //
        }
        #endregion

        #region btnLoginTraces_Click
        private void btnLoginTraces_Click(object sender, EventArgs e)
        {
            //--
            ...
            //pgTimer.Start();
            //-------------------------------------------
            if (!bgwTracesLogin.IsBusy)
                bgwTracesLogin.RunWorkerAsync(objList);
            //
        }
        #endregion

        #region btnChallanVerify_MouseMove
        private void btnChallanVerify_MouseMove(object sender, MouseEventArgs e)
        {
            if(btnChallanVerify.Text == "Stop")
                tllChallanVerify.SetToolTip(btnChallanVerify, "Click to Stop Checking Challan Status");
            else
                tllChallanVerify.SetToolTip(btnChallanVerify, "");
        }
        #endregion

        #region dgvShortDeductionsSD_CellFormatting
        private void dgvShortDeductionsSD_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (rbnRegularReturn.Checked == true)
            {
                //-- 2023/10/16
                if (Convert.ToString(dgvShortDeductionsSD.Rows[e.RowIndex].Cells["VERIFIED_PAN_STATUS"].Value) == "1")
                {
                    //NOW HIGHLIGHLING THE CELL 
                    e.CellStyle.BackColor = Color.Red;
                    e.CellStyle.ForeColor = Color.White;
                    dgvShortDeductionsSD.Rows[e.RowIndex].Cells[e.ColumnIndex].ToolTipText = "Invalid PAN";
                }
                //
                if (Convert.ToString(dgvShortDeductionsSD.Rows[e.RowIndex].Cells["VERIFIED_PAN_STATUS"].Value) == "2")
                {
                    //NOW HIGHLIGHLING THE CELL 
                    e.CellStyle.BackColor = Color.Yellow;
                    e.CellStyle.ForeColor = Color.Black;
                    dgvShortDeductionsSD.Rows[e.RowIndex].Cells[e.ColumnIndex].ToolTipText = "Inoperative PAN";
                }
                //
            }
        }
        ...
        #region BtnViewShortDeductions_MouseMove
        private void BtnViewShortDeductions_MouseMove(object sender, MouseEventArgs e)
        {
            if (cmbFormNo.Text == T_FormNo.F27Q)
                tlTipShortDeduction.SetToolTip(btnViewShortDeductions, "100% accuracy is not assured owing to multiple factors that determine the rate of tax deduction.\nIt is strongly referred to cross-check.");
        }
        ...
        #region bgwTracesLogin_DoWork
        private void bgwTracesLogin_DoWork(object sender, DoWorkEventArgs e)
        {
        ...
        #region bgwTracesLogin_RunWorkerCompleted
        private void bgwTracesLogin_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                switch (enmReqType)
                {
                    ...
                    case enmRequestType.BINPP_Details:
                        //pgTimerGrid.Stop();
                        //dgvChallanDetails.Rows[intRowIndex].Cells[7].Value = 100;
                        ////-----------------------------------------------------
                        //if (objResponse.Respons == enmResponse.SessionTimeout)
                        //{
                        //    ShowHideLoginDetails(enmRequestType.LogOff);
                        ...
                        //    dgvChallanDetails.Rows[intRowIndex].Cells[intColumnIndex - 3].Selected = true;
                        //    dgvChallanDetails.CurrentCell = dgvChallanDetails.Rows[intRowIndex].Cells[intColumnIndex - 3];
                        //    dgvChallanDetails.BeginEdit(true);
                        //    return;
                        //}
                        ////-----------------------------------------------------
                        //dTable = (DataTable)objResponse.CustomeTypes;
        ...
        void ShowHideLoginDetails(enmRequestType enmStatus)
        {
            switch (enmStatus)
            {
                ...
                case enmRequestType.CINPP:
                    ...
                    grpTracesLoginDetails.Visible = false;
                    //
                    //this.Cursor = Cursors.WaitCursor;
                    //
                    if (!bgwChallanValidation.IsBusy)
                        bgwChallanValidation.RunWorkerAsync();
                    //
                    break;
                case enmRequestType.LogOff:
                    ArrayList objList = new ArrayList();
        ...
        void PopulateDatagridView(DataTable dsRecords, enmRequestType enmRecType)
        {
            ...
            dgvChallanDetails.Columns.Clear();
            dgvChallanDetails.DataSource = null;
            dgvChallanDetails.DataSource = dsRecords;
            //---------------------------------------
            ...
            {
                dgvChallanDetails.Columns[0].Width = 0;
                dgvChallanDetails.Columns[0].Visible = false;
                dgvChallanDetails.Columns[0].ReadOnly = true;
                dgvChallanDetails.Columns[1].Width = 0;
                dgvChallanDetails.Columns[1].Visible = false;
                dgvChallanDetails.Columns[1].ReadOnly = true;
                dgvChallanDetails.Columns[2].Width = 130;
                dgvChallanDetails.Columns[2].ReadOnly = true;
                dgvChallanDetails.Columns[2].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;

                dgvChallanDetails.Columns[3].Width = 150;
                dgvChallanDetails.Columns[3].ReadOnly = true;
                dgvChallanDetails.Columns[3].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                dgvChallanDetails.Columns[4].ReadOnly = true;
                dgvChallanDetails.Columns[5].ReadOnly = true;
                dgvChallanDetails.Columns[6].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                dgvChallanDetails.Columns[6].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                dgvChallanDetails.Columns[6].SortMode = DataGridViewColumnSortMode.NotSortable;
                dgvChallanDetails.Columns[6].ReadOnly = false;
            ...
            {
                dgvChallanDetails.Columns[0].Width = 150;
                dgvChallanDetails.Columns[0].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                dgvChallanDetails.Columns[1].Width = 150;
                dgvChallanDetails.Columns[1].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                //dgvChallanDetails.Columns[3].Visible = false;
                dgvChallanDetails.Columns[3].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                dgvChallanDetails.Columns[3].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                dgvChallanDetails.Columns[3].SortMode = DataGridViewColumnSortMode.NotSortable;
                dgvChallanDetails.Columns[4].Visible = false;
                dgvChallanDetails.Columns[5].Visible = false;

                dgvChallanDetails.Columns[1].ReadOnly = true;
                dgvChallanDetails.Columns[2].ReadOnly = true;
                dgvChallanDetails.Columns[3].ReadOnly = false;
                dgvChallanDetails.Columns[4].ReadOnly = true;
                dgvChallanDetails.Columns[5].ReadOnly = true;
            ...
            dgvChallanDetails.Columns.Add(btn);
            btn.HeaderText = "";
            ...
            dgvChallanDetails.Columns.Add(prg);
            prg.Name = "";
            ...
            if (dsRecords.Rows.Count > 0)
            {
                if (enmRecType == enmRequestType.CINPP || enmRecType == enmRequestType.CINParticulars)
                {
                    dgvChallanDetails.Rows[0].Cells[6].Selected = true;

                    dgvChallanDetails.CurrentCell = dgvChallanDetails.Rows[0].Cells[6];
                    dgvChallanDetails.BeginEdit(true);
                }
                else
                {
                    dgvChallanDetails.Rows[0].Cells[3].Selected = true;

                    dgvChallanDetails.CurrentCell = dgvChallanDetails.Rows[0].Cells[3];
                    dgvChallanDetails.BeginEdit(true);
                }
            }
            ...
            //-- ARUP @ 2014/09/16
            //foreach (DataGridViewRow gridRow in dgvChallanDetails.Rows)
            //{
        ...
        #region LoadSelectBatch
        private void LoadSelectBatch()
        {
            string[,] strMatrixBatchGrid = null; string strOrderBy = ""; string strQuery = ""; DataSet dsetGridClone = new DataSet();
            ...
            if (dsetGridClone != null) dsetGridClone.Clear();
            //blnGridExit = true;
            dsetGridClone = dmlService.J_ShowDataInGrid(ref dgvSelectCorrection, strSQL, strMatrixBatchGrid);       //Show Data into the Grid
            //blnGridExit = false;
        }
        #endregion

        #region bgwChallanValidation_DoWork_COMMENTED
        //private void bgwChallanValidation_DoWork(object sender, DoWorkEventArgs e)
        //{
        //    ArrayList objRetval = new ArrayList();
        //    TracesResponse objResponse;
        //    //ArrayList objMessage = (ArrayList)e.Result;
        //    //TracesResponse objResponse = (TracesResponse)objMessage[0];
        ...
        //        //-------------------------------------
        //        objData.BSRCode = dgvChallanDetails.Rows[i].Cells[intGridBSRCode].Value.ToString();// txtBSRCode.Text.Trim();
        //        objData.TaxDepositedDate = String.Format("{0:dd-MMM-yyyy}",  dgvChallanDetails.Rows[i].Cells[intGridDepositDate].Value);//  mskChallanDate.Text;
        //        objData.ChallanSerialNo = dgvChallanDetails.Rows[i].Cells[intGridChallanNo].Value.ToString();//  txtChallanSerialNo.Text;
        //        objData.ChallanAmount = dgvChallanDetails.Rows[i].Cells[intGridTax].Value.ToString();//  txtChallanAmount.Text;
        //        //-- VALIDATION -----------------------
        ...
        //        objList.Add(objData);
        //        //
        //        //objResponse = objAccount.CIN_CIN_BINParticulars((TracesData)objList[1]);
        //        objResponse = objAccount.CIN_CIN_BINParticulars_New((TracesData)objList[1]);
        //        objRetval.Add(enmRequestType.CINParticulars);
        //        objRetval.Add(objResponse);
        //        e.Result = objRetval;
        //        //
        ...
        //            //--
        //            ArrayList objMessage = (ArrayList)e.Result;
        //            ArrayList objMessage1 = (ArrayList)e.Result;
        //            enmRequestType enmReqType = (enmRequestType)objMessage[0];
        //            TracesResponse objResponse1 = (TracesResponse)objMessage[1];
        //            DataTable dTable = (DataTable)objResponse.CustomeTypes;
        //            //
        //            TracesResponse objResponse2 = (TracesResponse)objMessage1[1];
        //            DataTable dTable1= (DataTable)objResponse1.CustomeTypes;
        //            //
        //            //dt.Rows[0].Field<string>(0)
        //            if (Convert.ToInt32(dTable.Rows.Count) > 0)
        //            {

        //                dgvChallanDetails.Rows[i].Cells[intGridReceiptNo].Value = dTable.Rows[0]["Recipt Number"].ToString();
        //                //--
        //                ArrayList objList1 = new ArrayList();
        //                TracesData objData1 = new TracesData();
        //                ////---------------------------------
        //                objData1.BSRCode = dgvChallanDetails.Rows[i].Cells[intGridBSRCode].Value.ToString();// Convert.ToString(dgvStatementList.Rows[e.RowIndex].Cells[0].Value) + Convert.ToString(dgvStatementList.Rows[e.RowIndex].Cells[1].Value);
        //                objData1.FromChallanDepositDate = String.Format("{0:dd-MMM-yyyy}", dgvChallanDetails.Rows[i].Cells[intGridDepositDate].Value);// Convert.ToString(dgvStatementList.Rows[e.RowIndex].Cells[2].Value);
        //                objData1.ChallanSerialNo = dgvChallanDetails.Rows[i].Cells[intGridChallanNo].Value.ToString();// Convert.ToString(dgvStatementList.Rows[e.RowIndex].Cells[3].Value);
        //                objData1.ChallanAmount = String.Format("{0:0.00}", Convert.ToDouble(dgvChallanDetails.Rows[i].Cells[intGridTax].Value.ToString()));//  Convert.ToString(dgvStatementList.Rows[e.RowIndex].Cells[6].Value);
        //                objData1.PRN_NO = dgvChallanDetails.Rows[i].Cells[intGridReceiptNo].Value.ToString();//  Convert.ToString(dgvStatementList.Rows[e.RowIndex].Cells[5].Value);
        //                                                                                                     //---------------------------------------------
        //                objList1.Add(enmRequestType.ConsumptionDetails);
        //                objList1.Add(objData1);
        //                //
        //                objResponse2 = objAccount.RequestForConsumptionDetailsCIN((TracesData)objList1[1]);
        //                objRetval.Add(enmRequestType.ConsumptionDetails);
        //                objRetval.Add(objResponse2);

        //                if (objResponse2.Respons == enmResponse.Failed)
        //                {
        //                    cmnService.J_UserMessage(objResponse.Message);
        //                    return;
        //                }
        ...
        //                {
        //                    dTable1.Clear();
        //                    dTable1 = (DataTable)objResponse2.CustomeTypes;
        //                    //
        //                    double dblAvailableAmount = 0;
        //                    if (Convert.ToInt32(dTable1.Rows.Count) > 1)
        //                    {
        //                        //row["ImagePath"].ToString();
        //                        foreach (DataRow row in dTable1.Rows)
        //                        {
        //                            if (!string.IsNullOrEmpty(row["Available Amount"].ToString()))
        //                                dblAvailableAmount = dblAvailableAmount + Convert.ToDouble(row["Available Amount"].ToString());
        //                        }
        //                    }
        //                    else if (Convert.ToInt32(dTable1.Rows.Count) == 1)
        //                        dblAvailableAmount = Convert.ToDouble(dTable1.Rows[0]["Available Amount"].ToString());
        //                    else
        //                        dblAvailableAmount = Convert.ToDouble(dgvChallanDetails.Rows[i].Cells[intGridTax].Value.ToString());
        //                    //
        //                    if (String.Format("{0:0.00}", Convert.ToDouble(dgvChallanDetails.Rows[i].Cells[intGridTax].Value.ToString())) == String.Format("{0:0.00}", dblAvailableAmount))
        //                        dgvChallanDetails.Rows[i].Cells[intGridAvailableBalance].Value = "";
        //                    else
        //                        dgvChallanDetails.Rows[i].Cells[intGridAvailableBalance].Value = String.Format("{0:0.00}", dblAvailableAmount);
        //                    //-- OVERBOOKED
        //                    if (lblPRN_NO.Text == "" && dgvChallanDetails.Rows[i].Cells[intGridAvailableBalance].Value.ToString().Trim() != "") //-- ONLY FOR THOSE RETURN THAT HAS NOT BEEN FILED
        //                    {
        //                        if ((Convert.ToDouble(dgvChallanDetails.Rows[i].Cells[intGridAvailableBalance].Value.ToString()) - Convert.ToDouble(dgvChallanDetails.Rows[i].Cells[intGridDeducteeTotal].Value.ToString())) < 0)
        //                        {
        //                            dgvChallanDetails.Rows[i].Cells[intGridOverBooked].Value = String.Format("{0:0.00}", Math.Abs(Convert.ToDouble(dgvChallanDetails.Rows[i].Cells[intGridAvailableBalance].Value.ToString()) - Convert.ToDouble(dgvChallanDetails.Rows[i].Cells[intGridDeducteeTotal].Value.ToString())));
        //                        }
        //                    }
        //                    //
        //                }
        //                //
        //                if (bgwChallanValidation.CancellationPending)
        //                {
        //                    e.Cancel = true;
        //                    break;
        //                }
        ...
        //            }
        //            else
        //                dgvChallanDetails.Rows[i].Cells[intGridAvailableBalance].Value = "Not Found";
        //            //
        //            if (bgwChallanValidation.CancellationPending)
        //            {
        //                e.Cancel = true;
        ...
        #region bgwChallanValidation_DoWork

        private void bgwChallanValidation_DoWork(
            object sender,
            DoWorkEventArgs e)
        {
            try
            {
                List<ChallanQuery> listChlnQuery =
                    new List<ChallanQuery>();

                // ============================================================
                // STEP 1 : PREPARE CHALLAN LIST
                // ============================================================

                for (int i = j;
                     i <= dgvChallanDetails.RowCount - 1;
                     i++)
                {
                    if (bgwChallanValidation.CancellationPending)
                    {
                    ...
                    if (cmbCompany.Text.Length >= 11)
                    {
                        objChallan.TAN =
                            cmbCompany.Text.Substring(
                                cmbCompany.Text.Length - 11,
                                10);
                    }
                    ...
                    DateTime dtChallanDate =
                        Convert.ToDateTime(
                            dgvChallanDetails.Rows[i]
                            .Cells[intGridDepositDate].Value);
                    ...
                    objChallan.BSRCode =
                        Convert.ToString(
                            dgvChallanDetails.Rows[i]
                            .Cells[intGridBSRCode].Value).Trim();
                    ...
                    object objChallanNo =
                        dgvChallanDetails.Rows[i]
                        .Cells[intGridChallanNo].Value;

                    if (objChallanNo != null &&
                        !String.IsNullOrEmpty(
                            Convert.ToString(objChallanNo)))
                    {
                        objChallan.ChallanNo =
                            Convert.ToString(
                                Convert.ToInt64(objChallanNo));
                    }
                    else
                    {
                        objChallan.ChallanNo = "";
                    ...
                    objChallan.ChallanAmount =
                        Convert.ToString(
                            dgvChallanDetails.Rows[i]
                            .Cells[intGridTax].Value).Trim();

                    if (objChallan.ChallanAmount == "0.0000")
                        objChallan.ChallanAmount = "0";

                    // --------------------------------------------------------
                    // ADD
                    // --------------------------------------------------------

                    listChlnQuery.Add(objChallan);
                }
                ...
                // ============================================================
                // STEP 3 : PROCESS ALL CHALLANS
                // ============================================================

                List<ChallanQuery> listResult =
                    objAccount.ChallanStatusQueryInBulk(
                        listChlnQuery,
        ...
        #region bgwChallanValidation_RunWorkerCompleted
        //private void bgwChallanValidation_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        //{
        //    this.Cursor = Cursors.Default;
        //    //
        //    if (e.Cancelled)
        //    {
        //        cmnService.J_UserMessage("Checking of Challan Status stopped");
        //    }
        //    else
        //        cmnService.J_UserMessage("Challan Status Checked");
        //    //
        //    btnChallanVerify.Text = "Check";
        //    btnChallanVerify.ForeColor = Color.Black;
        //    //
        ...
        private void bgwChallanValidation_RunWorkerCompleted(
            object sender,
            RunWorkerCompletedEventArgs e)
        {
            ...
            {
                // ============================================================
                // STEP 1 : CHECK WHETHER OPERATION WAS CANCELLED
                // ============================================================

                if (e.Cancelled)
                {
                    cmnService.J_UserMessage(
                        "Checking of Challan Status stopped");

                    return;
                }

                // ============================================================
                // STEP 2 : CHECK BACKGROUND WORKER ERROR
                // ============================================================

                if (e.Error != null)
                {
                    cmnService.J_UserMessage(e.Error.Message);
                    return;
                }

                // ============================================================
                // STEP 3 : CHECK EXCEPTION RETURNED FROM DoWork
                // ============================================================
                ...
                // ============================================================
                // STEP 4 : GET BULK RESULT
                // ============================================================

                if (e.Result is List<ChallanQuery>)
                {
                    List<ChallanQuery> listResult =
                        (List<ChallanQuery>)e.Result;

                    if (listResult == null || listResult.Count == 0)
                    {
                        cmnService.J_UserMessage(
                            "Unable to check Challan Status");

                        return;
                    }

                    // ========================================================
                    // STEP 5 : PROCESS EACH RETURNED CHALLAN
                    // ========================================================

                    foreach (ChallanQuery result in listResult)
                    {
                        int gridRow = -1;

                        // ----------------------------------------------------
                        // FIND CORRESPONDING GRID ROW
                        // Using BSR Code + Challan No + Challan Date
                        // ----------------------------------------------------
                             ...
                             i++)
                        {
                            ...
                            string gridChallanNo =
                                Convert.ToString(
                                    dgvChallanDetails.Rows[i]
                                        .Cells[intGridChallanNo].Value).Trim();

                            string gridDate = "";

                            object gridDateValue =
                                dgvChallanDetails.Rows[i]
                                    .Cells[intGridDepositDate].Value;

                            if (gridDateValue != null &&
                                gridDateValue != DBNull.Value)
                            {
                                DateTime dtGridDate;

                                if (DateTime.TryParse(
                                    Convert.ToString(gridDateValue),
                                    out dtGridDate))
                                {
                                    gridDate =
                                        dtGridDate.ToString("dd-MM-yyyy");
                                }
                            }

                            string resultDate = "";

                            DateTime dtResultDate;

                            if (DateTime.TryParse(
                                Convert.ToString(result.ChallanDate),
                                out dtResultDate))
                            {
                                resultDate =
                                    dtResultDate.ToString("dd-MM-yyyy");
                            }

                            if (gridBSR ==
                                    Convert.ToString(result.BSRCode).Trim()
                                &&
                                gridChallanNo ==
                                    Convert.ToString(result.ChallanNo).Trim()
                                &&
                                gridDate == resultDate)
                            {
                                gridRow = i;
                        ...
                        // ====================================================
                        // STEP 6 : HANDLE PROCESSING FAILED
                        // ====================================================

                        if (result.Message ==
                            enmChallanStatus.PROCESSING_FAILED)
                        {
                            dgvChallanDetails.Rows[gridRow]
                                .Cells[intGridAvailableBalance].Value =
                                "Not Found";
                        ...
                        // ====================================================
                        // STEP 7 : HANDLE RECORD NOT FOUND
                        // ====================================================

                        if (result.Message ==
                            enmChallanStatus.RECORD_NOT_FOUND)
                        {
                            dgvChallanDetails.Rows[gridRow]
                                .Cells[intGridAvailableBalance].Value =
                                "Not Found";
                        ...
                        // ====================================================
                        // STEP 8 : SET RECEIPT NUMBER
                        // ====================================================

                        if (!String.IsNullOrEmpty(result.ReceiptNumber))
                        {
                            dgvChallanDetails.Rows[gridRow]
                                .Cells[intGridReceiptNo].Value =
                                result.ReceiptNumber;
                        }

                        // ====================================================
                        // STEP 9 : SET AVAILABLE BALANCE
                        // ====================================================

                        if (!String.IsNullOrEmpty(result.AvailableAmount))
                        {
                            dgvChallanDetails.Rows[gridRow]
                                .Cells[intGridAvailableBalance].Value =
                        ...
                        // ====================================================
                        // STEP 10 : CALCULATE OVERBOOKED AMOUNT
                        // Same logic as your old working code
                        // ====================================================

                        if (lblPRN_NO.Text == "" &&
                            !String.IsNullOrEmpty(result.AvailableAmount))
                        {
                            double dblAvailableAmount = 0;
                            double dblDeducteeTotal = 0;

                            Double.TryParse(
                                result.AvailableAmount,
                                out dblAvailableAmount);

                            Double.TryParse(
                                Convert.ToString(
                                    dgvChallanDetails.Rows[gridRow]
                                        .Cells[intGridDeducteeTotal].Value),
                                out dblDeducteeTotal);

                            double dblDifference =
                                dblAvailableAmount - dblDeducteeTotal;

                            if (dblDifference < 0)
                            {
                                dgvChallanDetails.Rows[gridRow]
                                    .Cells[intGridOverBooked].Value =
                                    String.Format(
                                        "{0:0.00}",
                                        Math.Abs(dblDifference));
                            }
                            else
                            {
                                dgvChallanDetails.Rows[gridRow]
                    ...
                    // ========================================================
                    // STEP 11 : SHOW GLOBAL ERROR MESSAGE IF ANY
                    // ========================================================
        ...
        private void LoadChallanGrid(long BasicInfoID)
        {
            ...
            //--
            strSQL = @"SELECT TOP 1 MST_CATEGORY.CATEGORY_CODE FROM MST_COMPANY, MST_CATEGORY, TRN_BASIC_INFO WHERE MST_COMPANY.D_CATEGORY_ID = MST_CATEGORY.CATEGORY_ID AND MST_COMPANY.COMPANY_ID = TRN_BASIC_INFO.COMPANY_ID AND TRN_BASIC_INFO.BASIC_INFO_ID =" + BasicInfoID;
            string strDeductorType = Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL));
            //--
            if (strDeductorType == "A" || strDeductorType == "S")
            {
            ...
            strSQL = @"SELECT MST_ASSESSMENT.ASST_ID FROM MST_ASSESSMENT, TRN_BASIC_INFO WHERE MST_ASSESSMENT.ASST_ID = TRN_BASIC_INFO.ASST_ID AND TRN_BASIC_INFO.BASIC_INFO_ID =" + BasicInfoID;
            int intFAYearId = cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL)));
            if (intFAYearId >= T_FinancialYearID.F2013_14ID)
                strSectionSize = "0";
            else
                strSectionSize = "70";
            //--
            string[,] strMatrixChallanDetails = {{"ChallanID", "0", "", "", "", "F", ""},
                                        {"Sl", "40", "", "", "", "", ""},
                                        {"Section", strSectionSize, "S", "", "", "F", ""},
                                        {strChallanGridColumn, "95", "S", "", "", "", "T"},
                                        {"Deposit Dt", "80", "dd-MMM-yyyy", "", "", "", "T"},
                                        {strBSRCodeGridColumn, "80", "S", "", "", "", "T"},
                                        {"Value", "100", "0.00", "R", "", "", ""},
                                        {"Ded. Total", "100", "0.00", "R", "", "", "T"},
                                        {"Difference", "90", "0.00", "R", "", "", "T"},
                                        {"ReceiptNo", "0", "", "", "", "F", ""},
                                        {"Avbl Balance", "100", "0.00", "R", "", "", "T"},
                                        {"Short Amt.", "100", "0.00", "R", "", "", "T"}};
            //-----------------------------------------------------------
            ...
            //
            //string[,] strLoadChallanTotTaxIT2025Matrix = {{"TRN_CHALLAN.CHALLAN_NO = ''", "F", "TRN_CHALLAN.TDS - TRN_CHALLAN.INTEREST - TRN_CHALLAN.OTHERS", "F"},
            //                                        {"TRN_CHALLAN.TRANSFER_VOUCHER_NO = ''", "F", "TRN_CHALLAN.TDS - TRN_CHALLAN.INTEREST - TRN_CHALLAN.OTHERS - TRN_CHALLAN.LATE_FEE", "F"}};
            //-----------------------------------------------------------------
            //MODIFIED BY DHRUB ON 07/01/2014 TO DEDUCT LATE_FEE FOR CHALLAN 
            //-----------------------------------------------------------------
            //-----------------------------------------------------------------
            strOrderBy = "TRN_CHALLAN.SL_NO ";// "TRN_CHALLAN.CHALLAN_ID";
            ...
            //if (Convert.ToInt32(Support.GetItemData(cmbFinancialYear, cmbFinancialYear.SelectedIndex)) >= T_FinancialYearID.F2026_27ID)
            //    strQuery = strQuery + " (" + cmnService.J_SQLDBFormat(strLoadChallanTotTaxIT2025Matrix, J_SQLColFormat.Case_End) + " - TRN_CHALLAN.CTRL_TOT_TAX) AS DIFF,";
            //else
                strQuery = strQuery + " (" + cmnService.J_SQLDBFormat(strLoadChallanTotTaxMatrix, J_SQLColFormat.Case_End) + " - TRN_CHALLAN.CTRL_TOT_TAX) AS DIFF,";
            strQuery = strQuery + " '' AS RECEIPT_NO," +
            ...
            dsetGridClone = dmlService.J_ShowDataInGrid(ref dgvChallanDetails, strSQL, strMatrixChallanDetails);       //Show Data into the Grid
        }
        #endregion
