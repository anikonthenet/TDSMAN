...
{
    public partial class TrnCorrForm24Q194PSalaryDetails : TDSMAN.FormGen.GenForm
    {
        ResizeForm _form_resize;

        #region System Generated Code
        public TrnCorrForm24Q194PSalaryDetails()
        {
            InitializeComponent();
            ////--
        ...
        private void TrnForm24QSalaryDetails_Load(object sender, EventArgs e)
        {
            ...
            ClearControlsHeader();
            //------------------
            lngBatchHeaderID = TDSMAN.Classes.TDSMAN.T_pBatchId;
            //-------------------
            ...
            if (lngBatchHeaderID > 0)
            {
                LoadSalaryDetailsGrid(lngBatchHeaderID);
                //-- Generate Salary Detail Srl No.
                txtSalaryDetailSrlNo.Text = Convert.ToString(dmlService.J_ReturnMaxValue("COR_TRN_SALARY_DETAILS_194P", "SL_NO", "BATCH_HEADER_ID = " + lngBatchHeaderID + "") + 1);
            }
            else
            ...
            {
                if (Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT RECEIPT_NO FROM COR_HDR_BATCH WHERE BATCH_HEADER_ID = " + lngBatchHeaderID)) != "")
                {
        ...
        private void BtnEdit_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                if (dgcViewSalaryDetails.CurrentRow.Index >= 0)
                {
                    ...
                    //
                    if (ShowHDRRecord(Convert.ToInt64(dgcViewSalaryDetails.Rows[dgcViewSalaryDetails.CurrentRow.Index].Cells[0].Value)) == false)
                    {
                        if (dsetGridClone == null) return;
                        //dmlService.J_setGridPosition(ref this.dgcViewSalaryDetails, dsetGridClone, "TRN_SALARY_DETAILS_ID", lngSearchId);
                    }
                    //--------------------------------------------------
                    //lblMode.Text = J_Mode.Edit;
                    cmnService.J_StatusButton(this, lblMode.Text);
        ...
        private void tlStrpMnuBatch_Click(object sender, EventArgs e)
        {
            ...
            if (cmnService.J_UserMessage("This will Delete all the records present in this grid. Proceed for Deletion?", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                ...
                strSQL = "DELETE FROM COR_TRN_SALARY_DETAILS_194P WHERE BATCH_HEADER_ID =  " + lngBatchHeaderID + "";
                //-----------------------------------------------------------
        ...
        private void BtnSearchOK_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (txtSlNoSearch.Text.Trim() != "")
                    strCheckFields = strCheckFields + "AND COR_TRN_SALARY_DETAILS_194P.SL_NO = " + cmnService.J_ReturnInt32Value(txtSlNoSearch.Text.Trim()) + " ";
                //-------------------------------------------------------------------
                //-- name 
                //-------------------------------------------------------------------
                ...
                if (txtTotalDeductedSearch.Text.Trim() != "")
                    strCheckFields = strCheckFields + "AND COR_TRN_SALARY_DETAILS_194P.NET_TAX_PAYABLE = " + cmnService.J_ReturnDoubleValue(txtTotalDeductedSearch.Text.Trim()) + " ";
                //----------------------------------------------------------------------
        ...
        private void BtnRefresh_Click(object sender, EventArgs e)
        {
            ...
            //}
            //--
            //txtSalaryDetailSrlNo.Text = Convert.ToString(dmlService.J_ReturnMaxValue("COR_TRN_SALARY_DETAILS_194P", "SL_NO", "BATCH_HEADER_ID = " + lngBasicInfoID + "") + 1);                
            //-----------------------------
            // Added by Dhrub on 30/10/2013
            #region SUM OF DEDUCTION INTO FOUR QUARTERS
            if (TDSMAN.Classes.TDSMAN.T_pFinancialYearId >= T_FinancialYearID.F2013_14ID)
        ...
        private void txtEmployeeName_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (TDSMAN.Classes.TDSMAN.T_ShareReferenceNoF24Q == true)
                {
                    strSQLShowHelpEmployee = @"SELECT  TOP 10 SD.NAME,
                                                             SD.PAN,
                                                             SD.EMPLOYEE_SERIAL_NO
                                                FROM (
                                                SELECT DEDUCTEE_NAME AS NAME, 
                                                       DEDUCTEE_PAN         AS PAN, 
                                                        EMPLOYEE_SERIAL_NO AS EMPLOYEE_SERIAL_NO
                                                FROM   COR_TRN_DEDUCTEE_DETAILS 
                                                WHERE  DEDUCTEE_NAME LIKE '" + cmnService.J_ReplaceQuote(txtEmployeeName.Text) + "%' " +
                                                       @"AND BATCH_HEADER_ID = " + lngBatchHeaderID + " " +
                                               @"GROUP  BY DEDUCTEE_NAME, 
                                                ...
                                                WHERE  EMPLOYEE_NAME LIKE '" + cmnService.J_ReplaceQuote(txtEmployeeName.Text) + "%' " +
                                                      @"AND BATCH_HEADER_ID = " + lngBatchHeaderID + " " +
                                               @"GROUP  BY EMPLOYEE_PAN, 
                                                ...
                                                SELECT EMPLOYEE_NAME  AS NAME,
                                                        EMPLOYEE_PAN  AS PAN, 
                                                        EMPLOYEE_REF  AS EMPLOYEE_SERIAL_NO
                                                FROM   MST_EMPLOYEE, MST_COMPANY
                                                WHERE  MST_EMPLOYEE.COMPANY_ID  = MST_COMPANY.COMPANY_ID
                                                AND    MST_COMPANY.TAN_NO       = '" + TDSMAN.Classes.TDSMAN.T_pTAN + @"'  
                                                AND    EMPLOYEE_NAME LIKE '" + TDSMAN.Classes.TDSMAN.T_DeducteeSearchLikeOperator + cmnService.J_ReplaceQuote(txtEmployeeName.Text) + "%' " +
                                              @"AND    MST_EMPLOYEE.INACTIVE_FLAG = 0) AS SD
                                                ORDER  BY SD.NAME";
                }
                else
                {
                ...
                else
                {
                    ...
                    {
                        if (TDSMAN.Classes.TDSMAN.T_ShareReferenceNoF24Q == true) //-- 2020/06/03
                            lstEmployeeHelp.Items.Add(new ListBoxItem(drdShowEmployeeHelp["NAME"].ToString().PadRight(32) + drdShowEmployeeHelp["PAN"].ToString().PadRight(20) + drdShowEmployeeHelp["EMPLOYEE_SERIAL_NO"].ToString()));
                        else
                            lstEmployeeHelp.Items.Add(new ListBoxItem(drdShowEmployeeHelp["NAME"].ToString().PadRight(32) + drdShowEmployeeHelp["PAN"].ToString()));
                        //--
        ...
        private void lstEmployeeHelp_Click(object sender, EventArgs e)
        {
            ...
            {
                txtEmployeePAN.Text = cmnService.J_Right(strEmployeeHelp, 10);
                txtEmployeeName.Text = cmnService.J_Left(strEmployeeHelp, strEmployeeHelp.Length-10).Trim();
        ...
        private void BtnExit_Click(object sender, System.EventArgs e)
        {
            ...
            //TrnRegularReturn.lblNetTaxableIncome.Text = string.Format("{0:0.00}", Convert.ToDouble(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT SUM(TOTAL_INCOME) AS NET_TAXABLE_INCOME FROM COR_TRN_SALARY_DETAILS_194P WHERE BATCH_HEADER_ID = " + lngBasicInfoID)) == "" ? "0.00" : Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT SUM(TOTAL_INCOME) AS NET_TAXABLE_INCOME FROM COR_TRN_SALARY_DETAILS_194P WHERE BATCH_HEADER_ID = " + lngBasicInfoID))));
            //
        ...
        private bool ValidateFields()
        {
            try
            {
                ...
                else
                {
                    ...
                    //-- DUPLICATE ENTRY
                    //-----------------------------------------------------------------------
                    //strSQL = "SELECT COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_ID " +
                    //    "     FROM   MST_EMPLOYEE," +
                    //    "            COR_TRN_SALARY_DETAILS_194P," +
                    //    "            TRN_BASIC_INFO " +
                    //    "     WHERE  MST_EMPLOYEE.EMPLOYEE_ID         = COR_COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_ID " +
                    //    "     AND    COR_COR_TRN_SALARY_DETAILS_194P.BATCH_HEADER_ID = TRN_BASIC_INFO.BATCH_HEADER_ID " +
                    ...
                    //if (lblMode.Text == J_Mode.Edit)
                    //    strSQL = strSQL + "AND COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_ID <> " + cmnService.J_ReturnInt32Value(txtEmployeeID.Text);
                    ////--
            ...
            try
            {
                ...
                switch (lblMode.Text)
                {
                    ...
                    case J_Mode.Add:
                        ...
                        #region SQL
                        //-----------------------------------------------------------
                        strSQL = @"INSERT INTO COR_TRN_SALARY_DETAILS_194P (BATCH_HEADER_ID,
                                                EMPLOYEE_NAME,
                                                EMPLOYEE_PAN,
                                                EMPLOYEE_CATEGORY,
                                                MODE,
                        ...
                        {
                            strSQL = "UPDATE COR_TRN_SALARY_DETAILS_194P " +
                                     "SET SL_NO = DMAX('SL_NO','COR_TRN_SALARY_DETAILS_194P','BATCH_HEADER_ID =" + lngBatchHeaderID + "')+1 " +
                                     "WHERE TRN_SALARY_DETAILS_194P_ID = DMAX('TRN_SALARY_DETAILS_194P_ID','COR_TRN_SALARY_DETAILS_194P','')";
                        ...
                        {
                            long lngSerialNo = cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT MAX(SL_NO) FROM COR_TRN_SALARY_DETAILS_194P WHERE BATCH_HEADER_ID =" + lngBatchHeaderID)));
                            long lngSDID = cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT MAX(TRN_SALARY_DETAILS_194P_ID) FROM COR_TRN_SALARY_DETAILS_194P WHERE BATCH_HEADER_ID =" + lngBatchHeaderID)));
                            //
                            strSQL = "UPDATE COR_TRN_SALARY_DETAILS_194P " +
                                    "SET SL_NO = " + (lngSerialNo + 1) + " " +
                                    "WHERE TRN_SALARY_DETAILS_194P_ID = " + lngSDID;
                            if (dmlService.J_ExecSql(dmlService.J_pCommand, strSQL) == false)
                            {
                                dmlService.J_Rollback();
                                return;
                        ...
                        //-----------------------------------------------------------
                        lngSearchId = dmlService.J_ReturnMaxValue(dmlService.J_pCommand, "COR_TRN_SALARY_DETAILS_194P", "TRN_SALARY_DETAILS_194P_ID");
                        if (lngSearchId == 0)
                        {
                            dmlService.J_Rollback();
                            return;
                    ...
                    case J_Mode.Edit:
                        ...
                        //
                        strSQL = @"SELECT MODE 
                                   FROM   COR_TRN_SALARY_DETAILS_194P 
                                   WHERE  TRN_SALARY_DETAILS_194P_ID = " + lngSearchId;
                        if (Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL)) == T_CorrectionMode.Delete)
                        {
                            cmnService.J_UserMessage("Deleted Data can't be Updated", MessageBoxIcon.Exclamation);
                            return;
                        }
                        //--
                        if (ValidateFields() == false) return;
                        //--
                        strSQL = "SELECT IMPORT_FLAG " +
                                 "FROM COR_TRN_SALARY_DETAILS_194P " +
                                 "WHERE TRN_SALARY_DETAILS_194P_ID = " + lngSearchId;

                        if (Convert.ToInt32(dmlService.J_ExecSqlReturnScalar(strSQL)) == T_UPDATION_INDICATOR.ON)
                        {
                            strSQL = "SELECT HDR.PAN_COUNTER " +
                                     "FROM   COR_HDR_SALARY_DETAILS_194P AS HDR, " +
                                     "       COR_TRN_SALARY_DETAILS_194P AS TRN " +
                                     "WHERE  HDR.HDR_SALARY_DETAILS_194P_ID = TRN.HDR_SALARY_DETAILS_194P_ID " +
                                     "AND    TRN.TRN_SALARY_DETAILS_194P_ID = " + lngSearchId;

                            int intPanFlag = Convert.ToInt32(dmlService.J_ExecSqlReturnScalar(strSQL));

                            if ((txtEmployeePAN.Text != txtEmployeePAN.Tag) && intPanFlag == 2)
                            {
                                cmnService.J_UserMessage("Deductee PAN cannot be Modified as it has already been Updated in the Previous Statement.");
                                txtEmployeePAN.Text = txtEmployeePAN.Tag.ToString();
                                return;
                            }
                        }
                        //-----------------------------------------------------------
                        ...
                        //-----------------------------------------------------------
                        if (Check_Record(lngSearchId, "COR_TRN_SALARY_DETAILS_194P", "TRN_SALARY_DETAILS_194P_ID", ref dgcViewSalaryDetails) == 0)
                        {
                            dmlService.J_Rollback();
                            return;
                        }
                        //-----------------------------------------------------------
                        if (cmnService.J_UserMessage("Do you want to Update?", MessageBoxButtons.YesNo) == DialogResult.No)
                            return;
                        //
                        dmlService.J_BeginTransaction();
                        //-
                        if (Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT IMPORT_FLAG FROM COR_TRN_SALARY_DETAILS_194P WHERE TRN_SALARY_DETAILS_194P_ID = " + lngSearchId)) == "1")
                        {
                            if (ChkGenerateModifyValues() == false)
                            {
                                if (cmnService.J_UserMessage("To update the record the existing record will be deleted and a new record will be added.\n Proceed?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                                {
                                    dmlService.J_Commit();
                                    return;
                                }
                                // UPDATE MODE TO DELETE
                                strSQL = "UPDATE COR_TRN_SALARY_DETAILS SET " +
                                    "            MODE                   = '" + T_CorrectionMode.Delete + "'," +
                                    "            PAN_UPDATION_INDICATOR =  " + T_UPDATION_INDICATOR.OFF + ", " +
                                    "            EMPLOYEE_PAN           = '" + cmnService.J_ReplaceQuote(txtEmployeePAN.Text) + "' " + //Updated by Shrey on 30/12/2011
                                    "     WHERE  TRN_SALARY_DETAILS_ID  =  " + lngSearchId + " " +
                                    "     AND    IMPORT_FLAG            = 1";
                                ...
                                //-- 2019/05/21
                                if (cmnService.J_ReturnInt64Value(TDSMAN.Classes.TDSMAN.T_pFinancialYearId) >= T_FinancialYearID.F2018_19ID)
                                {
                                    strSQL = @"SELECT COUNT(*) FROM COR_TRN_SALARY_DETAILS 
                                           WHERE TRN_SALARY_DETAILS_ID  =  " + lngSearchId + @" 
                                           AND   MODE                  = '" + T_CorrectionMode.Delete + @"' 
                                           AND   IMPORT_FLAG           = 1 
                                           AND   TS_GS_SEC_17_1 + TS_GS_SEC_17_2 + TS_GS_SEC_17_3 = 0";
                                    if (cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL))) > 0)
                                    {
                                        strSQL = "UPDATE COR_TRN_SALARY_DETAILS SET " +
                                        "                TS_GS_SEC_17_1         =  TS_BALANCE ," +
                                        "                AIS_ITEM_1             =  AIS_Total , " +
                                        "                CVIA_SEC80C_DED_TOTAL  =  CVIA_SEC80CCE_TOTAL_DED_AMOUNT + CVIA_SEC80CCG_DED_AMOUNT  " +
                                        @"        WHERE  TRN_SALARY_DETAILS_ID  =  " + lngSearchId + @" 
                                              AND    MODE                  = '" + T_CorrectionMode.Delete + @"' 
                                              AND    IMPORT_FLAG           = 1 
                                              AND    TS_GS_SEC_17_1 + TS_GS_SEC_17_2 + TS_GS_SEC_17_3 = 0";
                            ...
                            // 
                            else if (ChkGenerateModifyPAN() == false)
                            {
                                strSQL = "UPDATE COR_TRN_SALARY_DETAILS_194P SET " +
                                        "        EMPLOYEE_PAN           = '" + cmnService.J_ReplaceQuote(txtEmployeePAN.Text.Trim()) + "'," +
                                        "        PAN_UPDATION_INDICATOR = " + T_UPDATION_INDICATOR.ON + " " +
                                        " WHERE  TRN_SALARY_DETAILS_ID  = " + lngSearchId + " " +
                                        " AND    IMPORT_FLAG            = 1";
                                if (dmlService.J_ExecSql(strSQL) == false)
                                {
                                ...
                                //NOW IF THE PAN IS UPDATED THEN CHECKING IF THE SAME 
                                //EMPLOYEE NAME AND ITS PAN EXISTS IN DEDUCTEE DETAILS SECTION ALSO

                                strSQL = "SELECT COUNT(*) " +
                                         "FROM   COR_TRN_SALARY_DETAILS_194P, " +
                                         "       COR_HDR_SALARY_DETAILS_194P " +
                                         "WHERE COR_HDR_SALARY_DETAILS_194P.HDR_SALARY_DETAILS_194P_ID = COR_TRN_SALARY_DETAILS_194P.HDR_SALARY_DETAILS_194P_ID " +
                                         "AND   COR_TRN_SALARY_DETAILS_194P.DEDUCTEE_PAN           = '" + cmnService.J_ReplaceQuote(txtEmployeePAN.Tag.ToString()) + "' " +
                                         "AND   COR_TRN_SALARY_DETAILS_194P.DEDUCTEE_NAME          = '" + cmnService.J_ReplaceQuote(txtEmployeeName.Text.Trim()) + "' " +
                                         "AND   COR_TRN_SALARY_DETAILS_194P.BATCH_HEADER_ID        = " + lngBatchHeaderID + " " +
                                         "AND   COR_HDR_SALARY_DETAILS_194P.PAN_COUNTER            <> 2";

                                long lngNoOfRecords = cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar(strSQL)));

                                if (lngNoOfRecords > 0)
                                {
                                    //Seeking confirmation of the user to modify the PANs in deductee details section
                                    if (cmnService.J_UserMessage("Do you want to modify " + lngNoOfRecords + " more record(s) of \"" + txtEmployeeName.Text +
                                                                 "\" with old PAN \"" + txtEmployeePAN.Tag.ToString() + "\" to new PAN \"" + txtEmployeePAN.Text +
                                                                 "\" in this return? \n(Includes only those records in which PAN modification is possible)", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                    {
                                        strSQL = "UPDATE     COR_TRN_SALARY_DETAILS_194P " +
                                                 "INNER JOIN COR_HDR_SALARY_DETAILS_194P " +
                                                 "        ON COR_HDR_SALARY_DETAILS_194P.HDR_DEDUCTEE_DETAIL_ID = COR_TRN_SALARY_DETAILS_194P.HDR_DEDUCTEE_DETAIL_ID " +
                                                 "SET     COR_TRN_SALARY_DETAILS_194P.DEDUCTEE_PAN              = '" + cmnService.J_ReplaceQuote(txtEmployeePAN.Text.Trim()) + "', " +
                                                 "        COR_TRN_SALARY_DETAILS_194P.PAN_UPDATION_INDICATOR    = " + T_UPDATION_INDICATOR.ON + " " +
                                                 "WHERE  COR_TRN_SALARY_DETAILS_194P.DEDUCTEE_PAN               = '" + cmnService.J_ReplaceQuote(txtEmployeePAN.Tag.ToString()) + "' " +
                                                 "AND    COR_TRN_SALARY_DETAILS_194P.DEDUCTEE_NAME              = '" + cmnService.J_ReplaceQuote(txtEmployeeName.Text.Trim()) + "' " +
                                                 "AND    COR_TRN_SALARY_DETAILS_194P.BATCH_HEADER_ID            = " + lngBatchHeaderID + " " +
                                                 "AND    COR_HDR_SALARY_DETAILS_194P.PAN_COUNTER  <> 2";
                            ...
                            else if (ChkGenerateModifyPAN() == true)
                            {
                                //Added by Shrey Kejriwal on 28/11/2011
                                //update pan updation indicator if pan is again made equal to its original value
                                strSQL = "UPDATE COR_TRN_SALARY_DETAILS_194P " +
                    ...
                    case J_Mode.Delete:
                        ...
                        //// RECALCALUTING SRL NO.
                        //if (TdsMan.T_UpdateSrlNo(cmnService.J_ReturnInt32Value(Convert.ToString(dmlService.J_ExecSqlReturnScalar("SELECT SL_NO FROM COR_TRN_SALARY_DETAILS_194P WHERE  TRN_SALARY_DETAILS_194P_ID = " + lngSearchId))),
                        //    lngBasicInfoID, Updt_SrlNo.SalaryDetailSrlNo) == false) return;

                        //Added by Shrey Kejriwal on 05/11/2011

                        int intCount = 0;
                        strSQL = "SELECT SL_NO FROM COR_TRN_SALARY_DETAILS_194P WHERE  TRN_SALARY_DETAILS_194P_ID = " + lngSearchId;
                        ...
                        //-----------------------------------------------------------
                        strSQL = "DELETE FROM COR_TRN_SALARY_DETAILS_194P WHERE TRN_SALARY_DETAILS_194P_ID =  " + lngSearchId + "";
                        //-----------------------------------------------------------
                        if (dmlService.J_ExecSql(dmlService.J_pCommand, strSQL) == false)
                        {
                            lblMode.Text = J_Mode.Add;
        ...
        #region LoadSalaryDetailsGrid
        private void LoadSalaryDetailsGrid(long BatchHeaderID)
        {
            ...
            //-----------------------------------------------------------
            //-----------------------------------------------------------
            strOrderBy = "COR_TRN_SALARY_DETAILS_194P.SL_NO";
            strQuery = "SELECT COR_TRN_SALARY_DETAILS_194P.TRN_SALARY_DETAILS_194P_ID  AS TRN_SALARY_DETAILS_194P_ID," +
                       "       COR_TRN_SALARY_DETAILS_194P.SL_NO                   AS SL_NO," +
                       "       COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_NAME           AS EMPLOYEE_NAME," +
                       "       COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_SERIAL_NO      AS EMPLOYEE_SERIAL_NO," +
                       "       COR_TRN_SALARY_DETAILS_194P.NET_TAX_PAYABLE         AS NET_TAX_PAYABLE " +
                       "FROM   COR_TRN_SALARY_DETAILS_194P " +
                       "WHERE  COR_TRN_SALARY_DETAILS_194P.BATCH_HEADER_ID  = " + BatchHeaderID + " ";
            //-----------------------------------------------------------
            strSQL = strQuery + "ORDER BY " + strOrderBy;
            //-----------------------------------------------------------
            if (dsetGridClone != null) dsetGridClone.Clear();
        ...
        private bool ShowRecord(long Id)
        {
            ...
            try
            {
                string[,] strShowHelpEmployeeMatrix = {{"COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_CATEGORY = 'S'", "F", T_EmployeeCategory.SeniorCitizen, "T"},
                                                       {"COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_CATEGORY = 'O'", "F", T_EmployeeCategory.SuperSeniorCitizen, "T"},
                                                       {"COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_CATEGORY = ''", "F", "", "T"}};
                ////
                strSQL = "SELECT  COR_TRN_SALARY_DETAILS_194P.TRN_SALARY_DETAILS_194P_ID                 AS SALARY_DETAILS_ID," +
                    "             COR_TRN_SALARY_DETAILS_194P.BATCH_HEADER_ID                          AS BASIC_INFO_ID," +
                    "             COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_NAME                          AS EMPLOYEE_NAME," +
                    "             COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_PAN                           AS EMPLOYEE_PAN," +
                    "             COR_TRN_SALARY_DETAILS_194P.EMPLOYEE_SERIAL_NO                     AS EMPLOYEE_REF," +
                    "            " + cmnService.J_SQLDBFormat(strShowHelpEmployeeMatrix, J_SQLColFormat.Case_End) + " AS EMPLOYEE_CATEGORY," +
                    "             COR_TRN_SALARY_DETAILS_194P.TS_GS_SEC_17_1                         AS TS_GS_SEC_17_1," +
                    "             COR_TRN_SALARY_DETAILS_194P.US_16_III                              AS US_16_III," +
                    "             COR_TRN_SALARY_DETAILS_194P.US_16_IA                               AS US_16_IA," +
                    "             COR_TRN_SALARY_DETAILS_194P.US_16_TOTAL                            AS US_16_TOTAL," +
                    "             COR_TRN_SALARY_DETAILS_194P.INCOME_CHARGEABLE                      AS INCOME_CHARGEABLE," +
                    "             COR_TRN_SALARY_DETAILS_194P.INTEREST_INCOME                        AS INTEREST_INCOME," +
                    "             COR_TRN_SALARY_DETAILS_194P.GROSS_TOTAL_INCOME                     AS GROSS_TOTAL_INCOME," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80C_GROSS_AMOUNT               AS CVIA_SEC80C_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80C_DED_AMOUNT                 AS CVIA_SEC80C_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80CCC_GROSS_AMOUNT             AS CVIA_SEC80CCC_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80CCC_DED_AMOUNT               AS CVIA_SEC80CCC_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80CCD_GROSS_AMOUNT             AS CVIA_SEC80CCD_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80CCD_DED_AMOUNT               AS CVIA_SEC80CCD_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_TOTAL_80C_80CCC_80CCD_GROSS_AMOUNT  AS CVIA_TOTAL_80C_80CCC_80CCD_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_TOTAL_80C_80CCC_80CCD_DED_AMOUNT  AS CVIA_TOTAL_80C_80CCC_80CCD_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80CCD1B_GROSS_AMOUNT           AS CVIA_SEC80CCD1B_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80CCD1B_DED_AMOUNT             AS CVIA_SEC80CCD1B_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80D_GROSS_AMOUNT               AS CVIA_SEC80D_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80D_DED_AMOUNT                 AS CVIA_SEC80D_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80E_GROSS_AMOUNT               AS CVIA_SEC80E_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80E_DED_AMOUNT                 AS CVIA_SEC80E_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80G_QUAL_AMOUNT                AS CVIA_SEC80G_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80G_GROSS_AMOUNT               AS CVIA_SEC80G_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80G_DED_AMOUNT                 AS CVIA_SEC80G_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80TTB_QUAL_AMOUNT              AS CVIA_SEC80TTB_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80TTB_GROSS_AMOUNT             AS CVIA_SEC80TTB_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80TTB_DED_AMOUNT               AS CVIA_SEC80TTB_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80DD_QUAL_AMOUNT               AS CVIA_SEC80DD_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80DD_GROSS_AMOUNT              AS CVIA_SEC80DD_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80DD_DED_AMOUNT                AS CVIA_SEC80DD_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80EE_QUAL_AMOUNT               AS CVIA_SEC80EE_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80EE_GROSS_AMOUNT              AS CVIA_SEC80EE_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80EE_DED_AMOUNT                AS CVIA_SEC80EE_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80EEA_QUAL_AMOUNT              AS CVIA_SEC80EEA_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80EEA_GROSS_AMOUNT             AS CVIA_SEC80EEA_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80EEA_DED_AMOUNT               AS CVIA_SEC80EEA_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80GG_QUAL_AMOUNT               AS CVIA_SEC80GG_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80GG_GROSS_AMOUNT              AS CVIA_SEC80GG_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80GG_DED_AMOUNT                AS CVIA_SEC80GG_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80GGC_QUAL_AMOUNT              AS CVIA_SEC80GGC_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80GGC_GROSS_AMOUNT             AS CVIA_SEC80GGC_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80GGC_DED_AMOUNT               AS CVIA_SEC80GGC_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80U_QUAL_AMOUNT                AS CVIA_SEC80U_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80U_GROSS_AMOUNT               AS CVIA_SEC80U_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_SEC80U_DED_AMOUNT                 AS CVIA_SEC80U_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_OTHERS_QUAL_AMOUNT                AS CVIA_OTHERS_QUAL_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_OTHERS_GROSS_AMOUNT               AS CVIA_OTHERS_GROSS_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_OTHERS_DED_AMOUNT                 AS CVIA_OTHERS_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.CVIA_AGGREGATE_DED_AMOUNT              AS CVIA_AGGREGATE_DED_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.TOTAL_TAXABLE_INCOME                   AS TOTAL_TAXABLE_INCOME," +
                    "             COR_TRN_SALARY_DETAILS_194P.TAX_TOTAL_INCOME                       AS TAX_TOTAL_INCOME," +
                    "             COR_TRN_SALARY_DETAILS_194P.REBATE_US_87A_AMOUNT                   AS REBATE_US_87A_AMOUNT," +
                    "             COR_TRN_SALARY_DETAILS_194P.SCHG_TOTAL_INCOME                      AS SCHG_TOTAL_INCOME," +
                    "             COR_TRN_SALARY_DETAILS_194P.ECESS_TOTAL_INCOME                     AS ECESS_TOTAL_INCOME," +
                    "             COR_TRN_SALARY_DETAILS_194P.TAX_PAYABLE                            AS TAX_PAYABLE," +
                    "             COR_TRN_SALARY_DETAILS_194P.US_89_LESS                             AS US_89_LESS," +
                    "             COR_TRN_SALARY_DETAILS_194P.NET_TAX_PAYABLE                        AS NET_TAX_PAYABLE," +
                    "             COR_TRN_SALARY_DETAILS_194P.SECTION_115BAC_FLAG                    AS SECTION_115BAC_FLAG " +
                    "     FROM    COR_TRN_SALARY_DETAILS_194P " +
                    "     WHERE   COR_TRN_SALARY_DETAILS_194P.TRN_SALARY_DETAILS_194P_ID = " + Id + " ";
        ...
        #region ShowHDRRecord
        private bool ShowHDRRecord(long Id)
        {
            //Making the Reader Blocked until the reader gets closed
            blnRestrictIncomeTaxCalculation = true;
            ...
            try
            {
                string[,] strShowHelpEmployeeMatrix = {{"COR_HDR_SALARY_DETAILS_194P.EMPLOYEE_CATEGORY = 'S'", "F", T_EmployeeCategory.SeniorCitizen, "T"},
                                                       {"COR_HDR_SALARY_DETAILS_194P.EMPLOYEE_CATEGORY = 'O'", "F", T_EmployeeCategory.SuperSeniorCitizen, "T"},
                                                       {"COR_HDR_SALARY_DETAILS_194P.EMPLOYEE_CATEGORY = ''", "F", "", "T"}};
                ////
                strSQL = "SELECT  COR_HDR_SALARY_DETAILS_194P.HDR_SALARY_DETAILS_194P_ID                 AS SALARY_DETAILS_ID," +
                    "             COR_HDR_SALARY_DETAILS_194P.BATCH_HEADER_ID                          AS BASIC_INFO_ID," +
                    "             COR_HDR_SALARY_DETAILS_194P.EMPLOYEE_NAME                          AS EMPLOYEE_NAME," +
                    "             COR_HDR_SALARY_DETAILS_194P.EMPLOYEE_PAN                           AS EMPLOYEE_PAN," +
                    "             COR_HDR_SALARY_DETAILS_194P.EMPLOYEE_SERIAL_NO                     AS EMPLOYEE_REF," +
                    "            " + cmnService.J_SQLDBFormat(strShowHelpEmployeeMatrix, J_SQLColFormat.Case_End) + " AS EMPLOYEE_CATEGORY," +
                    "             COR_HDR_SALARY_DETAILS_194P.TS_GS_SEC_17_1                         AS TS_GS_SEC_17_1," +
                    "             COR_HDR_SALARY_DETAILS_194P.US_16_III                              AS US_16_III," +
                    "             COR_HDR_SALARY_DETAILS_194P.US_16_IA                               AS US_16_IA," +
                    "             COR_HDR_SALARY_DETAILS_194P.US_16_TOTAL                            AS US_16_TOTAL," +
                    "             COR_HDR_SALARY_DETAILS_194P.INCOME_CHARGEABLE                      AS INCOME_CHARGEABLE," +
                    "             COR_HDR_SALARY_DETAILS_194P.INTEREST_INCOME                        AS INTEREST_INCOME," +
                    "             COR_HDR_SALARY_DETAILS_194P.GROSS_TOTAL_INCOME                     AS GROSS_TOTAL_INCOME," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80C_GROSS_AMOUNT               AS CVIA_SEC80C_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80C_DED_AMOUNT                 AS CVIA_SEC80C_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80CCC_GROSS_AMOUNT             AS CVIA_SEC80CCC_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80CCC_DED_AMOUNT               AS CVIA_SEC80CCC_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80CCD_GROSS_AMOUNT             AS CVIA_SEC80CCD_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80CCD_DED_AMOUNT               AS CVIA_SEC80CCD_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_TOTAL_80C_80CCC_80CCD_GROSS_AMOUNT  AS CVIA_TOTAL_80C_80CCC_80CCD_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_TOTAL_80C_80CCC_80CCD_DED_AMOUNT  AS CVIA_TOTAL_80C_80CCC_80CCD_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80CCD1B_GROSS_AMOUNT           AS CVIA_SEC80CCD1B_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80CCD1B_DED_AMOUNT             AS CVIA_SEC80CCD1B_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80D_GROSS_AMOUNT               AS CVIA_SEC80D_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80D_DED_AMOUNT                 AS CVIA_SEC80D_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80E_GROSS_AMOUNT               AS CVIA_SEC80E_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80E_DED_AMOUNT                 AS CVIA_SEC80E_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80G_QUAL_AMOUNT                AS CVIA_SEC80G_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80G_GROSS_AMOUNT               AS CVIA_SEC80G_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80G_DED_AMOUNT                 AS CVIA_SEC80G_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80TTB_QUAL_AMOUNT              AS CVIA_SEC80TTB_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80TTB_GROSS_AMOUNT             AS CVIA_SEC80TTB_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80TTB_DED_AMOUNT               AS CVIA_SEC80TTB_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80DD_QUAL_AMOUNT               AS CVIA_SEC80DD_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80DD_GROSS_AMOUNT              AS CVIA_SEC80DD_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80DD_DED_AMOUNT                AS CVIA_SEC80DD_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80EE_QUAL_AMOUNT               AS CVIA_SEC80EE_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80EE_GROSS_AMOUNT              AS CVIA_SEC80EE_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80EE_DED_AMOUNT                AS CVIA_SEC80EE_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80EEA_QUAL_AMOUNT              AS CVIA_SEC80EEA_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80EEA_GROSS_AMOUNT             AS CVIA_SEC80EEA_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80EEA_DED_AMOUNT               AS CVIA_SEC80EEA_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80GG_QUAL_AMOUNT               AS CVIA_SEC80GG_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80GG_GROSS_AMOUNT              AS CVIA_SEC80GG_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80GG_DED_AMOUNT                AS CVIA_SEC80GG_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80GGC_QUAL_AMOUNT              AS CVIA_SEC80GGC_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80GGC_GROSS_AMOUNT             AS CVIA_SEC80GGC_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80GGC_DED_AMOUNT               AS CVIA_SEC80GGC_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80U_QUAL_AMOUNT                AS CVIA_SEC80U_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80U_GROSS_AMOUNT               AS CVIA_SEC80U_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_SEC80U_DED_AMOUNT                 AS CVIA_SEC80U_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_OTHERS_QUAL_AMOUNT                AS CVIA_OTHERS_QUAL_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_OTHERS_GROSS_AMOUNT               AS CVIA_OTHERS_GROSS_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_OTHERS_DED_AMOUNT                 AS CVIA_OTHERS_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.CVIA_AGGREGATE_DED_AMOUNT              AS CVIA_AGGREGATE_DED_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.TOTAL_TAXABLE_INCOME                   AS TOTAL_TAXABLE_INCOME," +
                    "             COR_HDR_SALARY_DETAILS_194P.TAX_TOTAL_INCOME                       AS TAX_TOTAL_INCOME," +
                    "             COR_HDR_SALARY_DETAILS_194P.REBATE_US_87A_AMOUNT                   AS REBATE_US_87A_AMOUNT," +
                    "             COR_HDR_SALARY_DETAILS_194P.SCHG_TOTAL_INCOME                      AS SCHG_TOTAL_INCOME," +
                    "             COR_HDR_SALARY_DETAILS_194P.ECESS_TOTAL_INCOME                     AS ECESS_TOTAL_INCOME," +
                    "             COR_HDR_SALARY_DETAILS_194P.TAX_PAYABLE                            AS TAX_PAYABLE," +
                    "             COR_HDR_SALARY_DETAILS_194P.US_89_LESS                             AS US_89_LESS," +
                    "             COR_HDR_SALARY_DETAILS_194P.NET_TAX_PAYABLE                        AS NET_TAX_PAYABLE," +
                    "             COR_HDR_SALARY_DETAILS_194P.SECTION_115BAC_FLAG                    AS SECTION_115BAC_FLAG " +
                    "     FROM    (COR_HDR_SALARY_DETAILS_194P LEFT JOIN COR_TRN_SALARY_DETAILS_194P " +
                    "             ON COR_HDR_SALARY_DETAILS_194P.HDR_SALARY_DETAILS_194P_ID = COR_TRN_SALARY_DETAILS_194P.HDR_SALARY_DETAILS_194P_ID) " +
                    "     WHERE   COR_TRN_SALARY_DETAILS_194P.TRN_SALARY_DETAILS_194P_ID = " + Id + " ";
                ...
                while (drdShowRecord.Read())
                {
                    ...
                    //txtEmployeeID.Tag = Convert.ToString(drdShowRecord["EMPLOYEE_ID"]);

                    blnShowHelp = false;
                    txtEmployeeName.Tag = Convert.ToString(drdShowRecord["EMPLOYEE_NAME"]);
                    blnShowHelp = true;

                    blnShowPANHelp = false;
                    txtEmployeePAN.Tag = Convert.ToString(drdShowRecord["EMPLOYEE_PAN"]);
                    blnShowPANHelp = true;

                    if (TDSMAN.Classes.TDSMAN.T_pEditionType == T_EDITION_TYPE.ENTERPRISE_EDITION || TDSMAN.Classes.TDSMAN.T_pEditionType == T_EDITION_TYPE.ENTERPRISE_LITE_EDITION || TDSMAN.Classes.TDSMAN.T_pEditionType == T_EDITION_TYPE.ENTERPRISE_ULTIMATE_EDITION)
                        if (Convert.ToString(drdShowRecord["EMPLOYEE_REF"]) != "")
                            txtEmpRefNo.Tag = string.Format("{0:0000000000}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["EMPLOYEE_REF"])));

                    cmbEmployeeCategory.Tag = Convert.ToString(drdShowRecord["EMPLOYEE_CATEGORY"]);
                    //    //
                    //    if (Convert.ToString(drdShowRecord["ENTRY_MODE"]) == "0")
                    //    {
                    //        rbnMandatoryOptions.Enabled = true;
                    ...
                    if (Convert.ToString(drdShowRecord["SECTION_115BAC_FLAG"]) == "0")
                        ...
                        chkTaxation115BAC.Checked = true;
                    //    //--
                    txtGSSec17_1.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["TS_GS_SEC_17_1"])));
                    lblTotalGS.Tag = txtGSSec17_1.Text;
                    txtDedTaxEmployment.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["US_16_III"])));
                    txtDedTaxSection16ia.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["US_16_IA"])));
                    lblTotalDeductions.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["US_16_TOTAL"])));
                    lblIncomeChargeable.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["INCOME_CHARGEABLE"])));
                    txtInterestIncome.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["INTEREST_INCOME"])));
                    lblGrossTotalIncome.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["GROSS_TOTAL_INCOME"])));
                    txtSec80C1819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80C_GROSS_AMOUNT"])));
                    txtSec80C1819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80C_DED_AMOUNT"])));
                    txtSec80CCC1819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80CCC_GROSS_AMOUNT"])));
                    txtSec80CCC1819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80CCC_DED_AMOUNT"])));
                    txtSec80CCD11819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80CCD_GROSS_AMOUNT"])));
                    txtSec80CCD11819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80CCD_DED_AMOUNT"])));
                    txtTotalDeductionSec80C80CCC80CCD11819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_TOTAL_80C_80CCC_80CCD_GROSS_AMOUNT"])));
                    txtTotalDeductionSec80C80CCC80CCD11819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_TOTAL_80C_80CCC_80CCD_DED_AMOUNT"])));
                    txtSec80CCD1B1819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80CCD1B_GROSS_AMOUNT"])));
                    txtSec80CCD1B1819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80CCD1B_DED_AMOUNT"])));
                    txtSec80D1819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80D_GROSS_AMOUNT"])));
                    txtSec80D1819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80D_DED_AMOUNT"])));
                    txtSec80E1819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80E_GROSS_AMOUNT"])));
                    txtSec80E1819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80E_DED_AMOUNT"])));
                    txtSec80G1819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80G_GROSS_AMOUNT"])));
                    txtSec80G1819QualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80G_QUAL_AMOUNT"])));
                    txtSec80G1819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80G_DED_AMOUNT"])));
                    txtSec80TTB1819GrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80TTB_GROSS_AMOUNT"])));
                    txtSec80TTB1819QualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80TTB_QUAL_AMOUNT"])));
                    txtSec80TTB1819DeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80TTB_DED_AMOUNT"])));
                    txtSec80DDGrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80DD_GROSS_AMOUNT"])));
                    txtSec80DDQualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80DD_QUAL_AMOUNT"])));
                    txtSec80DDDeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80DD_DED_AMOUNT"])));
                    txtSec80EEGrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80EE_GROSS_AMOUNT"])));
                    txtSec80EEQualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80EE_QUAL_AMOUNT"])));
                    txtSec80EEDeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80EE_DED_AMOUNT"])));
                    txtSec80EEAGrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80EEA_GROSS_AMOUNT"])));
                    txtSec80EEAQualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80EEA_QUAL_AMOUNT"])));
                    txtSec80EEADeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80EEA_DED_AMOUNT"])));
                    txtSec80GGGrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80GG_GROSS_AMOUNT"])));
                    txtSec80GGQualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80GG_QUAL_AMOUNT"])));
                    txtSec80GGDeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80GG_DED_AMOUNT"])));
                    txtSec80GGCGrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80GGC_GROSS_AMOUNT"])));
                    txtSec80GGCQualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80GGC_QUAL_AMOUNT"])));
                    txtSec80GGCDeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80GGC_DED_AMOUNT"])));
                    txtSec80UGrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80U_GROSS_AMOUNT"])));
                    txtSec80UQualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80U_QUAL_AMOUNT"])));
                    txtSec80UDeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_SEC80U_DED_AMOUNT"])));
                    txtSecOthersGrossAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_OTHERS_GROSS_AMOUNT"])));
                    txtSecOthersQualifyingAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_OTHERS_QUAL_AMOUNT"])));
                    txtSecOthersDeductibleAmount.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_OTHERS_DED_AMOUNT"])));
                    lblSecChVIATotalAmt.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["CVIA_AGGREGATE_DED_AMOUNT"])));
                    txtTaxTotalIncome.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["TAX_TOTAL_INCOME"])));
                    txtRebateSec87.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["REBATE_US_87A_AMOUNT"])));
                    txtSurcharge.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["SCHG_TOTAL_INCOME"])));
                    txtEducationCess.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["ECESS_TOTAL_INCOME"])));
                    lblGrossTaxPayable.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["TAX_PAYABLE"])));
                    txtReliefUS89.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["US_89_LESS"])));
                    lblNetTaxPayable.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["NET_TAX_PAYABLE"])));
                    //    //
                    //    //-- 2020/05/19
                    lblTotalTaxableIncome.Tag = string.Format("{0:0.00}", cmnService.J_ReturnDoubleValue(Convert.ToString(drdShowRecord["TOTAL_TAXABLE_INCOME"])));
                    //    //
                    //    //if (cmnService.J_ReturnInt32Value(Convert.ToString(drdShowRecord["FORM16B_NEW_FORMAT_18_19"])) > 0)
                    //    //    FORM_DESIGN(true);
                    //    //else
                    ...
                    drdShowRecord.Close();
                    drdShowRecord.Dispose();
                    //    //cmbState.Tag = strStateName;
                    //    //cmbDistrict.Tag = strDistrictName;
...
//                    return;
//                //--                    
//                strSQL = @"SELECT DISTINCT CVIA_SEC80C_ITEM_1_DESC AS CVIA_SEC80C_ITEM_DESC FROM COR_TRN_SALARY_DETAILS_194P WHERE CVIA_SEC80C_ITEM_1_DESC LIKE '" + strTEXT + @"%' AND CVIA_SEC80C_ITEM_1_DESC <> ''
//                            UNION
//                            SELECT DISTINCT CVIA_SEC80C_ITEM_2_DESC AS CVIA_SEC80C_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE CVIA_SEC80C_ITEM_2_DESC LIKE '" + strTEXT + @"%' AND  CVIA_SEC80C_ITEM_2_DESC <> ''
//                            UNION
//                            SELECT DISTINCT CVIA_SEC80C_ITEM_3_DESC AS CVIA_SEC80C_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE CVIA_SEC80C_ITEM_3_DESC LIKE '" + strTEXT + @"%' AND  CVIA_SEC80C_ITEM_3_DESC <> ''
//                            UNION
//                            SELECT DISTINCT CVIA_SEC80C_ITEM_4_DESC AS CVIA_SEC80C_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE CVIA_SEC80C_ITEM_4_DESC LIKE '" + strTEXT + @"%' AND  CVIA_SEC80C_ITEM_4_DESC <> ''
//                            UNION
//                            SELECT DISTINCT CVIA_SEC80C_ITEM_5_DESC AS CVIA_SEC80C_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE CVIA_SEC80C_ITEM_5_DESC LIKE '" + strTEXT + @"%' AND  CVIA_SEC80C_ITEM_5_DESC <> ''";
//                //--
...
//                    return;
//                //--                    
//                strSQL = @"SELECT DISTINCT AIS_ITEM_1_DESC AS AIS_ITEM_DESC FROM COR_TRN_SALARY_DETAILS_194P WHERE AIS_ITEM_1_DESC LIKE '" + strTEXT + @"%' AND AIS_ITEM_1_DESC <> ''
//                           UNION
//                           SELECT DISTINCT AIS_ITEM_2_DESC AS AIS_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE AIS_ITEM_2_DESC LIKE '" + strTEXT + @"%' AND  AIS_ITEM_2_DESC <> ''
//                           UNION
//                           SELECT DISTINCT AIS_ITEM_3_DESC AS AIS_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE AIS_ITEM_3_DESC LIKE '" + strTEXT + @"%' AND  AIS_ITEM_3_DESC <> ''
//                           UNION
//                           SELECT DISTINCT AIS_ITEM_4_DESC AS AIS_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE AIS_ITEM_4_DESC LIKE '" + strTEXT + @"%' AND  AIS_ITEM_4_DESC <> ''";
//                //--
...
//                           SELECT DISTINCT CVIA_SEC80C_ITEM_5_DESC AS CVIA_SEC80C_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE CVIA_SEC80C_ITEM_5_DESC LIKE '" + strTEXT + @"%' AND  CVIA_SEC80C_ITEM_5_DESC <> ''
//                           UNION
//                           SELECT DISTINCT CVIA_SEC80C_ITEM_6_DESC AS CVIA_SEC80C_ITEM_DESC  FROM COR_TRN_SALARY_DETAILS_194P WHERE CVIA_SEC80C_ITEM_6_DESC LIKE '" + strTEXT + @"%' AND  CVIA_SEC80C_ITEM_6_DESC <> ''";
//                //--
...
//            }
        ...
        #region ChkGenerateModifyValues

        private bool ChkGenerateModifyValues()
        {
            if (txtEmployeeName.Text.Trim() != txtEmployeeName.Tag)
            {
                return false;
            }
            //-- 2020/06/02
            if (txtEmpRefNo.Text.Trim() != txtEmpRefNo.Tag)
            {
                return false;
            }
            //
            if (cmbEmployeeCategory.Text.Trim() != cmbEmployeeCategory.Tag)
            {
                return false;
            }
            //
            if (txtGSSec17_1.Text.Trim() != txtGSSec17_1.Tag)
            {
                return false;
            }
            //
            //if (txtDedEntAllowance.Text.Trim() != txtDedEntAllowanceM.Tag)
            //{
            //    return false;
            //}
            //
            if (txtDedTaxEmployment.Text.Trim() != txtDedTaxEmployment.Tag)
            {
                return false;
            }
            //
            if (txtDedTaxSection16ia.Text.Trim() != txtDedTaxSection16ia.Tag)
            {
                return false;
            }            
            //
            if (txtInterestIncome.Text.Trim() != txtInterestIncome.Tag)
            {
                return false;
            }
            //
            if (txtSec80C1819GrossAmount.Text.Trim() != txtSec80C1819GrossAmount.Tag)
            {
                return false;
            }
            if (txtSec80C1819DeductibleAmount.Text.Trim() != txtSec80C1819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80CCC1819GrossAmount.Text.Trim() != txtSec80CCC1819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80CCC1819DeductibleAmount.Text.Trim() != txtSec80CCC1819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80CCD11819GrossAmount.Text.Trim() != txtSec80CCD11819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtTotalDeductionSec80C80CCC80CCD11819GrossAmount.Text.Trim() != txtTotalDeductionSec80C80CCC80CCD11819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtTotalDeductionSec80C80CCC80CCD11819DeductibleAmount.Text.Trim() != txtTotalDeductionSec80C80CCC80CCD11819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80CCD1B1819GrossAmount.Text.Trim() != txtSec80CCD1B1819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80CCD1B1819DeductibleAmount.Text.Trim() != txtSec80CCD1B1819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80D1819GrossAmount.Text.Trim() != txtSec80D1819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80D1819DeductibleAmount.Text.Trim() != txtSec80D1819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80E1819GrossAmount.Text.Trim() != txtSec80E1819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80E1819DeductibleAmount.Text.Trim() != txtSec80E1819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80G1819GrossAmount.Text.Trim() != txtSec80G1819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80G1819QualifyingAmount.Text.Trim() != txtSec80G1819QualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80G1819DeductibleAmount.Text.Trim() != txtSec80G1819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80TTB1819GrossAmount.Text.Trim() != txtSec80TTB1819GrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80TTB1819QualifyingAmount.Text.Trim() != txtSec80TTB1819QualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80TTB1819DeductibleAmount.Text.Trim() != txtSec80TTB1819DeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80DDGrossAmount.Text.Trim() != txtSec80DDGrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80DDQualifyingAmount.Text.Trim() != txtSec80DDQualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80DDDeductibleAmount.Text.Trim() != txtSec80DDDeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80EEGrossAmount.Text.Trim() != txtSec80EEGrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80EEQualifyingAmount.Text.Trim() != txtSec80EEQualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80EEDeductibleAmount.Text.Trim() != txtSec80EEDeductibleAmount.Tag)
            {
                return false;
            }
            //
            //
            if (txtSec80EEAGrossAmount.Text.Trim() != txtSec80EEAGrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80EEAQualifyingAmount.Text.Trim() != txtSec80EEAQualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80EEADeductibleAmount.Text.Trim() != txtSec80EEADeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80GGGrossAmount.Text.Trim() != txtSec80GGGrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80GGQualifyingAmount.Text.Trim() != txtSec80GGQualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80GGDeductibleAmount.Text.Trim() != txtSec80GGDeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80GGCGrossAmount.Text.Trim() != txtSec80GGCGrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80GGCQualifyingAmount.Text.Trim() != txtSec80GGCQualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80GGCDeductibleAmount.Text.Trim() != txtSec80GGCDeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80UGrossAmount.Text.Trim() != txtSec80UGrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80UQualifyingAmount.Text.Trim() != txtSec80UQualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSec80UDeductibleAmount.Text.Trim() != txtSec80UDeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtSecOthersGrossAmount.Text.Trim() != txtSecOthersGrossAmount.Tag)
            {
                return false;
            }
            //
            if (txtSecOthersQualifyingAmount.Text.Trim() != txtSecOthersQualifyingAmount.Tag)
            {
                return false;
            }
            //
            if (txtSecOthersDeductibleAmount.Text.Trim() != txtSecOthersDeductibleAmount.Tag)
            {
                return false;
            }
            //
            if (txtTaxTotalIncome.Text.Trim() != txtTaxTotalIncome.Tag)
            {
                return false;
            }
            //
            if (txtRebateSec87.Text.Trim() != txtRebateSec87.Tag)
            {
                return false;
            }
            //
            if (txtSurcharge.Text.Trim() != txtSurcharge.Tag)
            {
                return false;
            }
            //
            if (txtEducationCess.Text.Trim() != txtEducationCess.Tag)
            {
                return false;
            }
            //
            if (txtReliefUS89.Text.Trim() != txtReliefUS89.Tag)
            {
                return false;
            }
            //--
        ...
        #region ChkGenerateModifyPAN

        private bool ChkGenerateModifyPAN()
        {
            //
            if (txtEmployeePAN.Text.Trim() != txtEmployeePAN.Tag)
            {
                return false;
            }
            //
